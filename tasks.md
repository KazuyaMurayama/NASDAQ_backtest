...
- [x] **CAGR押上げ＝レバ拡張×早期再エントリー（2026-06-16・[結果](LEVERUP_EXTENSION_RESULTS_20260616.md)/計画=LEVERUP_EXTENSION_PLAN_20260616.md）**: B3aの uniform leverage を1.15超へ拡張＋DH-W1 enter早期化を検証。**CAGRは scale を上げれば min⓽ +20.98%→約+24%までベト内で伸びる（ベト到達≈scale1.50）**。最良: scale1.35強map +23.83%(MaxDD−45%・CI95_lo+26.6%)／combo上端 +23.97%(MaxDD−48%ベト間近)。**ただし純レバ＝Sharpe非改善・MaxDD/worst年(−26%)比例悪化・min改善95%非有意**。早期再エントリーenter0.60はSharpe0.955だがgap圧縮がOOS強気依存。**レバ点はリスク選好で選ぶ＝B3aバランス維持・やや攻めscale1.25・攻めscale1.35強map**。REGISTRY §2登録・CURRENT_BESTダイヤル表。
- [x] **QC v2（2026-06-16・独立4エージェント＋方法論レビュー・[サインオフ](LEVERUP_EXTENSION_QC_SIGNOFF_20260616.md)）**: 数値正(独立再現<0.005pp・look-ahead実害なし)。**OOS境界感度でscale1.25/1.35のCAGR優位はOOS境界(2019-2023)頑健と確認＝単一強気OOS依存を否定**。是正: 純レバ＝Sharpe非改善・95%非有意・bear比例超悪化(−5.40%)・**Worst5Yがscale1.25/comboでマイナス転落**を明記、**combo/enter0.60を非推奨へ格下げ**(OOS境界脆弱＋実運用MaxDDベト超過リスク)。既知の方法論限界=追証/容量/実運用MaxDDギャップ/累積多重検定の未モデル化（次工程候補）
- [x] **高レバ実務限界検証＋v2＋QC（2026-06-17・[結果](MARGIN_CAPACITY_STRESS_RESULTS_20260617.md)/ルール=EVALUATION_STANDARD §1.5）**: scale1.35強mapの追証/強制ロスカット・容量・最悪損害を全期間検証。**v2核心（M5b/M6/M7＋独立QC）**: (1)建玉可能性も継続コストも問題なし（L頭打ちゼロ・必要証拠金AUM比 平均3.56/最大22.3%＝自己資金でカバー・唯一の継続コスト=金利相当額は計上済）。**【v4最終是正 2026-06-18】証拠金は担保で損益は建玉全額に発生→継続的CAGRドラッグを生まない（eToro等URL根拠）。v2(−0.9pp ×SOFR)もv3(−3.42pp 現金取り置き＝過少投資)も却下**。scale1.35強map 税後min⓽ **+23.83%は到達可能**（M6: 強制清算は1975-77に3回・各13-19%AUMだがCAGR+0.08pp・min⓽不変、5大危機はOUTで0）。実コストはテールリスクで、リスク低減はレバ水準（de-lever）。正典=[PRODUCT_COST_COMPARISON §10](PRODUCT_COST_COMPARISON_2026-06-10.md)。(2)歴史的大暴落は全てOUT＝追証ゼロ（実数確認）だが**構造保証でなく史実+運**（Exit平均17日・高レバIN中の突発急落で最大excess×Black MondayはAUM−31%）。(3)容量はTFX実OI83,035枚で**¥300M超まで非制約**（前回「薄い市場」推定は誤りで解消・OIモニタ要）。**採用条件=証拠金8〜12%＋容量内AUM**。独立QC(A/B)でロジック妥当性確認・M5b過剰補正の残バイアス(S2基準/ロールコスト)も是正
- [x] **ルール整備（2026-06-17 v1.7新設→2026-06-18 v1.9 最終是正）**: 「>3xレバ証拠金コスト前提」を [EVALUATION_STANDARD.md §1.5](EVALUATION_STANDARD.md)＋[PRODUCT_COST_COMPARISON §10](PRODUCT_COST_COMPARISON_2026-06-10.md)＋`src/product_costs.py` に整備（URL根拠付き）。**v1.9 最終結論＝証拠金は担保で継続的CAGRドラッグを生まない（v2 ×SOFR −0.9pp・v3 現金取り置き −3.42pp とも却下）。唯一の継続コストは金利相当額（計上済）、実コストはテールリスク、リスク低減はレバ水準（de-lever）**。独立QC APPROVE_W_CAVEATS。今後の>3x検証で必須・証拠金ゼロ前提は非標準（参考値）
