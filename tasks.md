...
- [x] **高レバ実務限界検証＋v2＋QC（2026-06-17・[結果](MARGIN_CAPACITY_STRESS_RESULTS_20260617.md)/ルール=EVALUATION_STANDARD §1.5）**: scale1.35強mapの追証/強制ロスカット・容量・最悪損害を全期間検証。**v2核心（M5b/M6/M7＋独立QC）**: (1)証拠金は取り置くが現実ドラッグは穏当＝S2分離+ロールコストで **scale1.35≈−0.9pp/B3a≈−0.7pp/P09≈−0.1pp・高レバ優位保存**（M5 v1の−10pp崩壊は資本会計バグで撤回）。(2)歴史的大暴落は全てOUT＝追証ゼロ（実数確認）だが**構造保証でなく史実+運**（Exit平均17日・高レバIN中の突発急落で最大excess×Black MondayはAUM−31%）。(3)容量はTFX実OI83,035枚で**¥300M超まで非制約**（前回「薄い市場」推定は誤りで解消・OIモニタ要）。**採用条件=証拠金8〜12%＋容量内AUM**。独立QC(A/B)でロジック妥当性確認・M5b過剰補正の残バイアス(S2基準/ロールコスト)も是正
- [x] **ルール整備（2026-06-17・v1.7）**: 「>3xレバ＝証拠金取り置き8%＋無利息＋資金制約＋ロールコスト＋容量」を [EVALUATION_STANDARD.md §1.5](EVALUATION_STANDARD.md) に新設（§0サマリ・PRチェックリスト反映）＋ `src/product_costs.py` に K365_* 定数追加。今後の>3x戦略検証で必須・証拠金ゼロ前提は非標準（参考値）
