...
- [x] **高レバ実務限界検証（2026-06-17・[結果](MARGIN_CAPACITY_STRESS_RESULTS_20260617.md)/計画=MARGIN_CAPACITY_STRESS_PLAN_20260617.md）**: scale1.35強mapの追証/強制ロスカット・容量・最悪損害を全期間検証。清算トリガー＝NASDAQ単日下落≥k365証拠金率（レバ非依存）。最小4.24%だと単日−4.24%で清算(全期間81日・1987/2008/COVID全清算)→CAGR−1.18pp・最悪1イベント−34%AUM。**証拠金8%(excess建玉の15-20%余剰現金)で全期間52年清算0回・損害0pp**。容量=k365薄く(推定OI500-10,000枚)AUM¥30-100M帯が天井(実OI確認前提・データギャップ)。**採用条件＝8%証拠金＋容量内AUM**。保守化はレバ点↓で同時軽減
