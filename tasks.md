...
## ✅ Completed
- 2026-06-03: **DH Dyn 2x3x [A] 改善 4 変種 (T1〜T4) 検証完了** — 商品 (TQQQ+TMF+2036) 維持で NEW CANDIDATE / D5 のタイミング機構 (vz=0.65 ゲート / lmax=5.5 cap / F10 ε=0.015) を移植。`src/g18_daily_trade_cost_wfa.py` に `build_dh_nav_with_timing_cost` 新関数追加（既存 `build_dh_nav_with_cost` は無変更）。`src/g21a〜g21f` 6 本で NAV / 9 指標 / WFA 50 窓 / Bootstrap / Permutation / 年次寄与分解を実行。最良 **DH-T4 (Full)** は IS-OOS gap +10.46→+6.81pp (-3.65pp) / WFE 0.662→0.920 で過学習抑制 ✅ / WFA PASS (CI95_lo=+14.24%, p=0) ✅ / MaxDD -37.63%・P10_5Y +9.80% は新最良。一方 Bootstrap CI95=[-6.33, +5.88pp] noise 内 / Permutation marginal (NULL 下位 7.0%) / OOS 2021 -11.03pp 集中で Active 昇格は v8 以降に保留。`STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md` を v3 化、§0'/§5/§6/§6-2 の 4 箇所に T4 を DH の 1 つ手前で追加。STRATEGY_REGISTRY.md §2 Shortlisted に `DH_T4_Improved` 登録。
