...
> このインデックスには全ブランチのファイル一覧・説明・重要度・所在ブランチが記載されています。

### 最新の戦略・研究結果を確認する場合の手順

1. **`FILE_INDEX.md` の「最新情報の確認フロー」セクションを確認する**
2. **同種ファイルが複数ある場合は、ファイル名の日付サフィックス（`_YYYY-MM-DD`）が最新のものを最優先で参照する**
   - 例: `SESSION_SUMMARY_2026-04-04.md` と `SESSION_SUMMARY_2026-03-01.md` → 2026-04-04 を参照
3. 生成ドキュメントはすべて `FILENAME_YYYY-MM-DD.md` 形式で命名されている（ルール）
4. `CLAUDE.md` / `FINAL_RESULTS_*.md` は概要・旧版として扱い、最新数値の根拠としない

> ⚠️ **注意**: `CLAUDE.md`（このファイル）に記載のパフォーマンス数値は古い場合があります。
> 最新のベスト戦略は **Dyn 2x3x (B0.55/L0.25/V0.1/G0.5)** で、
> CAGR 31.40% / Sharpe 1.297 / MaxDD -33.4% / Worst5Y +5.2%（2026-04-04 確定）。
