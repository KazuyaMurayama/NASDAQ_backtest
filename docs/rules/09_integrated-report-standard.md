...
- バージョン: v1.1
- 発効日: 2026-06-03
- 改訂日: 2026-06-03（v1.1: 適用範囲を「統合比較レポート」から「戦略検証全般」に拡大）
- 準拠: `STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md` §0'/§1'/§5/§6

> **戦略検証・採用判断を行うときは必ず §0' / §1' / §5 / §6 の4セクション必須。**
...
| **新戦略・改善候補を検証するとき（WFAを含む）** | 新アイデアの `g*_*.py` 検証スクリプト実行後 |
| **パラメータ sweep 後の有望候補を確定・報告するとき** | sweep 上位候補の9指標確定レポート |
| **既存 Active 戦略の再検証・更新をするとき** | 年次 bootstrap 更新、OOS追加年反映 |
| **複数戦略を並べて比較するレポートを作成するとき** | 候補比較、shortlist 更新 |

**適用外（sweep/gridスキャン段階）**: `STRATEGY_COMPARISON_*` 系・sweep MD の**初期スキャン**は §0'（10列）のみ必須（`CLAUDE.md §3` rules 1-6）。ただし**最終候補をレポートする段階では必ず4セクション必須**に移行する。
...
## §8 セルフチェックリスト（報告前に全項目確認・毎回必須）

> 戦略検証作業の完了報告前に、以下を全項目確認してから提出すること。

- [ ] **トリガー確認**: 今回の作業は §1 の適用トリガーに該当するか？（該当 → 以下を全チェック）
- [ ] §0' が `MD_HEADER_INTEGRATED` / `fmt_row_integrated` を使用しているか？（手書きヘッダ禁止）
- [ ] §0' に累積CAGR⓽列（OOS/IS 両値を1セルで並列表示）が含まれているか？
- [ ] §0' の全比較対象戦略 + NDX 1x B&H ベンチマーク行が揃っているか？
...
- [ ] 上記4セクションすべてのデータが最新の検証スクリプト出力から生成されているか？（古いコピーの流用禁止）
...
| v1.1 | 2026-06-03 | 適用範囲を「採用判断レポート」から「戦略検証全般」に拡大。§1 トリガー4件追加（新戦略テスト・sweep後確定・Active再検証・複数比較）。§8 チェックリスト2項目追加（トリガー確認・最新出力の確認） |
