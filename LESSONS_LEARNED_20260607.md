...
作成日: 2026-06-07
最終更新日: 2026-06-07 (v1.3: §6 環境別ベスト戦略表を canonical split (IS_END=2021-05-07) で再計算、pretax と完全同期、calendar/canonical split duality 廃止)
管理者: 男座員也 (Kazuya Oza)
...
- **`nasdaq_mom63 × S3 (DH-W1) × M6 defensive` (V7 pure_boost mapping)** — S3 overlay tuning sweep より
  ...
  - CAGR_IS +18.61% / CAGR_OOS +19.14% (canonical split) / IS-OOS gap **-0.53pp (overlay候補中最良)**
  ...
  - **税後 CAGR (canonical split)**: NISA 内非課税は pretax 値そのまま (V7 NISA OOS = **+19.14%**)。課税口座は yearly ×0.8273 を canonical split で適用 → V7 OOS aftertax = **+15.96%**。詳細 [`aftertax_cagr_canonical_20260607.csv`](data/signals/expansion/aftertax_cagr_canonical_20260607.csv) / 計算 [`scripts/compute_aftertax_cagr_v3_20260607.py`](scripts/compute_aftertax_cagr_v3_20260607.py)
...
> [`STRATEGY_REGISTRY.md`](STRATEGY_REGISTRY.md) §2 と同期。**pretax / aftertax 共に canonical split (IS_END=2021-05-07 / OOS_START=2021-05-08) ベース**、aftertax は yearly ×0.8273 を periodized returns に適用 ([`aftertax_cagr_canonical_20260607.csv`](data/signals/expansion/aftertax_cagr_canonical_20260607.csv))。

| 環境 | 戦略 | CAGR_IS (pretax) | CAGR_OOS (pretax) | OOS aftertax (CFD/ETF課税) | OOS aftertax (NISA非課税) | Sharpe_OOS | MaxDD | Worst10Y★ | P10_5Y▷ | Trades/yr | 推奨 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **CFD §1 Active** | E4 RegimeKLT (vz_thr=0.70) | +31.76% | +32.70% | **+28.01%** | N/A (NISA 非適用) | 0.892 | -60.01% | +18.67% | +9.78% | 27 | **維持** |
| CFD 副候補 | S1 F10 (vz=0.65, lmax=7, F10ε=0.015) | +32.58% | +35.95% | **+30.80%** | N/A | 0.935 | -63.09% | +18.58% | +10.27% | 119 (proxy) | Registry §2 |
| CFD 副候補 | S2 D5 (vz=0.65, lmax=5) | +29.61% | +32.72% | **+28.06%** | N/A | 0.950 | -51.82% | +18.33% | +11.87% | 119 (proxy) | Registry §2 |
| **ETF only** | S3 DH-W1 (Asymm+Hyst) | +18.10% | +18.88% | **+15.74%** | **+18.88%** (=pretax) | 0.845 | -34.57% | +10.37% | +4.82% | 69 (proxy) | Registry §2 |
| **ETF only ⭐ V0 def** | S3 + `nasdaq_mom63 × M6 def` overlay {1.1,1.0,0.9,0.8} | +16.69% | +18.03% | **+15.02%** | **+18.03%** (=pretax) | **0.892** | **-28.74%** | +10.75% | +5.21% | 59 (proxy) | Registry §2 ⭐ (MaxDD改善優先) |
| **ETF only ⭐ V7 pure_boost** (2026-06-07 追加) | S3 + `nasdaq_mom63 × M6 def` overlay {1.20,1.10,1.00,1.00} | **+18.61%** | **+19.14%** | **+15.96%** | **+19.14%** (=pretax) | 0.841 | -34.57% | +11.02% | +5.22% | 26.5 (proxy) | Registry §2 ⭐ (CAGR死守、min>18% target 達成) |

**18% target との関係 (V7, canonical split)**:
- NISA: V7 min(IS+18.61%, OOS+19.14%) = **+18.61% > 18% target 達成** (gap +0.61pp)
- 課税口座: V7 min(IS+15.74%, OOS+15.96%) = +15.74% (-2.26pp 未達)
