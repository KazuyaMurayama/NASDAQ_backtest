...
評価戦略:
  ...
  5. DH Dyn 6x2x3x [CFD-理論値]     ← CFD 6x (日本国内個人向け規制外・理論値)
  6. DH Dyn 7x2x3x [CFD-理論値]     ← CFD 7x (日本国内個人向け規制外・理論値)
...
⚠️ 6x/7x について:
  日本国内個人向けCFD商品として 6x/7x 専用商品は存在しない。
  CFD業者は最大10倍枠の中で設定可能だが、規制グレーゾーン/プロ口座向け。
  本バックテストは「理論値・レバレッジ感応度の上界確認」として実施。
...
# 6x/7x: 理論値フラグ (商品存在性確認済み: 日本国内個人向けには存在しない)
THEORETICAL_LEVERAGES = {6.0, 7.0}
...
def generate_report(all_metrics: dict, sensitivity: dict, data_info: dict) -> str:
    ...
    strat_order = [
        ...
        'DH Dyn 4x2x3x [CFD]',
        'DH Dyn 5x2x3x [CFD]',
        'DH Dyn 6x2x3x [CFD-理論値]',
        'DH Dyn 7x2x3x [CFD-理論値]',
        'A2 Optimized [TQQQ]',
    ]
    ...
    # --- Warning box ---
    lines.append('> ⚠️ **6x/7x は日本国内個人向けCFD規制範囲外の理論値**')
    lines.append('> - 調査結果: 6x/7x 専用NASDAQ CFD商品は日本国内個人向けには存在しない')
    lines.append('> - CFD業者の提供上限は最大10倍だが、6x/7xはプロ口座または合成ポートフォリオでのみ実現可能')
    lines.append('> - 本セクションはレバレッジ感応度の上界確認を目的とした理論値シミュレーション')
    ...
    m6 = all_metrics.get('DH Dyn 6x2x3x [CFD-理論値]', {})
    m7 = all_metrics.get('DH Dyn 7x2x3x [CFD-理論値]', {})
    ...
    lines.append(f'- **CFD 6x [理論値]**: CAGR {_fmt_pct(m6.get("CAGR_FULL"))}, MaxDD {_fmt_pct(m6.get("MaxDD_FULL"))}, Worst5Y {_fmt_pct(m6.get("Worst5Y"))}, NAV崩壊日 {_fmt_int(m6.get("Blowup_Days"))}日')
    lines.append(f'- **CFD 7x [理論値]**: CAGR {_fmt_pct(m7.get("CAGR_FULL"))}, MaxDD {_fmt_pct(m7.get("MaxDD_FULL"))}, Worst5Y {_fmt_pct(m7.get("Worst5Y"))}, NAV崩壊日 {_fmt_int(m7.get("Blowup_Days"))}日')
    ...
    lines.append('')
    lines.append('| # | 戦略名 | NASDAQスリーブ | Gold | Bond | シグナル | 実在性 |')
    lines.append('|---|--------|--------------|------|------|---------|--------|')
    lines.append('| 1 | DH Dyn 3x2x3x [TQQQ]       | TQQQ 3x (ETF, vol drag あり) | 2x | 3x | A2 Opt [A] | ○ 実在 |')
    lines.append('| 2 | DH Dyn 3x2x3x [CFD]         | CFD 3x (vol drag なし) | 2x | 3x | 同上 | ○ 実在 |')
    lines.append('| 3 | DH Dyn 4x2x3x [CFD]         | CFD 4x (vol drag なし) | 2x | 3x | 同上 | △ 指数CFD上限内 |')
    lines.append('| 4 | DH Dyn 5x2x3x [CFD]         | CFD 5x (vol drag なし) | 2x | 3x | 同上 | △ 指数CFD上限内 |')
    lines.append('| 5 | DH Dyn 6x2x3x [CFD-理論値]  | CFD 6x (vol drag なし) | 2x | 3x | 同上 | ✗ 理論値 |')
    lines.append('| 6 | DH Dyn 7x2x3x [CFD-理論値]  | CFD 7x (vol drag なし) | 2x | 3x | 同上 | ✗ 理論値 |')
    ...
    for name in strat_order:
        ...
        is_theory = '理論値' in name
        tag = ' ⚠️' if is_theory else ''
    ...
    dd_vals = [abs(all_metrics.get(f'DH Dyn {l}x2x3x [CFD{"" if l <= 5 else "-理論値"}]', {}).get('MaxDD_FULL', np.nan))
               for l in [3, 4, 5, 6, 7]]
    ...
    # CAGR monotonicity
    cagr_vals = [all_metrics.get(f'DH Dyn {l}x2x3x [CFD{"" if l <= 5 else "-理論値"}]', {}).get('CAGR_FULL', np.nan)
                 for l in [3, 4, 5, 6, 7]]
    ...
    # blowup check
    b6 = all_metrics.get('DH Dyn 6x2x3x [CFD-理論値]', {}).get('Blowup_Days', 0)
    b7 = all_metrics.get('DH Dyn 7x2x3x [CFD-理論値]', {}).get('Blowup_Days', 0)
    ...
    lines.append('### 全レバレッジのMaxDD・Worst5Y一覧')
    lines.append('')
    lines.append('| 戦略 | MaxDD | Worst5Y | Blowup日数 |')
    lines.append('|------|-------|---------|-----------|')
    for name in strat_order:
        m = all_metrics.get(name, {})
        lines.append(f'| {name} | {_fmt_pct(m.get("MaxDD_FULL"))} | {_fmt_pct(m.get("Worst5Y"))} | {_fmt_int(m.get("Blowup_Days", 0))} |')
    lines.append('')

    lines.append('### 実務上の制約')
    lines.append('')
    lines.append('- **6x/7x は日本国内規制外**: 個人向けCFDでは提供されない。プロ口座・海外業者でのみ可能。')
    ...
    lines.append(f'4. **CFD 6x [理論値]**: CAGR {_fmt_pct(m6.get("CAGR_FULL"))}, MaxDD {_fmt_pct(m6.get("MaxDD_FULL"))} — 日本国内規制外・実運用不可')
    lines.append(f'5. **CFD 7x [理論値]**: CAGR {_fmt_pct(m7.get("CAGR_FULL"))}, MaxDD {_fmt_pct(m7.get("MaxDD_FULL"))} — 日本国内規制外・実運用不可')
    ...
    lines.append('')
    lines.append('- 6x/7x は理論値であり実運用候補から除外')
...
def main():
    ...
    cfd_variants = [
        ...
        (6.0, 'DH Dyn 6x2x3x [CFD-理論値]'),
        (7.0, 'DH Dyn 7x2x3x [CFD-理論値]'),
    ]
    ...
    for i, (lev_cfd, name) in enumerate(cfd_variants, start=2):
        theory_mark = ' [理論値]' if lev_cfd in THEORETICAL_LEVERAGES else ''
