...
目的: P5_Kelly（連続 Kelly 公式の経験ベイズ近似）を
     Scenario D 条件で正式評価する。
     L* = safety × μ_excess / σ²

グリッド: safety × mu_window × sigma_window = 3×2×2 = 12 runs
  safety     ∈ {0.25, 0.5, 1.0}
  mu_window  ∈ {252, 504}
  sigma_window ∈ {30, 60}
...
出力:
  - コンソール結果テーブル
  - p5_kelly_sweep_results.csv  (プロジェクトルート)
  - P5_KELLY_SWEEP_2026-05-22.md (プロジェクトルート)
"""
...
from dynamic_leverage_strategies import compute_L_kelly
...
SAFETY_SWEEP  = [0.25, 0.5, 1.0]
MU_WIN_SWEEP  = [252, 504]
SIG_WIN_SWEEP = [30, 60]
P5_FIXED      = dict(l_min=1.0, l_max=7.0, step=0.5)
...
def generate_report(results):
    ...
    lines += [
        ...
        f'(safety={best["safety"]:.2f}, mu_window={best["mu_window"]}, sigma_window={best["sigma_window"]})',
        '',
        '## 判定',
        '',
    ]
    if beat_count > 0:
        lines.append(
            f'✅ **P5 は S2 ベースラインを {beat_count}/{len(results)} 構成で上回る。'
            '上位構成を STRATEGY_REGISTRY へ Shortlisted として追記すること。**'
        )
    else:
        lines.append(
            '❌ **P5 は全構成で S2 ベースラインを下回る。'
            'STRATEGY_REGISTRY へ Rejected として追記すること（理由: Kelly 近似レバでは改善なし）。**'
        )
    lines += [
        '',
        '## 再現コマンド',
...
def main():
    ...
    total_runs = len(SAFETY_SWEEP) * len(MU_WIN_SWEEP) * len(SIG_WIN_SWEEP)
    print('=' * 70)
    print(f'P5: Kelly Leverage Sweep — {total_runs} runs')
    ...
    sofr_ser = pd.Series(sofr_arr, index=close.index)  # P5 needs pd.Series for Kelly
    ...
    print(f'\nRunning P5 grid ({total_runs} runs)...')
    results = []
    run_idx = 0

    for safety, mu_w, sig_w in itertools.product(SAFETY_SWEEP, MU_WIN_SWEEP, SIG_WIN_SWEEP):
        run_idx += 1
        print(f'  [{run_idx:2d}/{total_runs}] safety={safety:.2f} mu_w={mu_w} sig_w={sig_w} ...',
              end=' ', flush=True)

        L_p5 = compute_L_kelly(
            ret, sofr_ser,
            safety=safety,
            mu_window=mu_w,
            sigma_window=sig_w,
    ...
    print()
    print('=' * 110)
    print(f'P5 Results ({total_runs} runs) — sorted by Sharpe_OOS desc')
    print('=' * 110)
    hdr = (f'{"safety":>6}  {"mu_w":>5}  {"sig_w":>5}  {"CAGR_IS":>9}  {"CAGR_OOS":>9}  '
    ...
    print(f'P5 beat S2: {beat_count}/{total_runs}')
    print(f'Best: safety={best["safety"]:.2f}, mu_w={best["mu_window"]}, sig_w={best["sigma_window"]} '
          f'→ CAGR_OOS {best["CAGR_OOS"]*100:+.2f}%, Sharpe_OOS {best["Sharpe_OOS"]:+.3f}')

    csv_path = os.path.join(BASE, 'p5_kelly_sweep_results.csv')
    pd.DataFrame(results).to_csv(csv_path, index=False, float_format='%.6f')
    print(f'\nSaved CSV: {csv_path}')

    md = generate_report(results)
    md_path = os.path.join(BASE, 'P5_KELLY_SWEEP_2026-05-22.md')
