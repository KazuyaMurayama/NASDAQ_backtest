...
仮説:
  F7 (TILT_CAP=0.10) の最良 tilt=0.50 → Sharpe=+0.905 で PASS閾値 0.9115 に
  あと 0.006 足りなかった。TILT_CAP が +10pp 上限で制約されていた可能性が
  高いため、cap を 0.15 / 0.20 に引き上げて確信度の高い bull 日のアップ
  サイドをさらに取りに行く。

メカニズム（F7 と同一、cap のみ可変）:
  bull mask:  raw_a2 > THRESHOLD (= 0.15)
  tilt_amount = tilt * (raw_a2 - THRESHOLD) * (1 - raw_a2)
  tilt_amount = clip(tilt_amount, 0, TILT_CAP)         # TILT_CAP は grid 値
  wn_tilted   = wn_A + tilt_amount    (bull 日のみ)
  wb_tilted   = max(wb_A - tilt_amount, 0)             # 債券から減らす（負禁止）

グリッド:
  tilt = [0.10, 0.20, 0.30, 0.50]
  cap  = [0.15, 0.20]
  → 4 × 2 = 8 configs + REF (tilt=0.0)
...
# グリッド・定数
# ---------------------------------------------------------------------------
TILT_GRID = [0.10, 0.20, 0.30, 0.50]   # tilt parameters
CAP_GRID  = [0.15, 0.20]                # TILT_CAP (cap=0.10 は F7 既出)

# E4 採用 config (k_lo=0.1, k_hi=0.8, vz_thr=0.7) — F7-v2 base
...
def main():
    ...
    print('=' * 70)
    print('F7-v2: Bull-Conviction wn Tilt スイープ（TILT_CAP拡張）')
    print('=' * 70)
    total = len(TILT_GRID) * len(CAP_GRID)
    print(f'グリッド: tilt={TILT_GRID} × cap={CAP_GRID}  ({total} configs + REF)')
    ...
    m_ref.update({'tilt': 0.0, 'cap': float('nan'),
    ...
    for cap in CAP_GRID:
        for tilt in TILT_GRID:
            ...
            tilt_amount = np.where(bull_mask, np.clip(tilt_amount, 0.0, cap), 0.0)
            ...
            m.update({'tilt': tilt, 'cap': cap,
                      'Trades_yr': trades_yr,
                      'WFA_CI95_lo': float('nan'), 'WFA_WFE': float('nan')})
            results.append(m)
            print(f'  [{idx}/{total}] tilt={tilt:.2f}/cap={cap:.2f}: '
    ...
    print(f'Best Sharpe: tilt={best_sharpe["tilt"]:.2f}/cap={best_sharpe["cap"]:.2f} → '
    ...
    } for r in results]).to_csv(os.path.join(BASE, 'f7v2_bull_tilt_results.csv'),
    ...
    tilt_rows = '\n'.join(
        fmt_row_1p(f'tilt={r["tilt"]:.2f}/cap={r["cap"]:.2f}', r)
        for r in results[1:]
    )
...
| **TILT_CAP グリッド** | {CAP_GRID}（F7 で cap=0.10 を既に評価済み） |
...
- **最高 Sharpe**: tilt={best_sharpe["tilt"]:.2f}/cap={best_sharpe["cap"]:.2f} → Sharpe={best_sharpe["Sharpe_OOS"]:+.3f}
- **総合判定: {verdict}**
...
"""
    md_path = os.path.join(BASE, 'F7V2_BULL_TILT_2026-05-24.md')
    with open(md_path, 'w', encoding='utf-8') as f:
        f.write(report)
    print(f'\nSaved: {os.path.join(BASE, "f7v2_bull_tilt_results.csv")}')
