"""
gen_f7v3_yearly_returns.py — F7v3+E4 (定式A tilt=2.0) の年次リターン生成
========================================================================
EVALUATION_STANDARD §3.12 準拠 (v1.1, 2026-05-24)

対象戦略:
  F7v3+E4 定式A bull-tilt (tilt=2.0, cap=0.10, THRESHOLD=0.15)
  ベース: E4 採用 config (k_lo=0.1, k_hi=0.8, vz_thr=0.7, LT2-N750, mode B)
  → 現行ベスト戦略 ◆ (G4 WFA PASS 2026-05-24)

処理:
  1. F7v3+E4 NAV を構築（f7v3_bull_tilt.py と同手順、採用 config のみ）
  2. 各年の年次リターン (%) を計算
  3. CSV `f7v3_yearly_returns.csv` を出力
  4. サニティチェック: OOS 期間 (2021-2026) の幾何平均が CAGR_OOS +36.30% と概ね一致するか確認

出力:
  - <BASE>/f7v3_yearly_returns.csv (columns: year, annual_return)
...
# サニティ参照値
REF_CAGR_OOS = 0.3630   # F7v3+E4 CAGR_OOS +36.30%
OOS_FIRST_YEAR = 2021
OOS_LAST_YEAR  = 2026


def build_f7v3_nav():
    """F7v3+E4 (定式A tilt=2.0) NAV を構築して返す。
...
def main():
    ...
    print('=' * 70)
    print('gen_f7v3_yearly_returns.py — F7v3+E4 (定式A tilt=2.0) 年次リターン生成')
    print(f'  E4 base: k_lo={K_LO}, k_hi={K_HI}, vz_thr={VZ_THR}, k_mid={K_MID}')
    print(f'  F7v3 定式A: tilt={TILT_A}, cap={TILT_CAP}, THRESHOLD={THRESHOLD}')
    print('=' * 70)

    print('Building F7v3+E4 NAV...')
    nav, dates = build_f7v3_nav()
    ...
    # CSV 出力
    out_csv = os.path.join(BASE, 'f7v3_yearly_returns.csv')
    pd.DataFrame({
        'year': years,
