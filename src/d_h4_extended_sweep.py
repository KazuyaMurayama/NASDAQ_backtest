"""Option D: H4拡張 (極端パラメータ) スイープ (2026-05-17)
============================================================
H4 (TQQQ 3x × TOCOM金 × TMF) を極端パラメータで拡張:
- wg_frac > 0.80
- L_g ∈ {3, 4, 5}
- wn_cap でDH信号を抑制 → Gold/Bondにより多く資金配分
...
グリッド:
- wg_frac ∈ {0.80, 0.85, 0.90, 0.95}    (4)
- L_g ∈ {3, 4, 5}                        (3)
- wn_cap ∈ {1.00, 0.85, 0.70}            (3)
- bond_drag ∈ {True, False}              (2)
- 合計: 72コンボ
"""
...
OUT_MD    = os.path.join(BASE, 'D_H4_EXTENDED_SWEEP_2026-05-17.md')
OUT_CSV   = os.path.join(BASE, 'D_H4_EXTENDED_SWEEP_2026-05-17.csv')

WG_FRAC_GRID = [0.80, 0.85, 0.90, 0.95]
LG_GRID      = [3.0, 4.0, 5.0]
WN_CAP_GRID  = [1.00, 0.85, 0.70]
...
def main():
    print('=' * 70)
    print('Option D: H4 拡張パラメータ スイープ (72コンボ)')
    print('=' * 70)
    ...
    gold_navs = {
        L: build_gold_tocom(gold_1x_arr, L, sofr, roll_cost_annual=0.02) for L in LG_GRID
    }
    bond_navs = {True: bond_3x_drag_arr, False: bond_3x_nodrag}

    print('Building DH Dyn [A] signal...')
    ...
    records = []
    total = len(WG_FRAC_GRID) * len(LG_GRID) * len(WN_CAP_GRID) * len(BOND_DRAG)
    done = 0
    ...
    for wg_frac in WG_FRAC_GRID:
        for L_g in LG_GRID:
            for wn_cap in WN_CAP_GRID:
                for bond_drag in BOND_DRAG:
                    done += 1
                    drag_label = 'drag' if bond_drag else 'nodr'
                    print(f'  [{done:2d}/{total}] wg={wg_frac:.2f} Lg={L_g:.0f} wn_cap={wn_cap:.2f} {drag_label}', end=' ')

                    wn = np.clip(wn_A_arr, 0.0, wn_cap)
                    rest = 1.0 - wn
                    wg = rest * wg_frac
                    wb = rest * (1.0 - wg_frac)
                    ...
                    nav = build_nav_strategy(
                        ...
                        gold_navs[L_g], bond_navs[bond_drag],
