...
K365_MARGIN_NONEARNING = True    # MARGIN COST PREMISE (v4 final, 2026-06-18). Canonical:
#   PRODUCT_COST_COMPARISON_2026-06-10.md s10 / EVALUATION_STANDARD.md s1.5.
# Margin is COLLATERAL backing the position; P&L accrues on the FULL notional to your equity (eToro:
#   "profit/loss applied to total exposure not the initial deposit"). => Posting margin does NOT create a
#   continuous CAGR drag. The ONLY recurring >3x cost is financing (kinri-soutou-gaku ~= SOFR+spread),
#   already in the cost model (borrow=SOFR+0.5% + K365_EXCESS_EXTRA 0.25%). JPY-denominated => no FX hedge.
# scale1.35-strong: after-tax min9 +23.83% IS ACHIEVABLE. M6 path-dependent (m=8% min margin) found forced
#   liquidation only 3x in 1975-77 (each 13-19% AUM) but CAGR impact +0.08pp, MaxDD/min9 unchanged; the 5
#   major crises (1987/2000/2008/COVID/2022) = 0 liquidations (strategy OUT). Real cost of high leverage =
#   TAIL RISK (forced liquidation in a future high-L crash), NOT a CAGR drag. Reduce risk via leverage level.
# REJECTED cost premises (do NOT use): (1) margin x SOFR (-0.9pp) -- margin is not idle cash.
#   (2) "set aside f% AUM as idle cash foregoing strategy return" (-1.68/-3.42pp) -- that is UNDER-INVESTMENT,
#       not required by margin mechanics; voluntary. de-lever (lower scale) cuts risk more efficiently.
#   (3) capital-constraint collapse (-7~-10pp, M5 v1 accounting bug). JPY-0% cash = currency mismatch (USD strategy).
