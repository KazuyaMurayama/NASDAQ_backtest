"""
Final 5-Strategy Comparison with Yearly Returns.

Realistic conditions: 5-day delay, 1.5% annual cost.

Strategies:
  1st: Ens2(Slope+TrendTV) Th=20%  [Best risk-adjusted]
...
def main():
    ...
    # =================================================================
    print("=" * 130)
    print("FINAL 5-STRATEGY COMPARISON")
    ...
    print(f"\n{'Rank':<6s} {'Strategy':<22s} {'Sharpe':>7s} {'CAGR':>8s} {'MaxDD':>8s} "
          f"{'Worst5Y':>8s} {'Sortino':>8s} {'Calmar':>8s} {'WinRate':>8s} {'Rebal/yr':>9s}")
    print("-" * 130)

    ranks = ['1st', '4th', '6th', 'Ref', 'Ref']
    for rank, m in zip(ranks, metrics_list):
        print(f"{rank:<6s} {m['Strategy']:<22s} {m['Sharpe']:>7.3f} {m['CAGR']*100:>+7.1f}% "
              f"{m['MaxDD']*100:>7.1f}% {m['Worst5Y']*100:>+7.1f}% "
              f"{m['Sortino']:>8.3f} {m['Calmar']:>8.3f} "
              f"{m['WinRate']*100:>7.1f}% {m['Rebal/Year']:>8.1f}")

    # Final NAV
    print(f"\n{'':6s} {'Strategy':<22s} {'Final NAV':>15s} {'$10,000 →':>15s}")
    print("-" * 65)
    for rank, m in zip(ranks, metrics_list):
        name = m['Strategy']
        final = navs[name].iloc[-1]
        print(f"{rank:<6s} {name:<22s} {final:>15,.1f} ${final*10000:>14,.0f}")
    ...
    row = f"{'Mean':<6s}"
    for n in names:
        row += f" {yearly_df[n].mean()*100:>+15.1f}%"
    print(row)

    # Median
    row = f"{'Med.':<6s}"
    for n in names:
        row += f" {yearly_df[n].median()*100:>+15.1f}%"
    print(row)

    # Std
    row = f"{'Std':<6s}"
    for n in names:
        row += f" {yearly_df[n].std()*100:>15.1f}%"
    print(row)

    # Best
    row = f"{'Best':<6s}"
    for n in names:
        row += f" {yearly_df[n].max()*100:>+15.1f}%"
    print(row)

    # Worst
    row = f"{'Worst':<6s}"
    for n in names:
        row += f" {yearly_df[n].min()*100:>+15.1f}%"
    print(row)
    ...
    output_yearly = os.path.join(script_dir, '..', 'final_5strategy_yearly_returns.csv')
