...
====================================
STRATEGY_PERFORMANCE_COMPARISON_2026-05-27.md 生成スクリプト
...
# ─── Group A: 4 既存ショートリスト候補 ──────────────────────────────────────

# E4: k_lo=0.1, k_hi=0.8, vz_thr=0.70 (b4 CSV に同一行あり)
...
# vz065+lmax5: D5 grid vz=0.65/lmax=5.0 + G10 WFA
...
# F10+lmax5: f10lmax5_fullmetrics.csv (Trades_yr は G8 WFA 値を使用)
...
# ─── Group B REF (= E4, 参照行) ─────────────────────────────────────────────
r_ref = mr(_e4_raw, ci95=_e4_ci, wfe=_e4_wfe)
...
# ─── A1: soft regime (alpha = hard-sigmoid 幅) ───────────────────────────────
...
# ─── A2: dynamic l_max (vol-sensitive) ──────────────────────────────────────
_a2 = _get_row('a2_dyn_lmax_results.csv', lmax_base=6.0, vol_sens=2.0)
r_a2 = mr(_a2)

# ─── A2B: rolling VOL_REF (rolling median) ──────────────────────────────────
...
# ─── C2: adaptive deadband ───────────────────────────────────────────────────
_c2 = _get_row('c2_adaptive_deadband_results.csv', eps_0=0.020, mode='adaptive')
r_c2 = mr(_c2)

# ─── C3: Yang-Zhang vol estimator ────────────────────────────────────────────
_c3 = _get_row('c3_yang_zhang_results.csv', yz_n=10, vz_thr=0.7)
r_c3 = mr(_c3)

# ─── D5: vz=0.65 row (G10 WFA 取得済み) ─────────────────────────────────────
_d5_55_raw = _get_row('d5_vz_lmax_grid_results.csv', vz_thr=0.65, l_max=5.5)
_d5_55_ci, _d5_55_wfe = _wfa('g10_wfa_vz065_lmax_row_summary.csv', 'vz065-lmax5p5')
r_d5_55 = mr(_d5_55_raw, ci95=_d5_55_ci, wfe=_d5_55_wfe)

_d5_60_raw = _get_row('d5_vz_lmax_grid_results.csv', vz_thr=0.65, l_max=6.0)
_d5_60_ci, _d5_60_wfe = _wfa('g10_wfa_vz065_lmax_row_summary.csv', 'vz065-lmax6')
r_d5_60 = mr(_d5_60_raw, ci95=_d5_60_ci, wfe=_d5_60_wfe)

_d5_70_raw = _get_row('d5_vz_lmax_grid_results.csv', vz_thr=0.65, l_max=7.0)
_d5_70_ci, _d5_70_wfe = _wfa('g10_wfa_vz065_lmax_row_summary.csv', 'vz065-lmax7')
r_d5_70 = mr(_d5_70_raw, ci95=_d5_70_ci, wfe=_d5_70_wfe)

_d5_v6_raw = _get_row('d5_vz_lmax_grid_results.csv', vz_thr=0.60, l_max=4.5)
r_d5_v6 = mr(_d5_v6_raw)  # WFA 未実施
...
W('# 戦略パフォーマンス比較表 v1.0 — 2026-05-27 実験全結果')
W('')
W('作成日: 2026-05-27')
W('最終更新日: 2026-05-27')
...
# ── §2 Group A 9指標比較表 ──────────────────────────────────────────────────
W('## 📊 §2 Group A — 既存4候補 9指標比較表（WFA 完了済み）')
W('')
...
W(fmt_row_strat('**F10 ε=0.015 ✅⚠**', r_f10))
W(fmt_row_strat('**vz065+lmax5 ✅⚠**', r_vz5))
W(fmt_row_strat('**F10+lmax5 ✅⚠**', r_f10l5))
...
W('### Group A 判定サマリー')
W('')
W('| Strategy | 過学習リスク | 判定 | 備考 |')
W('|----------|:----------:|:----:|------|')
W('| E4 ◆ | 低 | Active ✅ | G3 PASS (CI95_lo=+26.5%, WFE=+1.131)。gap=-1.81pp 最小群。Trades=27 低コスト。**現行ベスト確定**。 |')
W('| F10 ε=0.015 | 中 | Shortlisted ⚠ | G7 PASS (CI95_lo=+27.9%, WFE=+1.208)。gap=-4.31pp⚠ IS>OOS 乖離大。Trades=52 高コスト懸念。 |')
W('| vz065+lmax5 | 中 | Shortlisted ⚠ | G10 PASS (CI95_lo=+24.8%, WFE=+1.272)。Sharpe=+0.949 最高。MaxDD=-51.8% 最良。gap=-3.91pp⚠ |')
W('| F10+lmax5 | 中 | Shortlisted ⚠ | G8 PASS (CI95_lo=+25.6%, WFE=+1.278)。P10_5Y=+12.8% 最高。Trades=52 高コスト懸念。 |')
W('')
W('---')
W('')

# ── §3 Group B 9指標比較表 ──────────────────────────────────────────────────
W('## 🔬 §3 Group B — 2026-05-27 新規実験 9指標比較表')
W('')
W('> WFA 列は D5 vz=0.65 系のみ G10 値。その他 `—` は未実施。')
W('> **‡** = 参照行（E4 と同一、比較基準として掲載）')
W('')
W(HEADER_1)
W(HEADER_2)

W(fmt_row_strat('REF-E4 ‡', r_ref, sharpe_ref_mark=' ‡', maxdd_ref_mark=' ‡'))
W(fmt_row_strat('[B4] k_lo=0/k_hi=0.7/vz=0.7', r_b4))
W(fmt_row_strat('[A1] α=8 (Sharpe最大)', r_a1_8))
W(fmt_row_strat('[A1] α=2 (MaxDD改善)', r_a1_2))
W(fmt_row_strat('[A2] lmax_base=6/vol_sens=2', r_a2))
W(fmt_row_strat('[A2B] lmax_base=6/vol_sens=2 (rolling VOL_REF)', r_a2b))
W(fmt_row_strat('[A3] VoV+vz dual gate (vov_thr=1.3/α=0.2)', r_a3))
W(fmt_row_strat('[C2] adaptive deadband (ε₀=0.020)', r_c2))
W(fmt_row_strat('[C3] Yang-Zhang vol (yz_n=10/vz=0.7)', r_c3))
W(fmt_row_strat('[D5] vz=0.65/lmax=5.5 ✅', r_d5_55))
W(fmt_row_strat('[D5] vz=0.65/lmax=6.0 ✅', r_d5_60))
W(fmt_row_strat('[D5] vz=0.65/lmax=7.0 ✅⚠', r_d5_70))
W(fmt_row_strat('[D5] vz=0.60/lmax=4.5', r_d5_v6))
...
W('| **A1** α=8 | soft sigmoid で Sharpe 向上 | WARN | Sharpe=+0.904 向上だが MaxDD=-67.1% 悪化⚠。gap=-2.48pp 拡大。 |')
W('| **A1** α=2 | soft sigmoid で MaxDD 改善 | WARN | MaxDD=-61.7% 改善不十分。Sharpe=+0.886 < E4。効果微小。 |')
W('| **A2** base=6/sens=2 | vol 感応 l_max で MaxDD 改善 | WARN | MaxDD=-58.7% 改善。ただし CAGR=+32.6% 低下。E4 非優越。 |')
W('| **A2B** base=6/sens=2 | 10年ローリング VOL_REF で中立化 | FAIL | vol_ref_t=0.161（mean < 常時閾値）→ l_max 常時削減バイアス。CAGR=+31.7% 大幅低下。 |')
W('| **A3** vov=1.3/α=0.2 | VoV z-score で Volcker 期を回避 | FAIL | MaxDD=-60.01% 変化なし。1980-82 Volcker 期は vol 低下期→ VoV シグナル 0 日発火。構造的限界。 |')
W('| **C2** ε₀=0.020/adaptive | 適応 deadband で IS-OOS gap 縮小 | WARN | F10 と同性能（Sharpe=+0.935, CAGR=+36.84%）。gap=-4.28pp⚠ 改善なし。Trades=51 高コスト。 |')
...
for _, row in g10df.iterrows():
    ...
    if n == 'REF-E4':
        ...
        note = 'Group A vz065+lmax5 と同一。Sharpe 最高（WFA内）。'
    elif n == 'vz065-lmax5p5':
        note = 'CAGR=+34.6%, Sharpe=+0.945。CI95_lo 最高。'
    elif n == 'vz065-lmax6':
...
W('| Sharpe_OOS 最高 | vz065+lmax5 (Group A) | +0.949 |')
W('| MaxDD 最良 | D5 vz=0.60/lmax=4.5 | −49.2% |')
W('| Worst10Y★ 最高 | C3 yz_n=10 | +19.37% |')
W('| P10_5Y 最高 | F10+lmax5 (Group A) | +12.8% |')
W('| IS-OOS gap 最小 | D5 vz=0.60/lmax=4.5 | −0.15pp |')
W('| WFA CI95_lo 最高 | F10 ε=0.015 (Group A) | +27.9% |')
...
W('| **A1** soft regime | ❌ 棄却 | MaxDD 悪化 or Sharpe 改善微小。過学習リスク高。 |')
...
W('| **D5** vz=0.65 × lmax | ✅ 確認 | vz_thr=0.65 が Pareto 最適（全 l_max でベスト Sharpe）。vz065+lmax5 採用候補確定。 |')
W('')
W('### ◆ 現行ベスト維持 + 採用候補')
W('')
W('> **◆ 現行ベスト: E4 Regime k_lt (lo=0.1/hi=0.8/vz=0.7)** — Active 変更なし')
W('>')
W('> **Shortlisted (採用変更候補・ユーザー判断待ち)**:')
W('> 1. **vz065+lmax5**: Sharpe=+0.949（最高）, MaxDD=-51.8%（最良）, Trades=27（低コスト）')
W('>    G10 PASS (CI95_lo=+24.8%, WFE=+1.272)。gap=-3.91pp⚠ が唯一の懸念。')
W('> 2. **F10 ε=0.015**: CAGR=+36.8%（高リターン）。G7 PASS。ただし Trades=52 高コスト・gap=-4.31pp⚠。')
W('> 3. **F10+lmax5**: P10_5Y=+12.8%（最高）, MaxDD改善。G8 PASS。Trades=52 高コスト。')
W('>')
W('> **今回実験から追加検討値**:')
W('> - **D5 vz=0.65/lmax=5.5**: CAGR=+34.6%, Sharpe=+0.945。G10 PASS。次善候補。')
W('> - **C3 Yang-Zhang**: Worst10Y=+19.37%（最高）。WFA 未実施→要 WFA。')
W('')
W('---')
W('')
...
W('| ファイル | 実験 | 役割 |')
W('|----------|------|------|')
W('| `b4_klo_zero_results.csv` | B4 | k_lo=0 実験 9指標（E4 REF含む） |')
W('| `a1_soft_regime_klt_results.csv` | A1 | soft sigmoid 実験 9指標 |')
...
W('| `g8_wfa_lmax5_summary.csv` | G8 | F10+lmax5 / E4-lmax5 WFA（G8） |')
...
W('| 版 | 日付 | 変更内容 |')
W('|----|------|---------|')
W('| **v1.0** | 2026-05-27 | 初版。2026-05-27 全実験（B4/A1/A2/A2B/A3/C2/C3/D5）の §3.12 準拠 9指標比較表。Group A × 4 + Group B × 13 計 17 戦略。 |')
W('')
W('---')
W('')
