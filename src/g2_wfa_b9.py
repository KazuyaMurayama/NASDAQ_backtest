...
対象戦略:
  B9-Winner: S2+LT2-N750-k0.5-modeB + alloc(gold_frac=0.65, wn_min=0.20)
  B9-Stable: S2+LT2-N750-k0.5-modeB + alloc(gold_frac=0.60, wn_min=0.30)
  REF:       S2+LT2-N750-k0.5-modeB  + alloc(gold_frac=0.50, wn_min=0.30) ← サニティ
...
出力:
  - g2_wfa_b9_per_window.csv
  - g2_wfa_b9_summary.csv
  - G2_WFA_B9_2026-05-23.md
"""
...
# 定数
# ---------------------------------------------------------------------------
GOLD_FRAC_WINNER = 0.65
WN_MIN_WINNER    = 0.20
GOLD_FRAC_STABLE = 0.60
WN_MIN_STABLE    = 0.30
GOLD_FRAC_REF    = 0.50
WN_MIN_REF       = 0.30
...
STRATEGY_IDS = ['B9-Winner', 'B9-Stable', 'REF']

STRATEGY_LABELS = {
    'B9-Winner': 'B9-Winner (gf=0.65, wn_min=0.20) ✅⚠',
    'B9-Stable': 'B9-Stable (gf=0.60, wn_min=0.30) ✅',
    'REF':       'REF (gf=0.50, wn_min=0.30) ← サニティ ◆',
}
...
# ---------------------------------------------------------------------------
# F1専用ヘルパー: wn_min/gold_frac を引数化した rebalance 関数
# (f1_alloc_sweep.py の simulate_rebalance_A_wmin をコピー)
# ---------------------------------------------------------------------------
...
def build_all_navs(assets: dict) -> dict:
    ...
    raw_a2, vz, lev_mod = a['raw_a2'], a['vz'], a['lev_mod']

    navs = {}

    # B9-Winner: gold_frac=0.65, wn_min=0.20
    _, wn_w, wg_w, wb_w, _ = simulate_rebalance_A_wmin(
        raw_a2, vz, wn_min=WN_MIN_WINNER, gold_frac=GOLD_FRAC_WINNER)
    navs['B9-Winner'] = build_nav_strategy(
        close, lev_mod, wn_w, wg_w, wb_w, dates,
        gold_2x, bond_3x, sofr,
        nas_mode='CFD', cfd_leverage=a['L_s2'].values, cfd_spread=CFD_SPREAD_LOW,
    )

    # B9-Stable: gold_frac=0.60, wn_min=0.30
    _, wn_s, wg_s, wb_s, _ = simulate_rebalance_A_wmin(
        raw_a2, vz, wn_min=WN_MIN_STABLE, gold_frac=GOLD_FRAC_STABLE)
    navs['B9-Stable'] = build_nav_strategy(
        close, lev_mod, wn_s, wg_s, wb_s, dates,
        gold_2x, bond_3x, sofr,
        nas_mode='CFD', cfd_leverage=a['L_s2'].values, cfd_spread=CFD_SPREAD_LOW,
    )

    # REF (サニティ): gold_frac=0.50, wn_min=0.30 = 現行ベスト
    _, wn_r, wg_r, wb_r, _ = simulate_rebalance_A_wmin(
        raw_a2, vz, wn_min=WN_MIN_REF, gold_frac=GOLD_FRAC_REF)
    navs['REF'] = build_nav_strategy(
        close, lev_mod, wn_r, wg_r, wb_r, dates,
        gold_2x, bond_3x, sofr,
        nas_mode='CFD', cfd_leverage=a['L_s2'].values, cfd_spread=CFD_SPREAD_LOW,
    )
...
def compute_summary_stats(per_df: pd.DataFrame) -> dict:
    """短窓除外。IS窓: start_date < OOS_START、postIS窓: start_date >= OOS_START"""
...
def generate_md_report(results: dict, sanity: dict, windows: list,
    ...
    lines = []
    lines += [
        f'# G2: Walk-Forward Analysis — B9-Winner / B9-Stable × {len(windows)}窓',
        '',
        f'作成日: {TODAY}',
    ...
    # §5 サニティチェック
    ref_ci = results.get('REF', {}).get('summary', {}).get('WFA_CI95_lo', np.nan)
    ref_wfe = results.get('REF', {}).get('summary', {}).get('WFA_WFE', np.nan)
    ...
    lines += ['## 5. サニティチェック', '']
    lines += [
        f'- REF CI95_lo 参照値: +{REF_CI95_LO*100:.2f}% / '
        f'実測: {_fp(ref_ci)} / '
        f'差分: {ci_diff:+.2f} pp → {"OK (±5pp)" if ci_ok else "WARN"}',
        f'- REF WFA_WFE 参照値: +{REF_WFE:.3f} / '
        f'実測: {_ff(ref_wfe)} / '
    ...
    # §6 Shortlisted 昇格判定
    lines += ['## 6. Shortlisted 昇格判定', '']
    lines += [
        '| 戦略 | CI95_lo | WFE | α | β | 総合 | 昇格判断 |',
        '|------|--------:|----:|:--:|:--:|:---:|:--------|',
    ]
    for sid in ['B9-Winner', 'B9-Stable']:
        if sid not in results:
            continue
        s  = results[sid]['summary']
        cr = results[sid]['criteria']
        ...
        if v == 'PASS':
            decision = 'CURRENT_BEST_STRATEGY.md 更新を検討'
        elif v == 'WARN':
            decision = 'Shortlisted 継続 (β基準未達)'
        else:
            decision = 'Shortlisted 継続 (α基準未達)'
        lines.append(
            f'| {STRATEGY_LABELS.get(sid, sid)} '
    ...
    lines += [
        ...
        '```', '',
        '---', '',
        f'*生成スクリプト: `src/g2_wfa_b9.py`  '
        f'準拠: `EVALUATION_STANDARD.md v1.1`*',
        f'*参照: `CURRENT_BEST_STRATEGY.md`, `B9_COMPARISON_2026-05-23.md`*',
    ]

    md_text = '\n'.join(lines)
    md_path = os.path.join(base_dir, f'G2_WFA_B9_{TODAY}.md')
...
def main():
    ...
    print('=' * 70)
    print('G2: Walk-Forward Analysis — B9-Winner / B9-Stable × 非重複1年窓')
    print(f'実行日: {TODAY}')
    print('=' * 70)
    ...
    for sid in STRATEGY_IDS:
        ...
        wfe    = summary.get('WFA_WFE', np.nan)
        print(
            f'  {sid:<12}: mean_CAGR={mean_c*100:+.1f}%  '
    ...
    # S5: サニティチェック
    print('\n[S5] Sanity checks...')
    ref_ci  = results['REF']['summary'].get('WFA_CI95_lo', np.nan)
    ref_wfe = results['REF']['summary'].get('WFA_WFE', np.nan)
    ci_diff  = (ref_ci  - REF_CI95_LO) * 100 if not np.isnan(ref_ci)  else np.nan
    wfe_diff = (ref_wfe - REF_WFE)            if not np.isnan(ref_wfe) else np.nan
    ci_ok    = (not np.isnan(ci_diff))  and abs(ci_diff)  <= 5.0
    wfe_ok   = (not np.isnan(wfe_diff)) and abs(wfe_diff) <= 0.200
    ...
    # S7: CSV 保存
    print('\n[S7] Saving CSVs...')
    pw_path = os.path.join(BASE, 'g2_wfa_b9_per_window.csv')
    sm_path = os.path.join(BASE, 'g2_wfa_b9_summary.csv')
