...
目的: LT3 信号（N年CAGR 展開窓パーセンタイルランク、[-1, +1] 正規化）を
...
初回 Scenario D 評価 — LT3 は S2 との組み合わせ未検証。

出力:
  - b8_s2_lt3_sweep_results.csv  (プロジェクトルート)
  - B8_S2_LT3_SWEEP_2026-05-22.md (プロジェクトルート)
"""
...
def generate_report(results):
    ...
    lines = [
        f'# B8: S2_VZGated + {LT_SIGNAL} Sweep (N × k_lt)',
        '',
        '作成日: 2026-05-22',
        '最終更新日: 2026-05-22',
        '',
        f'**初回 Scenario D 正式評価**。{LT_SIGNAL}-modeB（N年CAGR 展開窓パーセンタイルランク）を S2_VZGated lev に適用。',
        '',
    ...
    lines += [
        ...
        '',
        '```',
        f'python -X utf8 src/b8_s2_lt3_sweep.py',
        '```',
        '',
...
def main():
    ...
    print('=' * 70)
    print(f'B8: S2_VZGated + {LT_SIGNAL} Sweep — {total_runs} runs')
    ...
    for N, k_lt in itertools.product(N_SWEEP, KLT_SWEEP):
        ...
        print(f'    LT3 signal range: [{lt_sig.dropna().min():.2f}, {lt_sig.dropna().max():.2f}]  ', end='')
    ...
    print()
    print('=' * 110)
    print(f'B8 {LT_SIGNAL} Results ({total_runs} runs) — sorted by Sharpe_OOS desc')
    print('=' * 110)
    ...
    csv_path = os.path.join(BASE, 'b8_s2_lt3_sweep_results.csv')
    pd.DataFrame(results).to_csv(csv_path, index=False, float_format='%.6f')
    print(f'\nSaved CSV: {csv_path}')

    md = generate_report(results)
    md_path = os.path.join(BASE, 'B8_S2_LT3_SWEEP_2026-05-22.md')
