"""
check_overfitting_vz065lmax5_bootstrap.py
vz065+lmax5戦略 Block Bootstrap 過学習検定
==========================================
OOS期間（2021-05-08〜最終日）の日次リターンに対して Stationary Block Bootstrap を適用し、
vz065+lmax5戦略（S2_VZGated(l_max=5.0) + LT2-N750 + Regime k_lt vz_thr=0.65）の
...
VZ065LMAX5_PARAMS = _mod.VZ065LMAX5_PARAMS
...
def main():
    ...
    print('=' * 60)
    print('vz065+lmax5戦略 Block Bootstrap 過学習検定')
    print('=' * 60)

    # Step 1: データ取得
    print('\n[Step 1] Loading vz065+lmax5 strategy assets ...')
    assets = build_vz065lmax5_strategy_assets()
    nav    = assets['nav_vz065lmax5']
    dates  = assets['dates']
    ...
    # Step 5: IS 期間 安定性検証
    print('\n[Step 5] IS stability (10-fold, nav_vz065lmax5) ...')
    ...
    # Output: Markdown
    # =========================================================================
    md_path = os.path.join(AUDIT_DIR, f'VZ065LMAX5_BOOTSTRAP_{TODAY_COMPACT}.md')
    ...
    vz_params_str = (
        f"vz_thr={VZ065LMAX5_PARAMS['vz_thr']}, l_max={VZ065LMAX5_PARAMS['l_max']}, "
        f"k_lo={VZ065LMAX5_PARAMS['k_lo']}, k_hi={VZ065LMAX5_PARAMS['k_hi']}, "
        f"k_mid={VZ065LMAX5_PARAMS['k_mid']}"
    )

    md_lines = [
        '# vz065+lmax5戦略 Block Bootstrap 過学習検定レポート',
        '',
        f'**戦略名:** S2_VZGated(l_max=5.0) + LT2-N750 + Regime k_lt ({vz_params_str})',
        ...
        f'- vz065lmax5戦略 vs BH 超過 Sharpe: {sr_observed - sr_bh1x:+.4f}',
        '',
        '## vz065+lmax5 パラメータ',
        '',
        f'| パラメータ | 値 |',
        '|:---|---:|',
        f'| vz_thr | {VZ065LMAX5_PARAMS["vz_thr"]} |',
        f'| l_max | {VZ065LMAX5_PARAMS["l_max"]} |',
        f'| k_lo (低ボラ) | {VZ065LMAX5_PARAMS["k_lo"]} |',
        f'| k_hi (高ボラ) | {VZ065LMAX5_PARAMS["k_hi"]} |',
        f'| k_mid (中ボラ) | {VZ065LMAX5_PARAMS["k_mid"]} |',
        '',
    ...
    # Output: YAML
    # =========================================================================
    yaml_path = os.path.join(AUDIT_DIR, 'vz065lmax5_bootstrap_results.yaml')
    ...
    print(f'\nIS stability (nav_vz065lmax5): mean={is_mean:.3f} std={is_std:.3f} CoV={cov_str}')
