"""
check_f10_parameter_sensitivity.py
F10戦略 (ε=0.015 deadband) パラメータ感度分析（ワンファクター法）
=====================================================================
中心値 (eps=0.015, k_lo=0.1, k_hi=0.8, vz_thr=0.7, N_lt2=750, target_vol=0.8)
から各パラメータを5点振ったときの Sharpe_OOS / CAGR_OOS 変動幅を計測する。
「ベストパラメータが孤立した峰か、広い台地か」を確認。

出力:
  audit_results/F10_PARAM_SENSITIVITY_20260526.md
  audit_results/f10_param_sensitivity.yaml
  audit_results/f10_param_sensitivity.csv
"""
...
build_f10_assets_with_override = _mod.build_f10_assets_with_override
...
# パラメータグリッド定義
CENTER_PARAMS = dict(
    eps=0.015, k_lo=0.1, k_hi=0.8, vz_thr=0.7, N_lt2=750, target_vol=0.8,
)
...
SENSITIVITY_GRIDS = {
    ...
    'vz_thr':     [0.60, 0.65, 0.70, 0.75, 0.80],
...
def build_markdown(all_results: dict, center_metrics: dict,
    ...
    lines = []
    lines.append('# F10 Parameter Sensitivity Analysis')
    lines.append('')
    ...
    lines.append('ROBUST基準: CAGR_OOS変動幅 ≤ 2pp ∧ Sharpe変動幅 ≤ 0.05')
    lines.append('FRAGILE基準: 中心値が局所最大 ∧ 片側で 5pp 以上低下')
...
def build_yaml_data(all_results: dict, center_metrics: dict,
    ...
    data = {
        'generated':    date_str,
        'strategy':     'F10 = F8-R5 + epsilon=0.015 deadband + E4 lev_mod_e4 + S2 L_s2',
...
def main():
    ...
    base_assets = build_f10_strategy_assets(force_rebuild=False)
    ...
    print(f'  f10_params: {base_assets["f10_params"]}')
    print()

    print('[STEP 2] 中心値メトリクス計算 ...')
    center_nav = base_assets['nav_f10']
    ...
    for param_name, grid in SENSITIVITY_GRIDS.items():
        ...
        for value in grid:
            ...
            if is_center:
                ...
                try:
                    assets_var = build_f10_assets_with_override(
                        param_name=param_name,
                        value=value,
                        base_assets=base_assets,
                    )
                    nav_var = assets_var['nav_f10']
    ...
    # Output
    md_filename = f'F10_PARAM_SENSITIVITY_{date_str.replace("-", "")}.md'
    ...
    yaml_path = os.path.join(audit_dir, 'f10_param_sensitivity.yaml')
    ...
    csv_path = os.path.join(audit_dir, 'f10_param_sensitivity.csv')
