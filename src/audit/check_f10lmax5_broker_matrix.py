"""
check_f10lmax5_broker_matrix.py
F10+lmax5 (ε=0.015 deadband + l_max=5.0) ブローカー別コストシナリオ × フルバックテスト比較
==================================================================================
S2_VZGated (l_max=5.0) + LT2-N750 + E4 Regime k_lt (k_lo=0.1, k_hi=0.8, k_mid=0.5, vz_thr=0.7)
+ ε-deadband tilt (eps=0.015) を6ブローカーコストシナリオで比較。

出力:
  audit_results/F10LMAX5_BROKER_MATRIX_<DATE>.md
  audit_results/f10lmax5_broker_matrix.yaml
"""
...
build_f10lmax5_strategy_nav_for_scenario = _mod.build_f10lmax5_strategy_nav_for_scenario
...
# ---------------------------------------------------------------------------
# Broker scenarios (SAME 6 as E4 broker matrix)
# ---------------------------------------------------------------------------
BROKER_SCENARIOS = [
...
def _load_e4_cagr_ref() -> dict:
    """E4戦略 (l_max=7.0) の CAGR_OOS / Sharpe_OOS を e4_broker_matrix.yaml から読み込む（比較用）。"""
    yaml_path = os.path.join(BASE, 'audit_results', 'e4_broker_matrix.yaml')
    if not os.path.exists(yaml_path):
        print(f'[WARN] e4_broker_matrix.yaml not found, comparison disabled.')
        return {}
...
def build_markdown(results: list, date_str: str, e4_ref: dict) -> str:
    lines = []
    lines.append('# F10+lmax5 ブローカー別コストシナリオ × フルバックテスト')
    lines.append('')
    lines.append(f'生成日: {date_str}')
    lines.append('')
    lines.append('**戦略:** F10 (ε=0.015 deadband tilt) + l_max=5.0 (S2_VZGated) + LT2-N750 + E4 Regime k_lt (k_lo=0.1, k_hi=0.8, k_mid=0.5, vz_thr=0.7)')
    lines.append('')
    ...
    # F10+lmax5 vs E4 比較セクション
    if e4_ref:
        lines.append('## F10+lmax5 vs E4 (l_max=7.0) 比較')
        lines.append('')
        lines.append('| シナリオ | E4 CAGR_OOS | F10+lmax5 CAGR_OOS | 差 | E4 Sharpe | F10+lmax5 Sharpe | F10+lmax5判定 |')
        lines.append('|:---------|------------:|-------------------:|---:|----------:|-----------------:|:------:|')
        for r in results:
            m = r['metrics']
            new_cagr   = m['CAGR_OOS']
            ...
            if r['name'] in e4_ref:
                ...
                lines.append(
                    f'| {r["name"]} | — | {new_cagr*100:+.1f}% | — | — | {new_sharpe:+.3f} | {new_v.split(" (")[0]} |'
                )
        lines.append('')
        lines.append('*差 = F10+lmax5 CAGR_OOS − E4 CAGR_OOS (pp)。E4 = S2_VZGated(l_max=7.0) + LT2-N750 + E4 Regime k_lt.*')
        lines.append('')
...
def build_yaml_data(results: list, date_str: str) -> dict:
    data = {
        'generated': date_str,
        'strategy': 'F10 (eps=0.015 deadband tilt) + l_max=5.0 (S2_VZGated) + LT2-N750 + E4 Regime k_lt (k_lo=0.1, k_hi=0.8, k_mid=0.5, vz_thr=0.7)',
        'thresholds': {
...
def main():
    ...
    print('[F10+lmax5] ブローカー別コストシナリオ × フルバックテスト比較')
    print(f'           BASE={BASE}')
    ...
    # Build / load F10+lmax5 assets (cached)
    ...
    print(f'  f10lmax5_params : {assets["f10lmax5_params"]}')
    print()
    ...
    for sc in BROKER_SCENARIOS:
        print(f'  → {sc["name"]} ...')
        nav = build_f10lmax5_strategy_nav_for_scenario(
            assets,
    ...
    # Write Markdown
    md_filename = f'F10LMAX5_BROKER_MATRIX_{date_compact}.md'
    ...
    # Write YAML
    yaml_path = os.path.join(audit_dir, 'f10lmax5_broker_matrix.yaml')
    ...
    print()
    print('=' * 70)
    print('F10+lmax5 ブローカー別コストシナリオ 総合結果サマリー')
    print('=' * 70)
    ...
    if e4_ref:
        print()
        print('--- F10+lmax5 vs E4 CAGR_OOS 差分サマリー ---')
        for r in results:
            m = r['metrics']
            if r['name'] in e4_ref:
                e4_cagr, _ = e4_ref[r['name']]
                diff_pp = (m['CAGR_OOS'] - e4_cagr) * 100
                print(f'  {r["name"]:<42} {diff_pp:+.2f}pp  (E4={e4_cagr*100:+.2f}%  F10+lmax5={m["CAGR_OOS"]*100:+.2f}%)')
