"""
check_overfitting_f10_bootstrap.py
F10戦略 (ε=0.015 deadband) Block Bootstrap 過学習検定
========================================================
OOS期間（2021-05-08〜最終日）の日次リターン (nav_f10) に対して
...
# E4 (旧 Active) の参照値（比較用）
REF_E4_CI95_LO_L63 = 0.264  # E4 Block Bootstrap CI95_lo (L=63) - 概算
REF_E4_P_L63       = 0.0    # E4 Block Bootstrap p-value (L=63)
...
def main():
    ...
    print('=' * 60)
    print('F10戦略 (ε=0.015 deadband) Block Bootstrap 過学習検定')
    print('=' * 60)

    # Step 1: データ取得
    print('\n[Step 1] Loading F10 strategy assets ...')
    assets = build_f10_strategy_assets()
    nav    = assets['nav_f10']
    dates  = assets['dates']
    ...
    print(f'  Sharpe_OOS (observed F10) = {sr_observed:.4f}')
    ...
    # Step 5: IS 期間 安定性検証
    print('\n[Step 5] IS stability (10-fold, nav_f10) ...')
    ...
    # Output: Markdown
    # =========================================================================
    md_path = os.path.join(AUDIT_DIR, f'F10_BOOTSTRAP_{TODAY_COMPACT}.md')
    ...
    f10_params_str = (
        f"eps={F10_PARAMS['eps']}, tilt={F10_PARAMS['tilt']}, "
        f"vz_reg={F10_PARAMS['vz_reg']}, cap_calm={F10_PARAMS['cap_calm']}, "
        f"cap_bull={F10_PARAMS['cap_bull']}, cap_bear={F10_PARAMS['cap_bear']}"
    )
    e4_params_str = (
    ...
    md_lines = [
        ...
        f'  + E4 lev_mod_e4 ({e4_params_str})',
        f'  + S2 L_s2 (l_max=7.0)',
        f'**F10 パラメータ:** {f10_params_str}',
        ...
        f'- F10戦略 vs BH 超過 Sharpe: {sr_observed - sr_bh1x:+.4f}',
        '',
        '## F10 パラメータ',
        '',
        f'| パラメータ | 値 |',
        '|:---|---:|',
        f'| eps (deadband) | {F10_PARAMS["eps"]} |',
        f'| tilt (CALM_BOOST) | {F10_PARAMS["tilt"]} |',
        f'| vz_reg | {F10_PARAMS["vz_reg"]} |',
        f'| cap_calm | {F10_PARAMS["cap_calm"]} |',
        f'| cap_bull | {F10_PARAMS["cap_bull"]} |',
        f'| cap_bear | {F10_PARAMS["cap_bear"]} |',
        '',
    ...
    # Output: YAML
    # =========================================================================
    yaml_path = os.path.join(AUDIT_DIR, 'f10_bootstrap_results.yaml')
    ...
    # =========================================================================
    print('\n' + '=' * 60)
    print(f'Sharpe_OOS (F10 observed) : {sr_observed:.4f}')
    print(f'BH 1x Sharpe_OOS          : {sr_bh1x:.4f}')
    ...
    print(f'\nIS stability (nav_f10): mean={is_mean:.3f} std={is_std:.3f} CoV={cov_str}')
