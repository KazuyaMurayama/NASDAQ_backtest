"""
check_overfitting_vz065lmax5_permutation.py
vz065+lmax5戦略 Permutation Test（シグナル置換検定）
=====================================================
S2_VZGated(l_max=5.0) + LT2-N750 + Regime k_lt (vz_thr=0.65, k_lo=0.1, k_hi=0.8, k_mid=0.5)
の予測力が本物か（NASDAQ右肩上がりに乗っているだけか）を検定する。

検定:
  (a) L_s2_lmax5 ブロックシャッフル           — CFD動的レバレッジのアルファ
  (c) lev_mod_065 ブロックシャッフル           — vz065市場参加タイミングのアルファ
  (d) L_s2_lmax5 + lev_mod_065 同時シャッフル  — 真のアルファ測定（KEY TEST）
...
def main():
    ...
    print('=' * 60)
    print('vz065+lmax5戦略 Permutation Test（シグナル置換検定）')
    print('=' * 60)

    # ------------------------------------------------------------------
    # Step 1: vz065+lmax5アセット取得と OOS 切り出し
    # ------------------------------------------------------------------
    print('\n[Step 1] Loading vz065+lmax5 strategy assets ...')
    assets = build_vz065lmax5_strategy_assets(force_rebuild=False)

    dates         = assets['dates']
    close         = assets['close']
    L_s2_lmax5    = assets['L_s2_lmax5']      # pd.Series (l_max=5.0)
    lev_mod_065   = assets['lev_mod_065']     # np.ndarray (vz_thr=0.65)
    wn_A          = assets['wn_A']
    wg_A          = assets['wg_A']
    wb_A          = assets['wb_A']
    ...
    lev_mod_arr    = np.asarray(lev_mod_065, dtype=float)
    ...
    print(f'  lev_mod_065 : [{lev_mod_arr.min():.2f}, {lev_mod_arr.max():.2f}]  median={np.median(lev_mod_arr):.2f}')
    ...
    sr_obs = compute_oos_sharpe(
        L_arr, lev_mod_arr, wn_arr, wg_arr, wb_arr,
        r_nas, r_g2, r_b3, idx, oos_mask_arr
    )
    ...
    print(f'  Sharpe_OOS (observed vz065lmax5) = {sr_obs:.4f}')
    print(f'  BH 1x Sharpe_OOS                 = {sr_bh:.4f}')

    # ------------------------------------------------------------------
    # Step 3: 検定(a) L_s2_lmax5 ブロックシャッフル
    # ------------------------------------------------------------------
    print('\n[Step 3] (a) L_s2_lmax5 block shuffle permutation (lev_mod_065 fixed) ...')
    results_a = []
    t_a = time.time()
    timing_a = []
    ...
    for b in range(B_PERM):
        ...
        sr_perm = compute_oos_sharpe(
            L_perm, lev_mod_arr, wn_arr, wg_arr, wb_arr,
            r_nas, r_g2, r_b3, idx, oos_mask_arr
        )
    ...
    # ------------------------------------------------------------------
    # Step 4: 検定(c) lev_mod_065 ブロックシャッフル
    # ------------------------------------------------------------------
    print('\n[Step 4] (c) lev_mod_065 block shuffle permutation (L_s2_lmax5 fixed) ...')
    results_c = []
    t_c = time.time()
    timing_c = []
    ...
    for b in range(B_PERM):
        t0 = time.time()
        lev_perm = block_shuffle(lev_mod_arr, BLOCK_LEN, rng_c)
        sr_perm = compute_oos_sharpe(
        ...
        if b % 100 == 0:
            print(f'  (c) lev_mod_065 block perm: {b}/{B_PERM}  ({time.time()-t_c:.1f}s elapsed)')
    ...
    # ------------------------------------------------------------------
    # Step 5: 検定(d) L_s2_lmax5 + lev_mod_065 同時シャッフル（KEY TEST）
    # ------------------------------------------------------------------
    print('\n[Step 5] (d) L_s2_lmax5 + lev_mod_065 SYNCHRONIZED block shuffle (KEY TEST) ...')
    results_d = []
    t_d = time.time()
    timing_d = []
    ...
    for b in range(B_PERM):
        ...
        L_perm, lev_perm = block_shuffle_synced(
            L_arr, lev_mod_arr, BLOCK_LEN, rng_d
        )
        sr_perm = compute_oos_sharpe(
    ...
    print(f'  verdict_c (lev_mod_065 block)   = {verdict_c}  (p={p_c:.4f})')
    ...
    # Output: YAML
    # ------------------------------------------------------------------
    yaml_path = os.path.join(AUDIT_DIR, 'vz065lmax5_permutation_results.yaml')
    ...
    # Output: Markdown
    # ------------------------------------------------------------------
    md_path = os.path.join(AUDIT_DIR, f'VZ065LMAX5_PERMUTATION_{TODAY_TAG}.md')
    ...
    md_lines = [
        ...
        '',
        f'**戦略名:** S2_VZGated(l_max=5.0) + LT2-N750 + Regime k_lt (vz_thr=0.65, k_lo=0.1, k_hi=0.8, k_mid=0.5)',
        f'**OOS 期間:** {OOS_START} 〜 {oos_end_str}（約4.9年）',
        f'**作成日:** {TODAY_STR}',
        f'**Permutation反復:** B={B_PERM}, block_len={BLOCK_LEN}',
        '',
        f'主眼: テスト(d) L_s2_lmax5+lev_mod_065 同時置換が「真のアルファ測定」。',
        f'{ref_note}',
        '',
        '## 観測値',
        '',
        '| 指標 | 値 |',
        '|---|---|',
        f'| Sharpe_OOS vz065lmax5 (観測) | {sr_obs:.4f} |',
        ...
        '| 検定 | 対象 | p値 | 判定 |',
        '|---|---|---|---|',
        f'| (a) L_s2_lmax5 ブロックシャッフル | CFD動的レバレッジ（lev_mod_065固定） | {p_a:.4f} | {verdict_a} |',
        f'| (c) lev_mod_065 ブロックシャッフル | vz065市場参加タイミング（L_s2_lmax5固定） | {p_c:.4f} | {verdict_c} |',
        f'| **(d) L_s2_lmax5+lev_mod_065 同時シャッフル** | **真のアルファ測定（KEY TEST）** | **{p_d:.4f}** | **{verdict_d}** |',
        '',
        ...
        _fmt_ds_row('(c) lev_mod_065 block', p_c, verdict_c, ds_c),
        ...
        f'## 総合判定: {overall}',
        '',
        f'*(d) L_s2_lmax5+lev_mod_065 同時置換 p={p_d:.4f} が主判定。p < 0.05 → 真のアルファあり PASS*',
        '',
        '### 個別判定根拠',
        f'- **(a)** L_s2_lmax5のみ置換: p={p_a:.4f} → {verdict_a}（CFD動的レバレッジの寄与）',
        f'- **(c)** lev_mod_065のみ置換: p={p_c:.4f} → {verdict_c}（vz065市場参加タイミングの寄与）',
    ...
    # ------------------------------------------------------------------
    print('\n' + '=' * 60)
    print('vz065+lmax5戦略 Permutation Test 結果サマリ')
    print('=' * 60)
    print(f'Sharpe_OOS (vz065lmax5 observed) : {sr_obs:.4f}')
    print(f'BH 1x Sharpe_OOS                 : {sr_bh:.4f}')
    print()
    print(f'{"検定":<35} {"p値":>8} {"判定":>8}')
    print(f'{"(a) L_s2_lmax5 block":<35} {p_a:>8.4f} {verdict_a:>8}')
    print(f'{"(c) lev_mod_065 block":<35} {p_c:>8.4f} {verdict_c:>8}')
