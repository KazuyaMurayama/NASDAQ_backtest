"""
check_vz065lmax5_parameter_sensitivity.py
vz065+lmax5戦略 パラメータ感度分析（ワンファクター法）
=========================================================
中心値 (vz_thr=0.65, l_max=5.0, k_lo=0.10, k_hi=0.80, N_lt2=750, target_vol=0.80)
から各パラメータを振ったときの Sharpe_OOS / CAGR_OOS 変動幅を計測する。

出力:
  audit_results/VZ065LMAX5_PARAM_SENSITIVITY_20260526.md
  audit_results/vz065lmax5_param_sensitivity.yaml
  audit_results/vz065lmax5_param_sensitivity.csv
"""
...
build_vz065lmax5_assets_with_override = _mod.build_vz065lmax5_assets_with_override
...
# ── パラメータグリッド定義 ───────────────────────────────────────────
CENTER_PARAMS = dict(
    vz_thr=0.65, l_max=5.0, k_lo=0.10, k_hi=0.80, N_lt2=750, target_vol=0.80,
)

SENSITIVITY_GRIDS = {
    'vz_thr':     [0.55, 0.60, 0.65, 0.70, 0.75],
...
def build_markdown(all_results: dict, center_metrics: dict,
    ...
    lines = []
    lines.append('# vz065+lmax5 Parameter Sensitivity Analysis')
    lines.append('')
...
def build_yaml_data(all_results: dict, center_metrics: dict,
    ...
    data = {
        'generated':    date_str,
        'strategy':     'S2_VZGated(l_max=5.0) + LT2-N750 + Regime k_lt (vz_thr=0.65)',
...
def main():
    ...
    print('=' * 70)
    print('vz065+lmax5 パラメータ感度分析（ワンファクター法）')
    print('=' * 70)
    print(f'  BASE     = {BASE}')
    ...
    # ── STEP 1: vz065+lmax5 アセット構築 / キャッシュ読み込み ───────────
    print('[STEP 1] vz065+lmax5戦略アセット構築 / キャッシュ読み込み ...')
    base_assets = build_vz065lmax5_strategy_assets(force_rebuild=False)
    ...
    print(f'  vz065lmax5_params: {base_assets["vz065lmax5_params"]}')
    print()

    # ── STEP 2: 中心値メトリクス計算 ──────────────────────────────────────
    print('[STEP 2] 中心値メトリクス計算 ...')
    center_nav = base_assets['nav_vz065lmax5']
    ...
    for param_name, grid in SENSITIVITY_GRIDS.items():
        ...
        for value in grid:
            ...
            if is_center:
                ...
                try:
                    assets_var = build_vz065lmax5_assets_with_override(
                        param_name=param_name,
                        value=value,
                        base_assets=base_assets,
                    )
                    nav_var = assets_var['nav_vz065lmax5']
    ...
    # ── STEP 5: ファイル出力 ─────────────────────────────────────────
    md_filename = f'VZ065LMAX5_PARAM_SENSITIVITY_{date_str.replace("-", "")}.md'
    ...
    yaml_path = os.path.join(audit_dir, 'vz065lmax5_param_sensitivity.yaml')
    ...
    csv_path = os.path.join(audit_dir, 'vz065lmax5_param_sensitivity.csv')
