...
def build_md():
    ...
    # ---- 先出し blockquote (KEY finding, QC是正版 2026-06-21) ----
    # NOTE: 旧版は無効な block=21 timing_P_maxdd を見出しにし B1 を「唯一の見どころ」と
    # 称揚していたが、独立QCで撤回。先出しは §7/§8/§9 の是正版結論を反映する固定文。
    # CAGR優位(b1_vs_a7_cagr)は §9 で「弱いブレーキ」と判明したため見出しから外す。
    w("> **🔑 結論（先出し・QC是正版 2026-06-21）: DD削減ブレーキ(B1-B4)に時機スキルの証拠は無い。**")
    w("> ① §5 の「DELEVER_ONLY」判定は block=21 で全系列MaxDDをブートストラップしたもので、"
      "経路依存のMaxDDに**無効**（暴落の並びを破壊しPを0.5付近へ固定）と独立QCで判明＝撤回。")
    w("> ② 経路頑健な再検定(§8, 無傷の暴落窓)でも**全ブレーキ TIMING_WEAK**"
      "（A7・B1とも暴落窓で双子より浅い回数0/5）。真因＝**危機はDH-W1のOUT-fillが既に処理済"
      "(危機中94-100% OUT)で、IN脚ブレーキに作用余地がない**。ただし暴落窓 n=5・実質1窓"
      "(tri_2015)のみ作用＝「時機なしの証明」でなく「時機効果の証拠が無い」（小サンプルの限界明記）。")
    w("> ③ B1のA7比CAGR優位は§9の等退避リテストで**「賢い信号」でなく「弱いブレーキ」**と判明"
      "（f̄をA7に揃えるとB1のMaxDDはA7より深い／CAGR優位は1999単年依存・2020では逆に負け）。")
    w("> ④ **正味の結論: DD削減は MaxDD↔CAGR のトレードオフ・ダイヤル**。CAGRをほぼ線形に払えば"
      "MaxDDは下がる（一律デレバ/B4で十分）。**CAGR完全維持で MaxDD削減はどの構成でも達成不能。** "
      "現行A0(MaxDD最小化でなくCAGR最高)維持か、許容CAGR低下に応じB4でデレバ水準を選ぶ。詳細§7/§8/§9。")
    w("")
    ...
    w(f"> **A7は1999年(上昇相場)に高ボラを誤ブレーキし A0比 {_signed(a7_99_vs_a0,2)}pp"
      ...
      f"**（ただし§9: この1999優位は74%が1999単年依存で、等退避f̄ではB1のMaxDDはA7より深い"
      f"＝『賢い信号』でなく『弱いブレーキ』。下方偏差が賢い証拠ではない。）**")
    w("")
