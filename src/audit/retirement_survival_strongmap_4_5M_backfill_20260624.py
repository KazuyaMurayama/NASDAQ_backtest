"""
src/audit/retirement_survival_strongmap_4_5M_backfill_20260624.py
================================================================
Retirement-survival simulation on the P09_STR (STRONG-map) leverage dial,
ANNUAL SPEND = 4,500,000 JPY, WITH A BACKFILL RULE (offset = 4,500,000 JPY):

  BACKFILL (same a/b/c rule as the 6M version, amounts scaled to 4.5M):
    (a) If the asset balance is below 20,000,000 JPY, that year's 4,500,000 JPY
        spending is set to ZERO (the shortfall is earned externally).
    (b) Interpretation: in bad markets the retiree earns 4.5M JPY outside and
  ...
  Decision rule (faithful to a/b/c, identical to 6M version):
    ...
    - Else -> withdraw 4,500,000 as usual.
    - Then apply that year's market return to the remaining balance.

  ALL OTHER CONDITIONS ARE IDENTICAL to the 6M backfill version:
    init 30M, 31 starts (1975-2005), 20yr horizon, strong-map after-tax
    returns, backfill threshold 20M, half-asset threshold 15M (initial half).
  Only the SPEND and the implied BACKFILL OFFSET change: 6.0M -> 4.5M.
  Initial withdrawal rate = 4.5M / 30M = 15.0%.

ASCII-only prints. Does NOT commit, no temp files.
Outputs:
  audit_results/retirement_survival_strongmap_4_5M_backfill_grid_20260624.csv
  audit_results/retirement_survival_strongmap_4_5M_backfill_paths_20260624.csv
"""
...
ANNUAL_SPEND = 4_500_000.0          # 450万/年 (this version) -- offset also 450万
...
def main():
    print("=" * 110)
    print("RETIREMENT SURVIVAL (STRONG-MAP, SPEND 4.5M, BACKFILL<20M)  2026-06-24")
    print("init=%.0f  spend=%.0f/yr (rate %.1f%%)  backfill if balance<%.0f -> spend 0 (external income 4.5M)"
          % (INIT_ASSET, ANNUAL_SPEND, 100 * ANNUAL_SPEND / INIT_ASSET, BACKFILL_THRESHOLD))
    ...
    pd.DataFrame(grid_rows).to_csv(
        os.path.join(out_dir, "retirement_survival_strongmap_4_5M_backfill_grid_20260624.csv"),
    ...
    pd.DataFrame(path_rows).to_csv(
        os.path.join(out_dir, "retirement_survival_strongmap_4_5M_backfill_paths_20260624.csv"),
        index=False, encoding="utf-8-sig")
    print("Saved paths CSV.")

    # ---- summary ----
    print("\n" + "=" * 110)
    print("SUMMARY (spend 4.5M + backfill<20M, of %d starts)" % len(START_YEARS))
    ...
    block = {
        "script": "retirement_survival_strongmap_4_5M_backfill_20260624.py", "date": "2026-06-24",
