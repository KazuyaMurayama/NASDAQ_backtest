...
目的: LT6 信号を S2_VZGated の lev シグナルに modeB で適用し、
...
出力:
  - b4_s2_lt6_sweep_results.csv  (プロジェクトルート)
  - B4_S2_LT6_SWEEP_2026-05-22.md (プロジェクトルート)
"""
...
def generate_report(results):
    ...
    lines = [
        f'# B4: S2_VZGated + {LT_SIGNAL} Sweep (N × k_lt)',
        '',
        '作成日: 2026-05-22',
        '最終更新日: 2026-05-22',
        '',
    ...
    lines += [
        ...
        '',
        '```',
        f'python -X utf8 src/b4_s2_lt6_sweep.py',
        '```',
        '',
...
def main():
    ...
    print('=' * 70)
    print(f'B4: S2_VZGated + {LT_SIGNAL} Sweep — {total_runs} runs')
    ...
    print()
    print('=' * 110)
    print(f'B4 {LT_SIGNAL} Results ({total_runs} runs) — sorted by Sharpe_OOS desc')
    print('=' * 110)
    ...
    csv_path = os.path.join(BASE, 'b4_s2_lt6_sweep_results.csv')
    pd.DataFrame(results).to_csv(csv_path, index=False, float_format='%.6f')
    print(f'\nSaved CSV: {csv_path}')

    md = generate_report(results)
    md_path = os.path.join(BASE, 'B4_S2_LT6_SWEEP_2026-05-22.md')
