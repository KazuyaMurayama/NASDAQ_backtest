...
> **本レポートは 「注目戦略比較表 (CFD軸, 6戦略)」および「6戦略比較表 (全指標 計算完了)」の2画像に記載されている主要10戦略を1表に統合し、品質チェック4観点とソースコード照合に基づく検証バックテスト結果を付記したものです。**
...
> 凡例: IS = in-sample期間 / OOS = out-of-sample期間 / ★ = カレンダー年ベース10年ローリングCAGR / ‡ = OOS期間(2021/5-2026/3)のみSharpe / ▷ = 日次ローリング5年CAGR下位10%パーセンタイル (§3参照) / **◆ = 現行ベスト戦略 (CURRENT_BEST_STRATEGY.md 2026-05-21)**

| # | 戦略 | CAGR_IS | CAGR_OOS | Worst5Y(FULL) | P10_5Y▷ | Worst10Y★ | MaxDD(FULL) | Sharpe_OOS | Trades/yr |
|---|------|---------|----------|---------|---------|----------|-------|------------|-----------|
| 1 | ◆ **S2_VZGated+LT2** (S2_VZGated + LT2-N750-k0.5-modeB) **← BEST** | +31.33% | +31.16% | −2.83% | **+9.36%** | +18.10%★ | −59.45% | **0.858** | ~27 |
| 2 | **S2_VZGated** (tv=0.8, k=0.3, gate=0.5) | +32.94% | +27.57% | −4.75% | **+7.32%** | +17.74%★ | −62.4% | 0.769 | 27 |
| 3 | P2 best (vol-target, tv=0.8) | +34.60% | +27.13% | −6.63% | **+8.54%** | +19.09%★ | −60.5% | 0.757 | 27 |
| 4 | S4_RelVol (l_base=7, k_rel=2.0) | +40.98% | +26.19% | −2.33% | **+11.54%** | +19.47%★ | −66.1% | 0.697 | 27 |
| 5 | CFD 7x (DH Dyn+7x 固定) | +43.35% | +24.44% | −5.24% | **+12.71%** | +25.37%★ | −65.0% | 0.670 | 27 |
| 6 | **DH Dyn 2x3x [A]** (TQQQ, th=0.15) Scenario D | +23.36% | +14.88% | +0.87% | **+9.57%** | +14.30%★ | −45.08% | 0.646 | ~27 |
| 7 | BH 1x (NASDAQ 素) | +11.13% | +10.11% | −16.77% | **+0.71%** | −5.67%★ | −77.9% | 0.540 | 0 |
| 8 | **P02_Dyn×CPI [mult]** (Best 2022防御) | +22.18% | +19.43% | +0.49% | **+8.54%** | +7.49%★ | −46.37% | 0.833‡ | ~27 |
| 9 | **P05_HY×CPI [mult]** (Best Worst5Y secondary) | +25.93% | +15.65% | +6.04% | **+11.10%** | +11.18%★ | −44.98% | 0.667‡ | ~27 |
| 10 | **P01_Dyn×HY [mult]** (Best DSR候補) | +22.24% | +19.92% | −0.25% | **+8.19%** | +8.48%★ | −42.85% | 0.829‡ | ~27 |
| 11 | **DH Dyn 2x3x [A+LT2]** (LT2-N750-k0.5-modeB, TQQQ参照) | +22.12% | +18.87% | +2.59% | **+9.69%** | +13.35%★ | −44.76% | 0.777 | ~27 |
...
- **▷ P10_5Y (下位10% 5年CAGR)**: 日次ローリング 5年窓 (252×5=1,260 日) の CAGR 分布の第10パーセンタイル。計算式: `(nav / nav.shift(1260))**0.2 - 1` の 10th percentile。1974〜2026 の全期間で約11,909サンプルから算出。Worst5Y (最悪の1サンプル) と比較して「非現実的でない悲観シナリオ」としての意義を持つ。`src/calculate_p10_5y.py` で計算 (2026-05-20)。戦略7〜9 (P-series) は `data/timing_signals_raw.csv` の HY/CPI シグナルを使用した異なるコストモデルで計算 (§17 参照)。
...
| **DH Dyn軸 (ETF 動的配分)** | DH Dyn 2x3x [A], DH Dyn 2x3x [A+LT2], P02/P01系 | TQQQ + Gold 2x + TMF |
...
## 14. 長期サイクルシグナル拡張 (LT sweep) — DH Dyn 2x3x [A+LT2] (2026-05-20 追補)

> **追補日: 2026-05-20 | 実行: claude-sonnet**

### 14.1 設計概要

3〜5年の長期サイクルを捉えるシグナル (LT) を DH Dyn 2x3x [A] に上乗せする拡張実験。
バックテスト対象: 72 configs = 3シグナル × 3ルックバック × 4強度 × 2モード。

| 次元 | 値 |
|------|-----|
| シグナル | LT1 (価格/N年MA対比 z-score), LT2 (N年モメンタム z-score), LT3 (拡張パーセンタイルランク) |
| ルックバック N | 750, 1000, 1250 営業日 (≈3/4/5年) |
| 強度 k_lt | 0.1, 0.2, 0.3, 0.5 |
| モード | A (raw_a2 をシグナルで乗算調整: タイミング・量両方変化), B (リバランス後 lev に加算バイアス: タイミング変化なし) |

**設計思想**: 長期サイクルは逆張り (contrarian)。シグナル正値 = 過熱 → NASDAQ 配分を減少、シグナル負値 = 割安 → 配分を増加。

### 14.2 採択基準 (Opus §6 品質ゲート)

ベースライン: DH Dyn 2x3x [A] Scenario D — CAGR_OOS=14.88%, Sharpe_OOS=0.6460, MaxDD_FULL=−45.08%, W5Y=+0.87%, W10Y★=+14.30%

| ゲート | 条件 |
|--------|------|
| C1 CAGR_OOS | ≥ ベースライン +0.5pp (≥15.38%) |
| C2 IS-OOS乖離 | ≤ 10pp |
| C3 Sharpe_OOS | ≥ ベースライン −0.02 (≥0.626) |
| C4 MaxDD | ≥ −55% |
| C5 Worst5Y | ≥ 0% |
| C6 Worst10Y★ | ≥ ベースライン −2pp (≥12.30%) |
| C7 取引回数 | 10〜60回/年 (総計 522〜3,136回) |

### 14.3 スイープ結果 — 採択候補トップ10 (Sharpe_OOS 順)

**採択数: 18 / 72 configs (全ゲート通過)**

| config | CAGR_IS | CAGR_OOS | CAGR_FULL | Sharpe_OOS | MaxDD | Worst5Y | Worst10Y★ | IS-OOS乖離 |
|--------|---------|---------|----------|-----------|-------|--------|----------|-----------|
| **LT2-N750-k0.5-modeB** | +22.12% | **+18.87%** | +21.81% | **0.777** | −44.76% | +2.59% | +13.35%★ | 3.25pp |
| LT2-N750-k0.3-modeB | +22.87% | +17.55% | +22.35% | 0.743 | −44.80% | +2.53% | +13.85%★ | 5.31pp |
| LT2-N750-k0.2-modeB | +23.36% | +16.88% | +22.72% | 0.721 | −45.17% | +2.55% | +14.08%★ | 6.48pp |
| LT3-N750-k0.5-modeB | +22.89% | +16.27% | +22.24% | 0.711 | −46.19% | +2.67% | +13.98%★ | 6.62pp |
| LT2-N750-k0.1-modeB | +23.68% | +16.32% | +22.94% | 0.701 | −45.77% | +2.47% | +14.21%★ | 7.35pp |
| LT3-N750-k0.3-modeB | +23.44% | +16.02% | +22.70% | 0.697 | −46.27% | +2.62% | +14.19%★ | 7.42pp |
| LT3-N1000-k0.5-modeB | +22.78% | +15.77% | +22.09% | 0.689 | −47.68% | +3.10% | +13.41%★ | 7.01pp |
| LT3-N750-k0.2-modeB | +23.63% | +15.78% | +22.84% | 0.687 | −46.32% | +2.56% | +14.25%★ | 7.85pp |
| LT2-N1000-k0.2-modeB | +23.00% | +16.11% | +22.32% | 0.687 | −47.10% | +3.29% | +12.84%★ | 6.89pp |
| LT2-N1000-k0.1-modeB | +23.52% | +15.47% | +22.71% | 0.670 | −46.75% | +2.81% | +13.92%★ | 8.05pp |

*ベースライン: CAGR_OOS=14.88%, Sharpe_OOS=0.6460*

### 14.4 ベストコンフィグ: LT2-N750-k0.5-modeB

**シグナル**: LT2 = 750営業日 (≈3年) モメンタム z-score (`close/close.shift(750) - 1` を 1500日ローリング分布で標準化)  
**モード B**: リバランス後 lev (0〜1スケール) に `bias = -0.5 × k_lt × signal` を加算。タイミング変更なし、NASDAQスリーブのみスケール調整。

| 指標 | ベースライン (DH Dyn [A] Sc.D) | LT2-N750-k0.5-modeB | 差分 |
|------|------------------------------|---------------------|------|
| CAGR_OOS | +14.88% | **+18.87%** | **+3.99pp** |
| CAGR_IS | +23.36% | +22.12% | −1.24pp |
| CAGR_FULL | +22.50% | +21.81% | −0.69pp |
| Sharpe_OOS | 0.646 | **0.777** | **+0.131** |
| MaxDD_FULL | −45.08% | −44.76% | +0.32pp (改善) |
| Worst5Y_FULL | +0.87% | +2.59% | +1.72pp (改善) |
| Worst10Y★ | +14.30% | +13.35% | −0.95pp |
| n_trades/yr | 27.1 | 27.1 | 変化なし |
| IS-OOS乖離 | — | 3.25pp | 採択候補中最小 |

**解釈**:
- **CAGR_OOS +3.99pp 改善**: OOS期間 (2021-2026) は NASDAQ 急騰 → 急落 → 回復のサイクルが顕著で、3年モメンタムの逆張りバイアスが有効に機能した。
- **IS CAGR 微減 (−1.24pp)**: IS期間の長期上昇トレンド局面で上方バイアスを一定量カットしたことによるコスト。許容範囲内。
- **IS-OOS 乖離 3.25pp が採択候補中最小**: 過学習リスクが相対的に低い。

### 14.5 注意事項・限界

1. **OOS サンプル期間が短い (4.9年)**: 1つのサイクルのみをカバー。追加 OOS データ蓄積が必要。
2. **LT2 採択集中**: 採択 18 config のうち LT2 が多数を占め、サンプル依存リスクに注意。
3. **CURRENT_BEST_STRATEGY.md は未更新**: 本§14は参考値。ベスト戦略更新には別途 Opus レビューを経ること。
4. **IS CAGR 低下を許容する採択基準**: 本ゲートは OOS 改善・リスク抑制優先であり、IS CAGR 低下は許容している。本番採用時はこのトレードオフを明確に認識すること。

### 14.6 スクリプト・出力ファイル

| ファイル | 説明 |
|---------|------|
| `src/long_cycle_signal.py` | LT1/LT2/LT3/LT4/LT6/LT7 シグナル計算・パイプライン統合モジュール |
| `src/backtest_lt_strategies.py` | 72-config スイープ runner・採択フィルタ (Phase-1) |
| `lt_sweep_results.csv` | 全 72 config の詳細指標 (CAGR/Sharpe/MaxDD/W5Y/W10Y★ 等) |

---

## 15. LT拡張スイープ Phase-2 (N拡張 + 新シグナル LT4/LT6/LT7 + Mode C) — 2026-05-20 追補

> **追補日: 2026-05-20 | 計画: claude-opus / 実行: claude-sonnet**

### 15.1 設計概要

Phase-1 (72 configs) の知見を踏まえた拡張スイープ。以下の4軸を追加検証した:

| 追加要素 | 内容 |
|---------|------|
| **N拡張** | N=500 (≈2年), N=600 (≈2.4年), N=1500 (≈6年) を追加 |
| **k拡張** | k=0.7, k=1.0 を追加 (Phase-1 best の k=0.5 超の領域を探索) |
| **LT4** | LT2(60%) + LT3(40%) 加重合成シグナル。2つの独立した長期レンズの ensemble |
| **LT6** | Vol-adjusted momentum (Sharpe-like)。AQR-style リスク調整済みモメンタム: `Σlog_ret(N) / (σ_daily(N) × √N)`, expanding 95th-pct で正規化 |
| **LT7** | LT2 マルチタイムフレームブレンド: `(LT2(N=750) + LT2(N=1250)) / 2` (N引数固定) |
| **Mode C** | Mode A + Mode B の等加重ブレンド (α=0.5)。raw_a2 を半強度 A 調整後にリバランス、さらに半強度 B バイアスを加算 |

**総追加 configs: 56 | 採択基準: Phase-1 と同一ゲート C1〜C7**

### 15.2 Phase-2 スイープ結果概要

| 区分 | 結果 |
|-----|------|
| 実行 configs | 56 |
| 全ゲート通過 | **18 / 56** |
| 累計通過 (Phase-1+2) | **33 / 128** |

### 15.3 採択候補 — Sharpe_OOS 上位 (Phase-2 内)

| config | CAGR_IS | CAGR_OOS | CAGR_FULL | Sharpe_OOS | MaxDD | Worst5Y | Worst10Y★ | IS-OOS乖離 |
|--------|---------|---------|----------|-----------|-------|--------|----------|-----------|
| **LT4-N750-k0.7-modeB** | +21.86% | **+18.75%** | +21.56% | **0.776** | −45.53% | +2.95% | +13.47%★ | 3.10pp |
| LT4-N750-k0.5-modeB | +22.34% | +18.04% | +21.93% | 0.760 | −44.90% | +2.57% | +13.61%★ | 4.30pp |
| LT7-N750x1250-k0.7-modeB | +21.44% | +18.39% | +21.15% | 0.750 | −44.64% | +2.60% | +12.65%★ | 3.05pp |
| LT7-N750x1250-k1.0-modeB | +20.96% | +18.39% | +20.72% | 0.745 | −44.76% | +2.55% | +12.53%★ | 2.57pp |
| LT7-N750x1250-k0.5-modeB | +21.90% | +17.97% | +21.52% | 0.745 | −44.87% | +2.36% | +12.95%★ | 3.92pp |
| LT2-N750-k0.7-modeB | +22.04% | +17.97% | +21.66% | 0.740 | −45.85% | +3.63% | +13.83%★ | 4.07pp |
| LT2-N750-k0.7-modeC | +19.61% | +17.48% | +19.41% | 0.739 | −44.76% | +2.09% | +12.61%★ | 2.14pp |
| LT4-N1000-k1.0-modeB | +20.35% | +18.65% | +20.19% | 0.738 | −49.33% | +2.47% | +9.69%★ | 1.70pp |
| LT2-N1000-k1.0-modeB | +19.90% | +18.52% | +19.77% | 0.720 | −49.17% | +1.56% | +8.84%★ | 1.38pp |
| LT4-N1000-k0.7-modeB | +20.57% | +17.33% | +20.10% | 0.712 | −48.58% | +2.02% | +10.36%★ | 3.79pp |

*注: LT4-N1000-k1.0 と LT2-N1000-k1.0 はW10Y★が8-9%台でゲートC6(≥12.30%)を割り込む可能性に注意*

### 15.4 累計ベスト比較 (Phase-1+2 合算 Top-5 by Sharpe_OOS)

| 順位 | config | CAGR_OOS | CAGR_FULL | Sharpe_OOS | MaxDD | Worst5Y | Worst10Y★ | IS-OOS乖離 |
|-----|--------|---------|----------|-----------|-------|--------|----------|-----------|
| 1 | **LT2-N750-k0.5-modeB** (Phase-1) | **+18.87%** | +21.81% | **0.777** | −44.76% | +2.59% | +13.35%★ | 3.25pp |
| 2 | **LT4-N750-k0.7-modeB** (Phase-2) | +18.75% | +21.56% | 0.776 | −45.53% | +2.95% | +13.47%★ | 3.10pp |
| 3 | LT4-N750-k0.5-modeB (Phase-2) | +18.04% | +21.93% | 0.760 | −44.90% | +2.57% | +13.61%★ | 4.30pp |
| 4 | LT7-N750x1250-k0.7-modeB (Phase-2) | +18.39% | +21.15% | 0.750 | −44.64% | +2.60% | +12.65%★ | 3.05pp |
| 5 | LT7-N750x1250-k0.5-modeB (Phase-2) | +17.97% | +21.52% | 0.745 | −44.87% | +2.36% | +12.95%★ | 3.92pp |

**Phase-1 ベスト (LT2-N750-k0.5-modeB) が依然総合最良。**  
LT4-N750-k0.7-modeB が Sharpe 0.776 でほぼ同等 (差 0.001)、かつ Worst5Y・Worst10Y★ では LT4 がやや優位。

### 15.5 シグナル別評価

| シグナル | 評価 | 主要観察 |
|---------|------|---------|
| **LT2** (Phase-2 追加 k) | ★★★★ | k=0.7 で N=750 が +17.97%/0.740。N=1000-1500 では Sharpe↑ だが W10Y★↓ |
| **LT4** (LT2+LT3 合成) | ★★★★ | N=750-k0.7 が Sharpe 0.776 でほぼ最良。IS-OOS gap 3.10pp は良好 |
| **LT7** (multi-TF blend) | ★★★ | k=0.7-1.0 で安定した OOS 18.4% 台。MaxDD も−44% 台と優秀 |
| **LT6** (vol-adj momentum) | ★★ | CAGR_OOS ≤ 15.0%、全 configs が C1 未達またはゲート失敗。構造的に不向き |
| **Mode C** (A+B blend) | ★★★ | Sharpe 0.739 (LT2-N750-k0.7) は良好。ただし IS CAGR が低下 (20%未満) |
| **N=500/600** (短期) | ★★ | IS-OOS gap が 7〜9pp に拡大。全 configs が C2 (≤10pp) 境界付近または超過 |
| **N=1500** (長期) | ★★★ | Sharpe が最大 0.814 (k=1.0) だが W10Y★ が 8.4% で C6 失敗。Sharpe重視には注目値 |

### 15.6 注目した不合格候補 (ゲート外の参考値)

**LT2-N1500-k1.0-modeB** — C6 不合格 (W10Y★=8.40% < 12.30%) だが最高 Sharpe=0.814
- CAGR_OOS=17.85%, MaxDD=−46.40%, IS-OOS gap=2.59pp
- 長期 (6年) ルックバックで OOS 期間の Sharpe が突出。ただし 10年最悪パフォーマンスが悪化するトレードオフ
- 用途: Sharpe 最大化重視・W10Y★ ゲートを緩和する場合の候補

**LT2-N750-k0.7-modeC** — 全ゲート通過 (Sharpe 0.739)
- IS CAGR=19.61% (低めだが 20% 未満)、IS-OOS gap=2.14pp (全候補中最小クラス)
- Mode C の頑健性検証として有意義な結果

### 15.7 インプリケーションと次ステップ

1. **N=750 の優位性が確認された**: Phase-1・Phase-2 を通じて N=750 (≈3年) が consistently 最良。米国景気サイクルの中期転換点を捉える長さとして妥当。
2. **LT4 (ensemble) が LT2 単独とほぼ同等**: 独立情報を統合することで IS-OOS 乖離がわずかに改善 (3.10pp vs 3.25pp)。単純な複雑化ではなく ensemble の効果が確認された。
3. **LT6 の失敗**: Vol-adjusted momentum はこの戦略との相性が悪い。DH Dyn 自体が已にリスク調整済み (threshold ベース) であり、LT6 の追加情報量が限定的だった。
4. **現状ベスト**: LT2-N750-k0.5-modeB (Phase-1) が Sharpe・CAGR_OOS で依然最良。LT4-N750-k0.7-modeB は実質同等で Worst5Y がやや優位。両者の ensemble 検討が次フェーズの候補。
5. **CURRENT_BEST_STRATEGY.md は引き続き未更新**: 本§15も参考値。正式採用には独立した Opus レビューと追加 OOS サンプルの蓄積を推奨。

### 15.8 スクリプト・出力ファイル

| ファイル | 説明 |
|---------|------|
| `src/long_cycle_signal.py` | LT4/LT6/LT7 シグナル追加 + Mode C ヘルパー関数 (更新) |
| `src/backtest_lt_extended.py` | 56-config 拡張スイープ runner (新規) |
| `lt_extended_results.csv` | Phase-2 全 56 config の詳細指標 |
| `lt_combined_results.csv` | Phase-1+2 累計 128 config 統合結果 |

---

## §16. ファクトチェック: レバレッジ製品コスト & ポジション遅延検証

**実施日:** 2026-05-20  
**対象:** DH Dyn 2x3x [A] Scenario D (`corrected_strategy_backtest.py`)  
**スクリプト:** `src/factcheck_costs_delay.py` → `factcheck_sensitivity_results.csv`

---

### 16.1 概要

Scenario D バックテストに使用しているコスト定数・遅延モデルが、実際の製品（UGL, TMF, TQQQ）の特性と整合しているかを以下4観点で検証した:

1. **Gold 2x TER**: バックテストプロキシ (WisdomTree 2036, TER=0.50%) vs 実運用可能製品 (UGL, TER=0.95%)
2. **TMF TER**: バックテスト想定 0.91% vs 最新研究報告 0.95-1.06%
3. **TQQQ スワップスプレッド**: OLS 回帰による実証検証 (想定 0.50%/yr)
4. **SOFR プロキシ品質**: DTB3 (3M T-bill, 割引ベース) の add-on 補正バイアス
5. **DELAY=2 (T+2) の妥当性**: 米国 T+1 移行 (2024-05) 後の日本側決済との整合性

---

### 16.2 コスト感度分析テーブル

ベースライン: **CAGR_FULL=22.97%, Sharpe_OOS=0.6498, MaxDD=−46.40%, W5Y=2.36%**

| シナリオ | 変更内容 | ΔCAGR_FULL (bps) | CAGR_FULL | ΔSharpe_OOS (bps) | 評価 |
|---------|---------|-----------------|----------|------------------|------|
| **D_baseline** | 現状 Scenario D | ±0 | **22.97%** | ±0 | 基準 |
| D1: Gold TER=0.95% | UGL 実費 | **−10.5** | 22.87% | −30.6 | ★ 最大コスト差 |
| D2: TMF TER=0.95% | +4 bps | −0.9 | 22.96% | −2.7 | 影響軽微 |
| D3: TMF TER=1.06% | +15 bps | −3.5 | 22.94% | −10.2 | 影響小 |
| D4: Gold+TMF 両方 realistic | D1+D3 | **−14.0** | 22.83% | −40.8 | 複合コスト影響 |
| D5: Delay=1 (T+1) | 1日早い | +147.9 | 24.45% | **−1155.3** | OOS 大幅悪化 → 非採用 |
| D6: Delay=3 (T+3) | 1日遅い | +15.0 | 23.12% | +105.9 | OOS 改善 → 参考値 |
| D7: Swap+25bps (0.75%) | +25bps | −41.6 | 22.56% | −127.1 | ★ スワップ感度高 |
| D8: Swap+50bps (1.00%) | +50bps | −83.1 | 22.14% | −254.3 | スワップ上限シナリオ |
| D9: ALL realistic | D1+D3+Delay=1 | +133.8 | 24.31% | **−1196.1** | CAGR↑だが OOS 大幅悪化 |

> **注**: D9 の「ALL realistic」は Delay=1 の効果 (+148 bps) が Gold/TMF コスト差 (−14 bps) を上回るため CAGR_FULL は上昇するが、Sharpe_OOS が −1,196 bps と壊滅的に悪化する (CAGR_OOS: 14.79% → 11.16%)。**Delay=1 はバックテストの IS データに対してより最適化されるが OOS では却って悪化するため、DELAY=2 の保守的設定は正当化される。**

---

### 16.3 SOFR プロキシ品質 (DTB3)

| 項目 | 値 |
|-----|---|
| DTB3 割引→add-on 補正プレミアム (2020年以降) | **+3.2 bps** |
| 推定 SOFR 過少計上 (コスト面) | 約 −13 bps/yr |
| ポートフォリオ SOFR エクスポージャー (TQQQ 2× + TMF 2× + Gold 1×) | 推定 **1.40×** |
| CAGR への影響推定 | **約 −18 bps/yr** (コスト過少計上 → リターン過大) |

**解釈**: DTB3 を `/252` で割って SOFR プロキシとして使用する手法は、割引ベース利回りの add-on 換算を省略するため実際の SOFR より約 3 bps/yr 低く見積もる。ポートフォリオ全体では年間 **−18 bps** のコスト過少計上バイアスが存在する。

---

### 16.4 TQQQ スワップスプレッド OLS 検証

分析期間: 2010-02-12 〜 2026-03-31 (n=4,027 日)

**回帰式**: `r_TQQQ = α + β_QQQ × r_QQQ + β_SOFR × sofr_daily`

| パラメータ | 推定値 | 期待値 | 評価 |
|-----------|-------|-------|------|
| β_QQQ | **2.957** | ~3.0 | ✅ ほぼ 3× |
| β_SOFR | **−2.138** | ~−2.0 | ✅ 2×SOFR ファイナンシング確認 |
| α (年換算) | −1.565%/yr | ~−2.3% | - |
| R² | 0.99784 | - | ✅ 高い説明力 |

**スワップスプレッド逆算:**
- OLS 実証スワップスプレッド: **0.705%/yr**
- バックテスト使用値: **0.50%/yr**
- 差異: **+20.5 bps/yr** (モデルがコストを過少計上)

α が期待値 (約 −2.3%) より小さいのは、α にスワップ以外の ETF コスト (配当影響など) が混在するため。β_SOFR ≈ −2.14 は **2×SOFR ファイナンシング構造を実証的に確認**する。

スワップスプレッド差 (+20.5 bps) は感度分析上 **D7 (−41.6 bps) の約 0.8倍 ≈ −34 bps/yr** に相当するコスト過少計上バイアスを意味する。

---

### 16.5 ポジション遅延モデル (DELAY=2)

| 項目 | 内容 |
|-----|-----|
| 実装 | シグナル計算日 t → ポジション反映 t+2 |
| 米国決済 (〜2024-04) | T+2 → DELAY=2 と整合 |
| 米国決済 (2024-05〜) | T+1 移行 (米国市場側) |
| **日本側決済 (現在)** | **T+2 継続** (SBI証券・楽天証券 米国 ETF) |
| 感度分析 (D5: DELAY=1) | CAGR_FULL +148 bps だが Sharpe_OOS −1,155 bps |
| 感度分析 (D6: DELAY=3) | CAGR_FULL +15 bps, Sharpe_OOS +106 bps |

**結論**: DELAY=2 は日本リテール投資家の実態と整合し、かつ OOS パフォーマンスでも T+1 より優位。保守的かつ現実的な設定として正当化される。

---

### 16.6 Gold 製品可用性とコスト整理

| 製品 | TER | 日本購入可否 | SOFR 構造 | バックテスト利用 |
|-----|-----|------------|----------|--------------|
| **WisdomTree 2036 (LSE)** | 0.49-0.50% | ❌ SBI/楽天 対応外 | 1×SOFR | プロキシとして使用 |
| **UGL (ProShares Ultra Gold 2x)** | **0.95%** | ✅ SBI 米国 ETF 対応 | 1×SOFR | **実運用製品** |
| SBI 金 CFD (最大 20x) | 非透明 (~4.4-5.0%/yr) | ✅ | SOFR + lease + margin | バックテスト外 |

TER ギャップ: **+0.45%/yr** → Gold 約 20% 保有時 → **CAGR 影響: −9 bps/yr (≈ −10.5 bps/yr 実測)**

---

### 16.7 コスト過少計上バイアス 総合評価

| バイアス項目 | 推定 CAGR 影響 | 確度 |
|------------|--------------|-----|
| Gold TER (0.50% → 0.95%) | **−10.5 bps/yr** | ★★★ 実測 |
| TMF TER (0.91% → 1.06%) | −3.5 bps/yr | ★★☆ 推定 |
| スワップスプレッド (+20.5 bps → 0.705%) | **−34 bps/yr** | ★★☆ OLS 逆算 |
| SOFR プロキシ add-on バイアス | −18 bps/yr | ★★☆ 理論推定 |
| **合計コスト過少計上** | **約 −66 bps/yr** | - |

**現実的な Scenario D CAGR 推定**:

```
22.97% (バックテスト) − 0.66% (コスト補正) ≈ 22.3%
```

> これは公表済みの Scenario D 公称値 **22.50%** (SOFR financing 補正 −8.13% 適用後) と概ね整合する。
...
| 観点 | 結論 | 重要度 |
|-----|-----|-------|
| **NASDAQ データ** | Close = Adj Close → 価格指数 ✅ TQQQ (NDX 3×) と整合 | - |
| **DELAY=2 (T+2)** | 日本側 T+2 と整合、OOS でも T+1 より優位 → **設定正当化** ✅ | 高 |
| **TQQQ SOFR 構造** | OLS 実証で β_SOFR ≈ −2.14 → 2×SOFR ファイナンシング **確認** ✅ | 高 |
| **Gold TER ギャップ** | WisdomTree proxy 0.50% vs UGL 実費 0.95% → **−10.5 bps/yr 過大** ⚠️ | 中 |
| **TMF TER** | 0.91% vs 最新 1.06% → −3.5 bps/yr の軽微な過大 ⚠️ | 低 |
| **スワップスプレッド** | OLS 逆算 0.705% vs モデル 0.50% → **−34 bps/yr 過大** ⚠️ | 中-高 |
| **SOFR プロキシバイアス** | DTB3 割引ベース → **−18 bps/yr 過大** ⚠️ | 中 |
| **合計推定バイアス** | **約 −66 bps/yr** (コスト過少計上) → 現実 CAGR ≈ **22.3%** | 中 |

**推奨アクション**:
1. `corrected_strategy_backtest.py` の `GOLD_2X_COST` を `0.0050` → `0.0095` に修正 (UGL 実費反映)
2. `SWAP_SPREAD` を `0.0050` → `0.0070` に引き上げ (OLS 実証値 0.705% 採用)
3. `product_costs.py` を main backtest の単一真実 (Single Source of Truth) として統合
...
| ファイル | 説明 |
|---------|------|
| `src/factcheck_costs_delay.py` | ファクトチェック実行スクリプト (新規) |
| `factcheck_sensitivity_results.csv` | 10シナリオ × 11指標 の感度分析結果 |
| `factcheck_log.txt` | 全セクション実行ログ (UTF-8) |

---

## §17. P10_5Y 分析 (Opus レビュー 2026-05-20)

### 17.1 P10_5Y が明らかにすること

P10_5Y は Worst5Y が捉えられない「典型的な悪いシナリオ」を可視化する。Worst5Y は GFC・ITバブル崩壊などの単一テールイベントの最底値であり、投資家が実際に経験する可能性の高い「悪い5年間」を反映しない。一方で P10_5Y は約11,909のローリング5年窓のうち下位10%、すなわち「100回に10回起こりうる最悪の5年CAGR」を示す。

**最重要洞察: 戦略5 (DH Dyn 2x3x [A] Scenario D) は Worst5Y=+0.87% という辛うじてプラスの数値から、P10_5Y=+9.57% へと大幅に改善する。** これは Worst5Y が孤立したアウトライヤーであり、90% の5年窓では約10%以上の年率リターンを達成することを示す。Worst5Y の見かけ上の辛さは実態を反映していない。

### 17.2 全10戦略の P10_5Y 解釈

| # | 戦略 | Worst5Y | P10_5Y | スプレッド | 評価 |
|---|------|---------|--------|-----------|------|
| 1 | S2_VZGated | −4.75% | +7.32% | 12pp | CFD 集中テールリスク |
| 2 | P2 best | −6.63% | +8.54% | 15pp | CFD 集中テールリスク |
| 3 | S4_RelVol | −2.33% | **+11.54%** | 14pp | 高 CAGR × 高 P10 |
| 4 | CFD 7x | −5.24% | **+12.71%** | 18pp | 最高 P10_5Y (全戦略中) |
| 5 | DH Dyn [A] Scenario D | +0.87% | +9.57% | 8.7pp | ★ テール集中なし |
| 6 | BH 1x | −16.77% | +0.71% | 17.5pp | ベンチマーク |
| 7 | P02_Dyn×CPI | +0.49% | +8.54% | 8.1pp | ゲートで安定化 |
| 8 | **P05_HY×CPI** | **+6.04%** | **+11.10%** | **5.1pp** | ★ 最良テール床 |
| 9 | P01_Dyn×HY | −0.25% | +8.19% | 8.4pp | ゲートで安定化 |
| 10 | DH Dyn [A+LT2] | +2.59% | +9.69% | 7.1pp | ★ 最小スプレッド |

> スプレッド = P10_5Y − Worst5Y。値が**小さいほど**テールリスクが分散的 (一度きりのイベントに依存していない)。

### 17.3 主要発見 (Opus 分析)

1. **最良テール耐性: 戦略8 (P05_HY×CPI)** — 絶対的な下限フロアが最高 (P10=+11.10%, Worst=+6.04%) かつスプレッドも最小 (5.1pp)。HY×CPI ゲートがインフレ期・信用逼迫期のエクスポージャーを抑制し、一貫したテールフロアを実現。

2. **CFD 7x の再評価** — P10_5Y=+12.71% は全戦略中最高。Worst5Y=−5.24% が示すテールリスクは「集中的 (concentrated)」であり、構造的に脆弱ではない。ただし MaxDD=−65.0% に注意。

3. **ヘッジあり DH Dyn 系のスプレッド優位性** — 戦略5, 7, 9, 10 (DH Dyn ファミリー) のスプレッドは 7〜9pp。CFD 軸 (1〜4) の 12〜18pp と比較して、ヘッジ戦略がより一貫したダウンサイド床を提供することを定量化。

4. **BH 1x の機会費用** — P10=+0.71% はレバレッジ戦略の +8〜+13% 集団と比較して如実。無ヘッジ買い持ちの機会費用を数値化。

### 17.4 留意事項

- **戦略7〜9のコストモデル差**: P-series (P01/P02/P05) は `build_gold_tocom` + `bond_3x (SOFR なし)` で計算されており、戦略1〜6, 10の Scenario D (gold_2x SOFR あり + bond_3x SOFR あり) と異なるコスト構造。クロス比較は参考値として扱うこと。
- **サンプル非独立性**: 11,909 日次ローリング窓は重複しており、実質独立サンプル数は約10〜15程度。P10_5Y の正確な数値は指標的 (indicative) として解釈するべき。
- **戦略8 (P05_HY×CPI)**: CPI ゲートがインフレ期 (主に1970-80年代) のエクスポージャーを大きく削減しており、その恩恵がバックテスト期間の P10 に反映されている。2022年以降の OOS 挙動の個別確認を推奨。

### 17.5 スクリプト・出力ファイル

| ファイル | 説明 |
|---------|------|
| `src/calculate_p10_5y.py` | 全10戦略 P10_5Y 計算スクリプト (新規) |
| `src/sleeves_extended.py` | P-series Gold/Bond ビルダー (他ブランチからコピー) |
| `p10_5y_results.csv` | 全10戦略 P10_5Y + Worst5Y 計算結果 |
| `data/timing_signals_raw.csv` | HY/CPI/DFF シグナル (FRED 取得、13169行) |