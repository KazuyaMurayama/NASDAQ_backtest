...
## ✅ 適用済みステータス（2026-06-11 更新）

> **本提案は 2026-06-11 に `CURRENT_BEST_STRATEGY.md` v5 として適用済み**（QC検証で不一致ゼロ確認）。  
> 適用内容: v4.5表を realistic full L×・正典窓(49)WFAで更新、R4（DH-W1 Trades 68.7→17.6）、R9（vz065_l7 N/A補填）、⓽/ⓒ基準明示、二面性注記。  
> g14が(L-1)×借入基準と判明し E4 ⓽OOS +22.4%→+18.06%（full L×正基準）に補正。

### 提案項目別の適用状況

| 提案ID | 内容 | ステータス |
|---|---|---|
| R4 | DH-W1 Trades/yr 68.7→17.6 | **[適用済み]** v5 適用 |
| R2/R10 | CAGR列⓽税後明示・ⓒ税前との基準分離 | **[適用済み]** v5 適用 |
| R7 | CFD realistic（SBI CFD 3.0% full L×）基準値の併記、E4 ⓽OOS +22.4%→+18.06%補正 | **[適用済み]** v5 適用（g14が(L-1)×借入基準と判明し補正） |
| R9 | vz065_l7 CI95/WFE「N/A」→+17.37%/1.461（realistic full L×）補填 | **[適用済み]** v5 適用 |
| 二面性注記 | v4.5推奨表末尾への二面性ブロック追加 | **[適用済み]** v5 適用 |
| R3 | DH-W1 CAGR二重値整理（旧split +13.66% vs canonical +15.74%）注記 | **[適用済み]** v5 適用 |
| P7「+14.90%」旧split個別照合 | P7 CAGR⓽ +14.90%と canonical +19.33% の差分を個別照合 | **[残課題]** 未実施（旧split・コスト体系の複合差異が未確定） |
| t_p/bootstrap | P7投信の permutation検定・block bootstrap | **[残課題]** 未実施（正式Active昇格条件として継続Pending） |
| EVALUATION_STANDARD §1.1 出典明記 | 税乗数0.8273の計算スクリプト出典を§1.1に追記 | **[残課題]** 未適用（EVALUATION_STANDARD.md未変更） |

---

## [適用済み] R4 — DH-W1 Trades/yr 誤り訂正
...
## [適用済み] R2 / R10 — CAGR 列の⓽税後明示・ⓒ税前との基準分離
...
## [適用済み] R7 — CFD realistic（SBI CFD / full L×）基準値の併記
...
## [適用済み] R9 — vz065_l7 の CI95 / WFE「N/A」を実値に補填
...
## [残課題] EVALUATION_STANDARD §1.1 への追記提案
...
> **本提案ファイルは修正の「提案」を記録するものである。**  
> 主要提案（R4/R2/R10/R7/R9/R3/二面性）は **2026-06-11 に `CURRENT_BEST_STRATEGY.md` v5 として適用済み**。  
> 残課題（P7旧split照合・t_p/bootstrap・EVALUATION_STANDARD §1.1出典明記）は上記 [残課題] セクション参照。
