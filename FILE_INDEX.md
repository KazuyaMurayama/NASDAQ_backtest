...
### 2026-06-07 DH-W1 キャッシュ・スリーブ 1倍投信置換（投信環境 Active 候補）

> DH-W1 の OUT(キャッシュ 46.9%/6,171日)期を 1 倍投信で運用置換。投信環境 Active 候補 4 戦略を確定。詳細は [STRATEGY_REGISTRY.md](STRATEGY_REGISTRY.md) §2 / [CURRENT_BEST_STRATEGY.md](CURRENT_BEST_STRATEGY.md) §投信環境 / [RESEARCH_CONTEXT.md](RESEARCH_CONTEXT.md) §3.3。

| ファイル | 説明 |
|---|---|
| `analysis_cash_sleeve/CASH_SLEEVE_REPORT_20260607.md` | **4戦略版レポート（正典）** — 10指標・年次表(1974-2026 税後)・全商品コスト表(SOFR/TER/スワップ/売買/税20.315%)・執行ラグ検証(レバ脚 DELAY=2/投信 5BD)・税率根拠 |
| `analysis_cash_sleeve/cash_sleeve_sim.py` | シミュレーション本体（P2/P5/P7 + DH-W1 baseline、5営業日ラグ・WFA 50窓） |
| `analysis_cash_sleeve/cash_sleeve_4strategies_metrics.csv` | 4戦略 × 10指標 |
| `analysis_cash_sleeve/cash_sleeve_4strategies_yearly.csv` | 4戦略 年次リターン(1974-2026・税後) |
| `analysis_cash_sleeve/cash_sleeve_7patterns_metrics.csv` | 全7パターン指標（NASDAQ/Gold/Bond単独・各種blend 含む） |
| `analysis_cash_sleeve/cash_sleeve_part1_asset_during_cash.csv` | OUT 期 6,171日の各1倍資産挙動（NASDAQ −7.63%/Gold +4.07%/Bond +6.51%） |

---

## 2026-06-11〜12 P09 GAS並走プロジェクト成果物（手動追記セクション）

> ⚠ **移動・削除禁止**: golden/close CSV と live_spec JSON は本番GAS（NASDAQ-strategy-gas の `testP09Parity()`）が **GitHub raw URL で実行時参照**している。パス変更時はGAS側 `P09_CONFIG.PARITY` の更新が必須。

| ファイル | 説明 |
|---|---|
| `P09_FORWARD_RETURN_REPORT_20260612.md` | P09実戦略 5営業日後フォワードリターン ビン別レポート（GAS通知の一次根拠・年1回更新） |
| `audit_results/p09_live_spec_20260611.json` | P09ライブ運用 凍結定数（GAS `P09_CONFIG` の正典） |
| `audit_results/p09_golden_vectors_20260611.csv` | ゴールデンベクトル756日分（GASパリティテスト参照・移動禁止） |
| `audit_results/p09_close_history_20260611.csv` | ^IXIC close履歴3,100日（LT2パリティ用・移動禁止） |
| `audit_results/p09_forward_return_20260612.csv` | フォワードリターン ビン別生データ |
| `src/audit/export_p09_live_spec_20260611.py` | 凍結spec / golden 生成スクリプト（年1回再実行） |
| `src/audit/preparity_p09_gas_port_20260611.py` | GAS移植アルゴリズムのPython側パリティ証明（7チェック） |
| `src/audit/p09_forward_return_20260612.py` | フォワードリターン表の生成（年1回再実行） |
