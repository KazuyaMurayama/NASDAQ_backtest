...
### 3.2 採否ルールの改訂案 — バランス型スコアカード（§3.13 の発展）

> **重要な自己訂正 (2026-06-11)**: 当初案「WFA ∧ CPCV_p10 ∧ Regime_min ∧ bootstrap」は **全てが"リターンの下限・頑健性"という1ファミリーを4通りに切っただけ**で、4つの独立ハードルではない。過学習・偶然は防げるが、**MaxDD・Sharpe・テール(W10Y/P10)・コスト(Trades・CFD依存度)を一切含まず**、高レバ・高DD戦略が通り抜ける。→ 単独基準として**不適**。以下の **多次元スコアカード**に置換する。

採否は「全ハード閾値AND」ではなく、**6次元を提示し、ユーザー選好（現状: CAGR優先・MaxDD許容）で重み付けした総合判断**:

| # | 次元 | 指標 | 役割 |
|---|---|---|---|
| 1 | **リターン水準** | min(IS,OOS) CAGR⓽ **かつ** WFA CI95下限⓽（co-primary 両提示） | 長期CAGRの保守値＋窓跨ぎ汎化 |
| 2 | **リターン頑健性（過学習防止）** | CPCV p10 / Regime_min / WFE∈[0.5,1.5] | "1局面のまぐれ"でないか。**ここが①と相関する1ファミリー** |
| 3 | **リスク** | MaxDD（許容上限内）・Sharpeⓒ（baseline近傍以上） | DD許容でも無制限ではない |
| 4 | **テール** | Worst10Y★⓽>0 ・ P10_5Y▷⓽（極端な負でない） | 10年・5年の悲観局面 |
| 5 | **コスト/実装** | Trades/yr（妥当）・実効>3×比率（CFD依存度・課税は全戦略20.315%で共通） | 実運用摩擦 |
| 6 | **有意性（補助）** | bootstrap対base P>0.90 / permutation / DSR・PBO | 平均エッジの統計確証 |

- **強昇格(Active候補)**: ①②③④で baseline以上かつ⑥の一部PASS（②⑥両方が完全PASSなら理想）。
- **条件付き(Shortlist)**: ①②は良いが⑥の bootstrap平均が非有意、等（**頑健性は本物だが平均エッジ未確証**）。
- **棄却**: WFE>2.0 or Regime_min が baseline比大幅悪化 or MaxDDが許容外。
- **「②だけ」「⑥だけ」での採否は禁止**（①〜⑤の多次元で見る）。min と WFA CI95 の主従確定は Phase 4 結果後だが、**どちらか片方＝1次元だけで決めない**のが本ルールの核心。
