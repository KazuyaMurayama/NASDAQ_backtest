...
作成日: 2026-05-21
最終更新日: 2026-06-07 (信号拡張プロジェクト Phase A→D + Sessions 1-5 完了 — DH-W1 baseline の canonical split 9+1 指標を §2 Shortlisted に追補、S3 + nasdaq_mom63 × M6 defensive overlay を §2 新規追加。同日早朝に DH-W1 キャッシュ・スリーブ 1倍投信置換 4戦略 P2/P5/P7 を §2 Shortlisted に追加済み。**同日後半**: S3 overlay tuning sweep (10 variants) から **V7 pure_boost {1.20,1.10,1.00,1.00}** を Shortlisted 追加 — min(CAGR_IS, CAGR_OOS) > 18% target 達成の唯一 "MaxDD 据置" 候補 (V0 defensive と相補)。全 Shortlisted エントリに税後 CAGR (NISA 内非課税 / 課税口座 ×0.8273) 情報を併記) / **2026-06-11 (DH-W1+V7改善検証: §2 に DH-W1 P09_TQQQ 追加 — V7 boost + BondタイミングOUT-fill + NASDAQ脚 CFD→TQQQ是正、⓽min +17.51%。G-gate 3/6: WFA α/β+permutation PASS だが対baseline bootstrap FAIL → Shortlist/Active保留。批判的QC独立4Agent是正済み)**
**2026-06-15 (ベスト戦略確定)**: §1 を **B3a_k365（ベスト戦略・分析確定）** に更新（E4 は §2 fallback に保全・本番稼働継続）。§2 に **P09_C1（ベスト戦略候補・バランス重視）** と **B3c_k365（B3aのDD削減版）** を追加。一次根拠 LEVERUP_SWEEP_RESULTS_20260612.md／QC LEVERUP_QC_SIGNOFF_20260613.md（独立再実装で§6.1値80/79再現）。本番切替は7月判断。
**2026-06-26 (ベスト戦略昇格)**: §1 を **P09_STR_sc1.6（strong-map×scale1.6・CAGR_OOS+26.21%）** に昇格（ユーザー決定）。旧ベスト B3a_k365 は §1 下段の候補①（保守fallback）に保全。一次根拠 SCALE2_PROMOTION_ELIGIBILITY_20260626.md（確定値4桁再現・経路頑健MaxDD・gap純レバ幾何効果）。MaxDD−51.95%・最大実効レバ7.68xのリスク選好判断。本番切替は7月判断。
管理者: 男座員也（Kazuya Oza）
...
> `CURRENT_BEST_STRATEGY.md` §🏆 と同期。差異があれば本ファイルではなく `CURRENT_BEST_STRATEGY.md` を一次根拠とする。
> **⚠ 「ベスト戦略（分析）」≠「本番稼働中」**: 本番Active（スプレッドシート稼働中）は依然 **E4 RegimeKLT**、**P09 は GAS で並走運用中**。P09_STR_sc1.6 はユーザー決定（2026-06-26）による分析上のベスト戦略で、**本番切替（E4→P09 等）は7月上旬の別判断**。E4 の指標行は §2（fallback）に保全。旧ベスト B3a_k365 は候補①として §1 下段に保全。
> 全値⓽税後（×0.8273・特定口座課税。TQQQ/くりっく株365 はNISA対象外）。

| 戦略（ID / 構成） | Date | Cost | CAGR IS / OOS（min）⓽ | gap⓽ | Sharpeⓒ | MaxDDⓒ | Worst10Y★⓽ | P10⓽5Y | Tradeⓞ | WFEⓞ | CI95ⓡ | Status | Decision Reason | Evidence |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **P09_STR_sc1.6** 🏆ベスト戦略<br>DH-W1 + strongマップ<br>{Q0:1.60,Q1:1.50,<br>Q2:1.10,Q3:1.00}×scale1.6<br>+ P09 OUT充填 + C1<br>≤3x=TQQQ / >3x=くりっく株365 | 2026-06-26 | realistic TQQQ+k365（⓽=×0.8273） | +30.26% / **+26.21%**（**min +26.21%**） | +4.90pp | +1.055 | −51.95% | +19.36% | N/A | N/A | 0.898 | **+30.52%** | **🏆 ベスト戦略（分析確定 2026-06-26・CAGR重視・レバ拡張点）。本番切替は7月判断** | 旧ベスト B3a_k365（CAGR_OOS+20.98%）から昇格。CAGR_OOS +26.21%（+5.23pp）を取得する対価に MaxDD−38.2→−51.95%・Regime−2.88→−7.71%・最大実効レバ≤4.83→7.68x を引き受けるリスク選好判断（ユーザー決定）。独立QCで確定値を4桁再現、IS-OOS gap+4.90ppは純レバの幾何効果（暴落/OUT年2001/2002/2008/2022はscale不変・強気年だけ振幅拡大）と判明。MaxDD−51.95%は経路集約指標(TUW93.4%/avgDD−14.59%/CVaR5%DD−39.54%)でも頑健に深化（過小評価なし・block=21未使用 R-STAT-1）。WFA正典49窓α/β PASS(CI95_lo+30.52%/WFE0.898)。Sharpeは純レバアップで非改善。scale1.4(MaxDD−46.48%)とscale2.0(CAGR+29.11%/MaxDD−61.63%)の中間点。 | [SCALE2_PROMOTION_ELIGIBILITY_20260626.md](SCALE2_PROMOTION_ELIGIBILITY_20260626.md), [P09_STR_SCALE_DIAL_20260623.md](P09_STR_SCALE_DIAL_20260623.md), [src/audit/scale2_promotion_qc_20260626.py](src/audit/scale2_promotion_qc_20260626.py), [src/audit/p09_strongmap_scale_dial_20260623.py](src/audit/p09_strongmap_scale_dial_20260623.py), [CURRENT_BEST_STRATEGY.md](CURRENT_BEST_STRATEGY.md) |
| **B3a_k365** 候補①（旧ベスト・保守fallback）<br>DH-W1 + V7マップ<br>{Q0:1.40,Q1:1.40,<br>Q2:1.05,Q3:1.00}×uniform1.15<br>+ P09 OUT充填 + C1<br>≤3x=TQQQ / >3x=くりっく株365 | 2026-06-15 | realistic TQQQ+k365（⓽=×0.8273） | +23.10% / **+20.98%**（**min +20.98%**、選択バイアス割引後≈20.1%） | +2.57pp | 0.904 | −38.20% | +14.53% | +8.08% | 33.3 | ✅ 0.987 | **+22.52%** | **候補①（旧ベスト 2026-06-15〜2026-06-26）。MaxDD/WFE/Regimeで sc1.6 を上回る保守fallback** | 旧ベスト（CAGR重視）。MaxDD−38.2%（sc1.6比+13.75pp浅）・WFE0.987（sc1.6比+0.089良）・Regime_min−2.88%（sc1.6比+4.83pp良）でリスク効率・保守性が sc1.6 を上回る。sc1.6 切替を見送る/巻き戻す際の即時fallback。全ゲートPASS（ハードベト無・WFA正典49窓 α/β・CPCV p10+16.01%・multi-metric bootstrap）。 | [LEVERUP_SWEEP_RESULTS_20260612.md](LEVERUP_SWEEP_RESULTS_20260612.md), [LEVERUP_QC_SIGNOFF_20260613.md](LEVERUP_QC_SIGNOFF_20260613.md), [src/audit/leverup_sweep_20260612.py](src/audit/leverup_sweep_20260612.py), [src/audit/k365_recost_20260612.py](src/audit/k365_recost_20260612.py), [CURRENT_BEST_STRATEGY.md](CURRENT_BEST_STRATEGY.md) |
...
| **DH-W1** ⭐v4.5<br>ETF Active候補<br>Asymm Hyst<br>Enter≥0.7, Exit≤0.3<br>DH_Dyn+VR+Hyst | 2026-06-07 | D（ⓒ 税引前）<br>⓽→ aftertax_cagr_canonical 参照 | ⓒIS +18.10% / ⓒOOS +18.88%（**min +18.10%ⓒ**）<br>⓽(canonical split, ETF課税): IS **+15.31%** / OOS **+15.74%** / **min +15.31%**<br>(NISA非課税は pretax と同値: OOS +18.88%) | −0.78pp(ⓒ) | 0.845 | −34.57% | +10.37% | +4.82% | 68.7 | ✅ 1.023 | +13.61% | **Shortlisted → ETF 環境 Active 候補 (v4.5 §7-2 min ルール下、ETF 制約下で最良)** | DH-W1 は ETF only 環境 (CFD 不可、NISA 等) で DH (min +9.56%) を +4.10pp 上回る唯一の改善案。CFD 環境では vz065_l7_F10eps015 (min +20.23%) に大きく劣後 (-6.57pp)。WFA 50窓 PASS (CI95_lo=+13.95%, **WFE=0.997 完全汎化**, p=0.0000, Trades 17.8/yr)。Bootstrap (W1 vs DH-REF) median +3.65pp positive, P(diff>0)=72%。Threshold (enter, exit) 24 組合せ sweep 全て OOS +12〜16.7% でロバスト。**ETF 制約環境の第一推奨**。canonical split (IS_END=2021-05-07): CAGR_IS=+18.10%, CAGR_OOS=+18.88%, Sharpe_OOS=0.845, MaxDD=-34.57%, Worst10Y★=+10.37%, P10_5Y▷=+4.82%, Trades=68.7/yr(NAV proxy), IS-OOS gap=-0.78pp, WFA_CI95_lo=+13.61%, WFE(50w Sharpe)=1.023。 | [STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md](STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md) §0'/§6-2.5/§6-3/§7-2 (v4.3/v4.5), [STRATEGY_DH_REFINEMENT_20260603.md](STRATEGY_DH_REFINEMENT_20260603.md), [SIGNAL_EXPANSION_FINAL_DECISION_20260607.md](SIGNAL_EXPANSION_FINAL_DECISION_20260607.md) §3.1/§3.6 (canonical), [src/g23a_dh_refinement_variants.py](src/g23a_dh_refinement_variants.py), [g23d_dh_w1_wfa.csv](g23d_dh_w1_wfa.csv), [g23e_dh_w1_bootstrap_results.csv](g23e_dh_w1_bootstrap_results.csv), [g23f_dh_w1_threshold_sweep.csv](g23f_dh_w1_threshold_sweep.csv), [data/signals/expansion/decision_9metrics_20260607.csv](data/signals/expansion/decision_9metrics_20260607.csv), [data/signals/expansion/aftertax_cagr_canonical_20260607.csv](data/signals/expansion/aftertax_cagr_canonical_20260607.csv) |
| **DH-W1+mom63 V0** ⭐NEW<br>nasdaq_mom63 overlay<br>M6 def / Q4 mult<br>{1.1,1.0,0.9,0.8}<br>**defensive (MaxDD改善優先)**<br>DH_Dyn+Signal | 2026-06-07 | D（ⓒ 税引前）<br>⓽→ aftertax_cagr_canonical 参照 | ⓒIS +16.69% / ⓒOOS +18.03%（**min +16.69%ⓒ**）<br>⓽(canonical split, ETF課税): IS **+14.07%** / OOS **+15.02%** / **min +14.07%**<br>(NISA非課税は pretax と同値: OOS +18.03%) | −1.34pp(ⓒ) | **0.892** | **−28.74%** | +10.75% | +5.21% | N/A | ✅ 1.005 | +13.00% | **Shortlisted (ETF 環境 overlay 採用可、Phase D Hard Gate 4/4 全 PASS、S3 限定)** | 信号拡張プロジェクト (Phase A→D + Sessions 1-5, 76 信号 × 306+ patterns 検証) の **唯一の正式 ADOPT 候補**。DH-W1 baseline 比で **MaxDD -34.57% → -28.74% (+5.83pp 改善)** が headline。Sharpe_OOS +0.845→+0.892 (+0.047 改善)、Worst10Y★ +10.37%→+10.75%、P10_5Y▷ +4.82%→+5.21%。CAGR_OOS は -0.85pp の trade-off (+18.88%→+18.03%)。Phase D Hard Gate: WFE(50w Sharpe)=1.005 PASS, CI95_lo=+13.00% PASS, **Bootstrap P(Sharpe>base)=0.930 PASS, P(MaxDD better)=0.988 PASS** ⭐。Session 5 cross-strategy transfer test で S2(D5) WFE=0.963, E4(Active) WFE=0.958 と FAIL → **S3 (ETF only) 限定**で運用、CFD 系には転用不可。**ETF only 環境 (NISA 等) ユーザーの第一推奨 overlay**。t_p/permutation 未実施のため §1 Active 昇格は保留、ユーザー判断で採用可。税後 CAGR: NISA 内非課税は pretax と同値、課税口座は yearly ×0.8273 を canonical split で適用。 | [SIGNAL_EXPANSION_FINAL_DECISION_20260607.md](SIGNAL_EXPANSION_FINAL_DECISION_20260607.md) §1.2/§3.1/§3.2/§3.6 (canonical), [LESSONS_LEARNED_20260607.md](LESSONS_LEARNED_20260607.md), [data/signals/expansion/phase_d_audit_nasdaq_mom63_S3_M6_def_20260605.md](data/signals/expansion/phase_d_audit_nasdaq_mom63_S3_M6_def_20260605.md), [data/signals/expansion/decision_9metrics_20260607.csv](data/signals/expansion/decision_9metrics_20260607.csv), [data/signals/expansion/session5_transfer_report_20260605.md](data/signals/expansion/session5_transfer_report_20260605.md), [data/signals/expansion/aftertax_cagr_canonical_20260607.csv](data/signals/expansion/aftertax_cagr_canonical_20260607.csv), [scripts/compute_aftertax_cagr_v3_20260607.py](scripts/compute_aftertax_cagr_v3_20260607.py), [src/integration/build_strategy_with_signal.py](src/integration/build_strategy_with_signal.py) |
| **DH-W1+mom63 V7** ⭐NEW (2026-06-07)<br>nasdaq_mom63 overlay<br>M6 def / Q4 mult<br>{1.20,1.10,1.00,1.00}<br>**pure_boost (CAGR死守 / MaxDD baseline据置)**<br>DH_Dyn+Signal | 2026-06-07 | D（ⓒ 税引前）<br>⓽→ aftertax_cagr_canonical 参照 | ⓒIS +18.61% / ⓒOOS +19.14%（**min +18.61%ⓒ**）<br>⓽(canonical split, ETF課税): IS **+15.74%** / OOS **+15.96%** / **min +15.74%**<br>(NISA非課税は pretax と同値: OOS +19.14%) | **−0.53pp**(ⓒ) | 0.841 | −34.57% | +11.02% | +5.22% | 26.5 | ✅ 1.029 | +14.06% | **Shortlisted (ETF 環境 overlay、CAGR 死守シナリオ候補、Phase D Bootstrap 未実施)** | V0 (defensive) と相補的な mapping variant。S3 + nasdaq_mom63 × M6 で q3 cut を 0.80 → 1.00 に緩和、q0 boost を 1.10 → 1.20 に強化することで「下位 quartile では brush up、上位 quartile では baseline 据置」の boost-only 構成。**S3 overlay tuning 10 variants 中、min(CAGR_IS, CAGR_OOS) > 18% target を達成する唯一の "MaxDD 据置" 案**。canonical split (IS_END=2021-05-07): CAGR_IS=+18.61%, CAGR_OOS=+19.14%, Sharpe_OOS=0.841, MaxDD=−34.57% (DH-W1 baseline と同値、改善なし), Worst10Y★=+11.02%, P10_5Y▷=+5.22%, IS-OOS gap=−0.53pp (全 overlay 候補中最良), Trades/yr=26.5 (NAV proxy), WFA_CI95_lo=+14.06% (annual CAGR), WFE(50w Sharpe)=1.029。V0 比: CAGR 攻撃力 +1.11〜1.92pp 強い / IS-OOS gap +0.81pp 改善 / 一方 MaxDD 改善は失われる (V0 −28.74% / V7 −34.57%)。**Phase D Bootstrap audit (P_MaxDD, P_Sharpe, P_CAGR) 未実施**のため正式 ADOPT 化保留、ユーザー判断で V0 (MaxDD改善) と V7 (CAGR死守) を運用方針に合わせ選択可。税後 CAGR: NISA 内非課税は pretax と同値、課税口座は yearly ×0.8273 を canonical split で適用。 | [S3_OVERLAY_TUNING_REPORT_20260607.md](S3_OVERLAY_TUNING_REPORT_20260607.md) §6.2/§11 (canonical aftertax), [data/signals/expansion/s3_overlay_tuning_20260607.csv](data/signals/expansion/s3_overlay_tuning_20260607.csv), [data/signals/expansion/aftertax_cagr_canonical_20260607.csv](data/signals/expansion/aftertax_cagr_canonical_20260607.csv), [SIGNAL_EXPANSION_FINAL_DECISION_20260607.md](SIGNAL_EXPANSION_FINAL_DECISION_20260607.md) §3.6 (税後 CAGR canonical), [scripts/compute_aftertax_cagr_v3_20260607.py](scripts/compute_aftertax_cagr_v3_20260607.py), [scripts/tune_s3_overlay_20260607.py](scripts/tune_s3_overlay_20260607.py), [src/integration/build_strategy_with_signal.py](src/integration/build_strategy_with_signal.py) |
| **DH-W1 P09_TQQQ** ⭐NEW (2026-06-11)<br>V7 boost + Bondタイミング<br>OUT-fill(Gold常時+<br>Bond@mom252>0,逆ボラW63)<br>NASDAQ脚=TQQQ-cost<br>DH_W1+Signal+CashSleeve | 2026-06-11 | realistic TQQQ（ⓒ税前 / ⓽=×0.8273） | ⓒIS +22.77% / ⓒOOS +21.17%（**min +21.17%ⓒ**）<br>⓽(canonical, 特定口座20.315%課税): IS **+18.84%** / OOS **+17.51%** / **min +17.51%**<br>(TQQQはレバ型でNISA対象外＝⓽が手取り) | +1.60pp(ⓒ) | 0.901 | −35.18% | +13.83% | +7.93% | 29.2 | ✅ 1.017 | +17.94% | **🟢 Active候補 (2026-06-11昇格・ユーザー承認)。拡張評価6次元スコアカード総合上位(8.12/10、baseline V7=6.93)。WFA α/β+permutation+CPCV p10(+13.6%)+Regime_min(−1.6%)+5y/10yテール優位。対baseline平均bootstrapは非有意だが頑健性で採用** | DH-W1+V7 の OUTキャッシュ期(47%)を **Bondタイミング条件付き1x充填**(Gold常時+Bond@`bond_mom252>0`, 逆ボラW63, T+5ラグ)で運用 ＋ NASDAQ脚を **CFD→TQQQコスト是正**。⓽min +17.51% は ETF環境の従来候補 DH-W1(+15.85% TQQQ是正後)/V7(+16.27%) を上回る。**G-gate 3/6**: WFA α(CI95_lo +17.94%, t_p≈0) PASS / WFA β(WFE 1.017) PASS / permutation(OUT-fill mask block-shuffle, p≈0) PASS = **単体戦略として頑健・過学習なし・充填は本物のシグナル**。一方 **対 TQQQ-V7 paired bootstrap は3指標とも FAIL**(P_min 0.80 / P_Sharpe 0.59 / P_MaxDD 0.60, CI95_lo 全て負) = 「baseline超過」増分は統計的未確定。MaxDD −35.18%(baseline比 −0.71pp悪化)・最悪暦年 2015 −18.73%(gold+bond同時安)。lever-up変種 LU1(⓽min +18.12%)はレバ増のaggressive variant(>3×はCFD/信用要・コスト微増)。**Active昇格は保留**(bootstrap改善 or 絶対WFA+permutation基準での昇格はユーザー判断)。批判的QC(独立4Agent)是正済み。 | [DHW1_V7_IMPROVEMENT_BACKTEST_20260611.md](DHW1_V7_IMPROVEMENT_BACKTEST_20260611.md), [P09_TQQQ_REGISTRY_DRAFT_20260611.md](audit_results/P09_TQQQ_REGISTRY_DRAFT_20260611.md), [p09_tqqq_ggate_20260611.csv](audit_results/p09_tqqq_ggate_20260611.csv), [src/audit/run_p09_tqqq_validate_20260611.py](src/audit/run_p09_tqqq_validate_20260611.py), [tqqq_correction_etf_strategies_20260611.csv](audit_results/tqqq_correction_etf_strategies_20260611.csv) |
| **P09_C1** 🥈ベスト戦略候補<br>V7-TQQQ + P09 OUT充填 + C1<br>デフォルトmom63マップ<br>{Q0:1.20,Q1:1.10,1.00,1.00}<br>scale 1.0・>3x超過課金なし<br>DH_W1+Signal+CashSleeve | 2026-06-15 | realistic TQQQ（⓽=×0.8273） | +19.88% / **+17.77%**（**min +17.77%**） | +2.56pp | **0.912** | −34.99% | +11.49% | +7.02% | 29.2 | ✅ 0.989 | +18.96% | **🥈 ベスト戦略候補（2026-06-15・バランス/risk-adjusted重視）** | P09_TQQQ に C1（OUT∧bondOFF日に現金利回りSOFR計上＝+0.25pp）を加えた版。**6次元バランス採点で首位8.50**・CFD依存最小（>3x日≈6.1%・超過課金なし）・**2008防御+19.86%（全候補最強）**・Regime_min−0.08%（最良）・worst暦年−18.7%（最小）。min⓽は20%未達だが Sharpe(risk-adjusted)はB3aと同等で純レバアップでないぶん頑健。全ゲートPASS。**B3aのSharpeは対P09中立**＝B3aはCAGRもリスクも比例増。本番P09はGAS並走中・E4→P09切替7月判断。 | [LEVERUP_SWEEP_RESULTS_20260612.md](LEVERUP_SWEEP_RESULTS_20260612.md), [LEVERUP_QC_SIGNOFF_20260613.md](LEVERUP_QC_SIGNOFF_20260613.md), [src/audit/leverup_b1c1_20260612.py](src/audit/leverup_b1c1_20260612.py), [src/audit/multimetric_bootstrap_20260615.py](src/audit/multimetric_bootstrap_20260615.py), [CURRENT_BEST_STRATEGY.md](CURRENT_BEST_STRATEGY.md) |
| **B3c_k365**<br>B3aと同一マップ<br>×uniform **1.10**（B3aは1.15）<br>+ P09充填 + C1<br>≤3x=TQQQ / >3x=くりっく株365 | 2026-06-15 | realistic TQQQ+k365（⓽=×0.8273） | +22.38% / **+20.41%**（**min +20.41%**） | +2.38pp | 0.911 | −37.07% | +14.07% | +7.94% | 33.3 | ✅ **0.995** | +21.75% | **Shortlisted（B3aのDD約1pp削減版・CAGR重視の保守代替）** | B3a_k365 とマップ同一、uniform leverage のみ 1.15→1.10。min⓽ −0.57pp（+20.41%）の代わりに MaxDD −37.07%（B3a比+1.1pp浅）・**WFE 0.995（全候補最良＝過学習兆候最小）**。「CAGRはほぼ維持しDDを少し削りたい」場合の代替。全ゲートPASS。 | [LEVERUP_SWEEP_RESULTS_20260612.md](LEVERUP_SWEEP_RESULTS_20260612.md), [src/audit/leverup_sweep_20260612.py](src/audit/leverup_sweep_20260612.py), [src/audit/k365_recost_20260612.py](src/audit/k365_recost_20260612.py) |
| **B3a レバ拡張フロンティア**<br>scale 1.20〜1.35<br>(±強map/early re-entry)<br>＝B3aの高CAGR側ダイヤル | 2026-06-16 | realistic TQQQ+k365（⓽=×0.8273） | 〜+26.8% / 〜**+23.97%**（scale/enterで連続） | +2.8〜3.6pp | 0.88〜0.95 | −40〜−48% | +15〜17% | 〜+9% | 33〜36 | ✅ 0.92〜0.95 | 〜+26.6% | **Shortlisted（CAGR最大化ダイヤル・リスク選好で点を選ぶ）** | B3aの uniform leverage を 1.15超へ拡張すると min⓽ が連続的に上昇（1.25→+22.1% / 1.35→+23.8% / combo e0.60×sc1.25→**+23.97%上端**）。**ベト−50%到達は概算 scale≈1.50**でフロンティアに余地あり。**ただし純レバアップ＝Sharpe非改善・MaxDD比例悪化・min改善は95%非有意（bootstrap CI95_lo負）・worst年−26%**。推奨: バランス=B3a維持／やや攻め=scale1.25(MaxDD−40%)／攻め=scale1.35強map(MaxDD−45%・CI95_lo+26.6%)。**QC v2（独立4エージェント・2026-06-16）**: 数値正・**scale1.25/1.35のCAGR優位はOOS境界(2019-2023)に頑健**（単一強気OOS依存を否定）。ただし純レバ＝Sharpe非改善・95%非有意・bear比例超悪化(−5.40%)・**Worst5Yはscale1.25/comboでマイナス転落**。**combo/enter0.60はOOS境界脆弱＋実運用MaxDDベト超過リスクで非推奨**。実運用MaxDD/追証/容量/累積多重検定は未モデル(限界)。詳細=[LEVERUP_EXTENSION_RESULTS_20260616.md]／QC=[LEVERUP_EXTENSION_QC_SIGNOFF_20260616.md]。 | [LEVERUP_EXTENSION_RESULTS_20260616.md](LEVERUP_EXTENSION_RESULTS_20260616.md), [src/audit/leverext_scale_20260616.py](src/audit/leverext_scale_20260616.py), [src/audit/leverext_reentry_20260616.py](src/audit/leverext_reentry_20260616.py), [src/audit/leverext_combo_20260616.py](src/audit/leverext_combo_20260616.py) |
| ~~**B3a+G5_vix_hard**~~<br>B3a + vix_mom21 防御<br>オーバーレイ{Q2:0.92,Q3:0.85}<br>→**QCで非推奨**<br>DH_W1+Signal+DefOverlay | 2026-06-16 | realistic TQQQ+k365（⓽=×0.8273） | +22.7% / +20.73%（min +20.73%） | +2.0pp | 0.928 | −35.93% | +13.74% | +8.26% | 52.9 | ✅ 0.990 | +21.54% | **Rejected（探索済・一律デレバに劣後・非推奨, QC 2026-06-16）** | 当初「リスク低減オプション」として登録したが、**3エージェントQCで撤回**。MaxDD改善+2.27ppの**66%は単なる平均デレバ（vix無関係の一律スケールで+1.50pp出る）、vix時機固有+0.77ppはシャッフルプラセボ p=0.40で非有意**。数値・コードは正しく漏れも実害なし（独立再現一致）だが、**同じMaxDD低減を B3c_k365（uniform 1.10）が低取引(33 vs 53/yr)・高採点(BAL 8.361 vs 8.13〜8.29)で達成**。リスク低減は exotic オーバーレイでなくレバ水準（B3c）で行うのが正解。取引削減版 G5q_2quant も B3c に劣後。**MaxDD縮小には B3c を使うこと**。 | [MULTISTRATEGY_COMBINE_QC_SIGNOFF_20260616.md](MULTISTRATEGY_COMBINE_QC_SIGNOFF_20260616.md), [MULTISTRATEGY_COMBINE_RESULTS_20260615.md](MULTISTRATEGY_COMBINE_RESULTS_20260615.md), [src/audit/placebo_audit_g5_20260616.py](src/audit/placebo_audit_g5_20260616.py) |
| **P09 レバ拡張フロンティア**<br>P09_C1 strong-map<br>{Q0:1.60,Q1:1.50,Q2:1.10,Q3:1.00}<br>×scale {1.4,1.6,1.8,2.0}<br>+ P09充填 + C1<br>≤3x=TQQQ / >3x=くりっく株365<br>DH_W1+Signal+CashSleeve | 2026-06-21 | realistic TQQQ+k365（⓽=×0.8273） | scale別 OOS: 1.4 **+24.34%** / 1.6 +26.21% / 1.8 +27.81% / 2.0 +29.11%（**min(1.4) +24.34%**・上端2.0で+29.11%） | +3.80pp(1.4)〜+7.57pp(2.0)（高scaleで>5pp＝過学習） | 1.074(1.4)〜1.028(2.0) | −46.48%(1.4) 〜 −61.63%(2.0) | +17.87%(1.4)〜+20.84%(2.0) | +9.05%(1.4)〜+9.38%(2.0) | 35.2 | ✅ 0.936(1.4)〜0.818(2.0) | +27.35%(1.4)〜+36.80%(2.0) | **Shortlisted → 前向きに採用検討中（2026-06-21・ユーザー方針確定）。CAGR最大化ダイヤル・スケール点はリスク選好で選ぶ** | P09_C1(scale 1.0, min⓽+17.77%)の**高CAGR側レバ拡張ダイヤル**。strong-map×scaleで CAGR_OOS を +24〜29% へ延長。**ユーザー方針(2026-06-21): P09×scaleを前向きに採用検討（B1×scaleは被支配で不採用、§3.4）**。ただし**純レバアップ**＝Sharpe非改善(1.07→1.03で漸減)・MaxDDは scale比例で悪化（1.6で既に−51.95%＝ベト−50%超）・**高scale(1.8/2.0)はIS-OOSギャップ>5pp で過学習フラグ・regime_min(bear) −9.6%(1.8)/−11.4%(2.0)でハードベト接近**。**−50%以内に収めるなら実質 scale≈1.4が上限**（1.4: MaxDD−46.48%・regime−5.86%・WFE0.936・gap想定3pp台で過学習なし＝最も信頼できる拡張点）。実運用MaxDD/追証/容量/累積多重検定は未モデル(限界)。全scale点の WFA/bootstrap/CPCV は前回 LEVERUP_SCALE_FRONTIER で実施済（Stage-1）。本番採用は B3a/P09_C1 等との最終比較とユーザー承認待ち。 | [LEVERUP_SCALE_FRONTIER_20260619.md](LEVERUP_SCALE_FRONTIER_20260619.md), [B1_SCALE_FRONTIER_20260621.md](B1_SCALE_FRONTIER_20260621.md), [src/audit/leverext_high_stage1_20260618.py](src/audit/leverext_high_stage1_20260618.py), [src/audit/leverext_high_sc180_200_20260620.py](src/audit/leverext_high_sc180_200_20260620.py), [audit_results/leverext_high_stage1_20260618.csv](audit_results/leverext_high_stage1_20260618.csv), [audit_results/leverext_high_sc180_200_20260620.csv](audit_results/leverext_high_sc180_200_20260620.csv) |
...
| **B1×Scale 1.4–2.0** ❌<br>B1下方偏差ブレーキ×レバ拡張<br>default/strong map ×{1.4,1.6,1.8,2.0}<br>P09_C1+IN脚DDブレーキ+scale<br>DH_W1+Signal+CashSleeve | 2026-06-21 | realistic TQQQ+k365（⓽=×0.8273） | B1_STR別 OOS: 1.4 +20.75% / 1.6 +22.30% / 1.8 +23.24% / 2.0 +24.07%（min(1.4) +20.75%） | 3.9〜5.7pp(高scaleで>5pp) | 1.085(1.4)〜1.086(2.0) | −39.19%(1.4)〜−47.08%(2.0) | +13.10%(1.4)〜+13.08%(2.0) | +8.15%(1.4)〜+9.20%(2.0) | 35.2 | ✅ 0.87(1.4)〜0.78(2.0) | +23.8%(1.4)〜+29.1%(2.0) | **Rejected（探索済・P09×scaleに被支配・不採用, ユーザー方針 2026-06-21）** | B1(P09_C1+IN脚下方偏差ブレーキ)にレバ拡張を掛け「高CAGR×DD抑制両立」を狙ったが、**同MaxDD帯で P09×低スケールが両軸(CAGR・MaxDD)で同等以上**＝被支配。**B1×2.0(+24.07%/−47.08%) ≈ P09×1.4(+24.34%/−46.48%)**で、ブレーキは実効的に「低スケール化装置」。同map同scale比較ではブレーキはMaxDDを+7〜15pp浅くするが CAGRを −3.6〜−5.0pp 払う＝**フリーランチでなくトレードオフ、かつ P09低スケールが上**。ブレーキの時機効果は実在（等fbar一律デレバ双子比 MaxDD +2〜6pp）だが危機内でなく危機外IN脚DDの平滑化（主要暴落はDH-W1 OUT-fillが既に処理）。**高scale(1.8/2.0)はIS-OOSギャップ>5ppで過学習フラグ**＝最も信頼できない比較点。**結論: レバ拡張は B1 でなく P09 で実施すること（§2 P09レバ拡張フロンティア参照）**。関連: A7 IN脚ボラブレーキ＋A0配分A1-A6も全て「時機でなくデレバ・ノイズ」で不採用（DD削減はレバ水準ダイヤル＝B3c/P09スケールが本質）。 | [B1_SCALE_FRONTIER_20260621.md](B1_SCALE_FRONTIER_20260621.md), [A7_DD_REDUCTION_VARIATIONS_20260621.md](A7_DD_REDUCTION_VARIATIONS_20260621.md), [P09C1_ALLOCATION_VARIATIONS_20260620.md](P09C1_ALLOCATION_VARIATIONS_20260620.md), [src/audit/b1_scale_stage1_20260621.py](src/audit/b1_scale_stage1_20260621.py), [audit_results/b1_scale_stage1_20260621.csv](audit_results/b1_scale_stage1_20260621.csv), [audit_results/b1_scale_delever_control_20260621.csv](audit_results/b1_scale_delever_control_20260621.csv) |
...
### LeverageScale / レバ拡張フロンティア（uniform leverage を上げる CAGR最大化ダイヤル）
- **Shortlisted**: B3a_レバ拡張フロンティア（scale 1.20〜1.35, B3aマップ）, **P09_レバ拡張フロンティア（scale 1.4〜2.0, strong-map, 2026-06-21 前向きに採用検討）** → §2
- **Rejected (2026-06-21)**: **B1×Scale 1.4–2.0**（B1下方偏差ブレーキ×レバ拡張・P09×scaleに被支配＝B1×2.0≈P09×1.4）→ §3.4
- 結論: **risk-return はレバ水準（uniform leverage / scale）で制御するのが本質**。CAGR最大化は scale を上げる純レバアップで達成（Sharpe非改善・MaxDD/bear比例悪化・高scaleで過学習）。**DD削減も exotic ブレーキ（B1/A7/A0配分）でなくレバ水準を下げるのが正解**（B3c uniform1.10 / P09低スケール）。レバ拡張は **B1 でなく P09/B3a で実施**。一次根拠: [LEVERUP_SCALE_FRONTIER_20260619.md](LEVERUP_SCALE_FRONTIER_20260619.md), [LEVERUP_EXTENSION_RESULTS_20260616.md](LEVERUP_EXTENSION_RESULTS_20260616.md), [B1_SCALE_FRONTIER_20260621.md](B1_SCALE_FRONTIER_20260621.md)。

### DDReductionBrake / 配分バリエーション（2026-06-20〜21・全て不採用）
- **Rejected**: A0配分A1-A6（OUTスリーブ配分・全bootstrap非有意=ノイズ）, A7 IN脚ボラブレーキ（時機でなくデレバ・危機はOUT-fill処理済）, B1×Scale（§LeverageScale参照） → §3.4
- 結論: **シグナル→アクション（配分・ブレーキ）の小細工は DD削減に対し一律デレバを統計的に上回らない**。block=21 MaxDDブートストラップは経路依存極値に無効と判明し経路頑健な暴落窓検定で再確認（全 TIMING_WEAK）。**DD削減はレバ水準ダイヤル**（§LeverageScale）。一次根拠: [P09C1_ALLOCATION_VARIATIONS_20260620.md](P09C1_ALLOCATION_VARIATIONS_20260620.md), [A7_DD_REDUCTION_VARIATIONS_20260621.md](A7_DD_REDUCTION_VARIATIONS_20260621.md)。
