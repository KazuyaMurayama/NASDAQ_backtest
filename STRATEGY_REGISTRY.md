...
| **vz065_l7_F10eps015** ⭐v4.5 | S2_VZGated + LT2-N750 + E4 (vz_thr=0.65, l_max=7.0) + F10 ε=0.015 — 報告書 v2 で「vz=0.65+l7+F10ε (REF)」と表記 | CFDLeverage + LongCycleSignal + VolRegime + RegimeCapBullTilt + EpsilonDeadband | 2026-06-05 | D (moderate 0.05%) | +21.49% | 0.829 | −65.95% | **Shortlisted → CFD 環境 Active 候補 (v4.5 §7-2 min ルール下で 5 戦略中 1 位、攻め型)** | v4.5 で保守的採用基準 (min(IS, OOS) CAGR + Worst10Y + P10_5Y) を導入した結果、本戦略が **全 3 軸で 1 位**: min CAGR=+20.23% (IS+20.23%, OOS+21.49%)、Worst10Y +9.96%、P10_5Y +4.05%。WFE 1.369 (range 内)、CI95_lo +19.85% (PASS)。OOS 累積 ×3.38 (5 戦略中最高)。**CFD 利用可投資家への攻め推奨**。Active 正式昇格 (CURRENT_BEST_STRATEGY 更新) はユーザー判断。 | [STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md](STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md) §0'/§7-2 (v4.5), [src/g26_new_asymmetric_variants.py](src/g26_new_asymmetric_variants.py) (REF builder), [g24_5strategies_verification.csv](g24_5strategies_verification.csv) |
| **vz065_l5p5_F10eps015** ⭐v4.6 | S2_VZGated + LT2-N750 + E4 (vz_thr=0.65, **l_max=5.5**) + F10 ε=0.015 | CFDLeverage + LongCycleSignal + VolRegime + RegimeCapBullTilt + EpsilonDeadband | 2026-06-05 | D (moderate 0.05%) | +19.65% | 0.832 | −59.87% | **Shortlisted → CFD 環境 Active 候補 (v4.6 バランス推奨)** | v4.6 §6-5 lmax sweep で「攻めと守りのバランス最良」と判定。**REF (l7) 比で**: min CAGR -0.58pp (許容) / Worst10Y +2.50pp 改善 / P10_5Y +3.87pp 改善 / MaxDD -6.08pp 浅化 / gap +0.73pp (最小、過学習なし) / WFE 1.373 (REF 同水準) / CI95_lo +19.90% (REF +19.85% 同水準)。Trades 91 で REF 105 より 13% 低コスト。**「攻めは多少抑えてでも守りを強化したい CFD ユーザー」への第一推奨**。 | [STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md](STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md) §6-5 (v4.6), [src/g27_vz065_lmax_sweep.py](src/g27_vz065_lmax_sweep.py), [g27_vz065_lmax_sweep_metrics.csv](g27_vz065_lmax_sweep_metrics.csv) |
| **vz065_l5_F10eps015** ⭐v4.6 | S2_VZGated + LT2-N750 + E4 (vz_thr=0.65, **l_max=5.0**) + F10 ε=0.015 | CFDLeverage + LongCycleSignal + VolRegime + RegimeCapBullTilt + EpsilonDeadband | 2026-06-05 | D (moderate 0.05%) | +18.93% | 0.841 | −56.72% | **Shortlisted → CFD 環境 Active 候補 (v4.6 防御最優先)** | v4.6 §6-5 lmax sweep で防御指標 (MaxDD/Worst10Y/P10_5Y) すべて最良の保守型候補。**REF (l7) 比で**: min CAGR -1.30pp / Worst10Y +2.71pp 改善 / **P10_5Y +4.70pp 大幅改善** / MaxDD -9.23pp 浅化 / Sharpe +0.012 改善 / Trades 86 (REF 比 18% 低コスト)。gap +1.23pp で過学習なし、WFE 1.389。**「bear regime 防御を最優先する CFD ユーザー」への推奨**。 | [STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md](STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md) §6-5 (v4.6), [src/g27_vz065_lmax_sweep.py](src/g27_vz065_lmax_sweep.py) |
