...
| **DH_T4_Improved** | DH Dyn 2x3x [A] + vz=0.65 gate + lmax=5.5 cap + F10 ε=0.015（商品は TQQQ+TMF+2036 維持、タイミングのみ NEW CANDIDATE/D5 から移植） | DH_Dyn + VolRegime + RegimeCapBullTilt + EpsilonDeadband | 2026-06-03 | D (moderate 0.10%) | +9.44% | 0.671 | −37.63% | Shortlisted (要追加検証 / Active 昇格は v8 以降) | DH 過学習 (gap +10.46pp→+6.81pp, -3.65pp) と WFE (0.662→0.920) を改善。WFA 50窓 PASS (CI95_lo=+14.24%, WFE=0.920, p=0.0000)。Bootstrap (T4 vs REF) は CI95=[-6.33, +5.88pp] で OOS 6 年では noise 内識別不可。Permutation marginal (実 gap NULL 下位 7.0%)。OOS 2021 単年 -11.03pp 集中 (40.4%)、6 年 sum -2.74pp で OOS 絶対値は REF と同等。MaxDD -37.63% / P10_5Y +9.80% は新最良。Worst10Y +9.37% は REF +12.57% に劣後 → 防御性能 trade-off。商品変更なしの「過学習抑制バリアント」として併記。 | [STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md](STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md) §0'/§5/§6/§6-2 §9-v3, [src/g18_daily_trade_cost_wfa.py](src/g18_daily_trade_cost_wfa.py), [src/g21a_dh_improved_variants.py](src/g21a_dh_improved_variants.py), [g21b_dh_improved_9metrics.csv](g21b_dh_improved_9metrics.csv), [g21c_dh_improved_wfa.csv](g21c_dh_improved_wfa.csv), [g21d_dh_bootstrap_oos_results.csv](g21d_dh_bootstrap_oos_results.csv), [g21e_dh_permutation_summary.csv](g21e_dh_permutation_summary.csv), [g21f_dh_t4_yearly_returns_aftertax.csv](g21f_dh_t4_yearly_returns_aftertax.csv) |
...
- 2026-06-03: §2 Shortlisted に **DH_T4_Improved** を追加（商品は TQQQ+TMF+2036 維持、NEW CANDIDATE/D5 の vz=0.65+lmax=5.5+F10ε タイミングを移植）。DH の IS-OOS gap +10.46pp→+6.81pp (-3.65pp)、WFE 0.662→0.920 で過学習抑制成功。MaxDD -37.63%・P10_5Y +9.80% は新最良。一方 OOS Bootstrap は noise 内 / Permutation marginal / Worst10Y は REF に劣後で Active 昇格は要追加検証。STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md (v3) の §0'/§5/§6/§6-2 4箇所に DH の 1 つ手前で追加。
