...
作成日: 2026-05-21
最終更新日: 2026-06-07 (DH-W1 キャッシュ・スリーブ 1倍投信置換 4戦略 P2/P5/P7 を §2 Shortlisted「投信環境 Active 候補」に追加・WFA 50窓 PASS)
管理者: 男座員也（Kazuya Oza）
...
| **DH_W1_AsymmHyst** ⭐v4.5 | DH Dyn 2x3x [A] + Asymmetric Hysteresis (Enter `lev_mod_065 ≥ 0.7`, Exit `≤ 0.3`、binary HOLD/OUT) + DH base 配分。商品 TQQQ+TMF+2036 維持、レバ操作なし、peak lev 2.40x≤3.0x assert で機械検証 | DH_Dyn + VolRegime + AsymmHysteresis + BinaryRegimeGate | 2026-06-03 | D (moderate 0.10%) | +13.66% | 0.844 | −34.57% | **Shortlisted → ETF 環境 Active 候補 (v4.5 §7-2 min ルール下、ETF 制約下で最良)** | DH-W1 は ETF only 環境 (CFD 不可、NISA 等) で DH (min +9.56%) を +4.10pp 上回る唯一の改善案。CFD 環境では vz065_l7_F10eps015 (min +20.23%) に大きく劣後 (-6.57pp)。WFA 50窓 PASS (CI95_lo=+13.95%, **WFE=0.997 完全汎化**, p=0.0000, Trades 17.8/yr)。Bootstrap (W1 vs DH-REF) median +3.65pp positive, P(diff>0)=72%。Threshold (enter, exit) 24 組合せ sweep 全て OOS +12〜16.7% でロバスト。**ETF 制約環境の第一推奨**。 | [STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md](STRATEGY_PERFORMANCE_INTEGRATED_20260603-v2.md) §0'/§6-2.5/§6-3/§7-2 (v4.3/v4.5), [STRATEGY_DH_REFINEMENT_20260603.md](STRATEGY_DH_REFINEMENT_20260603.md), [src/g23a_dh_refinement_variants.py](src/g23a_dh_refinement_variants.py), [g23d_dh_w1_wfa.csv](g23d_dh_w1_wfa.csv), [g23e_dh_w1_bootstrap_results.csv](g23e_dh_w1_bootstrap_results.csv), [g23f_dh_w1_threshold_sweep.csv](g23f_dh_w1_threshold_sweep.csv) |
