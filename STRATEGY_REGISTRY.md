...
> **⚠ 「ベスト戦略（分析）」≠「本番稼働中」**: 本番Active（スプレッドシート稼働中）は依然 **E4 RegimeKLT**、**P09 は GAS で並走運用中**。B3a_k365 はユーザー決定（2026-06-15）による分析上のベスト戦略で、**本番切替（E4→P09 等）は7月上旬の別判断**。E4 の指標行は §2（fallback）に保全。
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **B3a_k365** 🏆ベスト戦略<br>DH-W1 + V7マップ<br>{Q0:1.40,Q1:1.40,<br>Q2:1.05,Q3:1.00}×uniform1.15<br>+ P09 OUT充填 + C1<br>≤3x=TQQQ / >3x=くりっく株365 | 2026-06-15 | realistic TQQQ+k365（⓽=×0.8273） | +23.10% / **+20.98%**（**min +20.98%**、選択バイアス割引後≈20.1%） | +2.57pp | 0.904 | −38.20% | +14.53% | +8.08% | 33.3 | ✅ 0.987 | **+22.52%** | **🏆 ベスト戦略（分析確定 2026-06-15・CAGR重視）。本番切替は7月判断** | CAGR重視のベスト。全ゲートPASS（ハードベト無・WFA正典49窓 α/β・CPCV p10+16.01%・Regime_min−2.88%・multi-metric bootstrap）。min⓽ +20.98%（点推定・best-of-44で割引後≈20.1%・bootstrap下限18.2%）は P09候補(+17.77%)を上回る。代償: MaxDD−38.2%（P09比−3.2pp）・2008防御+16.78%（P09比−3.08pp弱体）・**Sharpeは対P09中立**（レバはrisk-adjusted不変＝純レバアップ）。boot P(min vs V7)=0.893（95%未達・年次blockで0.94接近）。B3c(uniform1.10)はDD約1pp浅い代替。独立再実装QCで指標再現済。 | [LEVERUP_SWEEP_RESULTS_20260612.md](LEVERUP_SWEEP_RESULTS_20260612.md), [LEVERUP_QC_SIGNOFF_20260613.md](LEVERUP_QC_SIGNOFF_20260613.md), [src/audit/leverup_sweep_20260612.py](src/audit/leverup_sweep_20260612.py), [src/audit/k365_recost_20260612.py](src/audit/k365_recost_20260612.py), [src/audit/multimetric_bootstrap_20260615.py](src/audit/multimetric_bootstrap_20260615.py), [CURRENT_BEST_STRATEGY.md](CURRENT_BEST_STRATEGY.md) |
