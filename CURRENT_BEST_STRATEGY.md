...
| **ETF only (NISA等)** | **DH-W1** (Asymm Hysteresis) | IS +18.10% / OOS +18.91%（**min +18.10%**） | −0.81pp | 0.845 | −34.57% | +10.37% | +4.82% | 68.7 | ✅ 1.023 | +13.61% | 🟡 ETF 環境 Active 候補 |

> ⚠ **§1 Active（E4: CAGRⓒ_OOS +33.53%）との比較注意**: E4 は税引前ⓒ値。税後⓽換算の概算は約 +26.5%（→ 詳細は下記§1 主要指標参照）。vz=0.65+l5+F10ε の +18.93% は税後⓽で直接比較可。
...
| CAGR ⓽_OOS（税後・§3-A概算） | **≈ +26.5%**（保守値 ≈ +24.6%） | (33.53−0.66)×0.8273 ≈ +27.2%。コスト過少計上 −66bps 補正後: (30.5−0.66)×0.8273 ≈ +24.6% |
...
| 売買税ドラッグ (年27回、税率20.315%) | §3-A 逐年適用 → CAGR⓽_OOS ≈ **(30.5−0.66)×0.8273 ≈ +24.6%（最終手取り推計）** |
