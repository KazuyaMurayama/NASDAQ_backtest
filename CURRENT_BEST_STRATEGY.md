...
作成日: 2026-05-11
最終更新日: **2026-06-26 (v9: ベスト戦略を P09_STR_sc1.6 に昇格[ユーザー決定 2026-06-26]。旧ベスト B3a_k365 はベスト戦略候補に保全。一次根拠 SCALE2_PROMOTION_ELIGIBILITY_20260626.md[独立4桁再現・経路頑健MaxDD・gap純レバ幾何効果]・P09_STR_SCALE_DIAL_20260623.md。本番稼働は E4 継続・P09 GAS並走・切替判断7月)** / **2026-06-15 (v8: ベスト戦略を B3a_k365、ベスト戦略候補を P09_C1 に確定。冒頭 §🏆 に追記。一次根拠 LEVERUP_SWEEP_RESULTS_20260612.md・QC LEVERUP_QC_SIGNOFF_20260613.md[独立再実装で§6.1値80/79再現]。本番稼働は E4 継続・P09 GAS並走・切替判断7月)** / 2026-06-07 (v4.9.2: 税後 CAGR を canonical split (IS_END=2021-05-07) で統一。`scripts/compute_aftertax_cagr_v3_20260607.py` で全戦略を pretax と同じ canonical split に同期、calendar/canonical split duality を廃止) / 2026-06-08 (標準10指標・IS/OOS min 表記・ⓒ/⓽コスト前提明確化 全セクション適用) / 2026-06-10 (v4.5推奨表: 全値⓽に統一・比較注意書き削除、V0/V7 overlay・P7投信の3行追加) / 2026-06-10 v2 (v4.5推奨表: E4 ⓽行追加で§1 Active vs vz065_l5 を同一基準で直接比較可能化) / **2026-06-10 v3 (コスト誤謬修正: E4 CAGR⓽ を CFD_SPREAD_LOW=0.20%/yr の誤値 +27.41% から SBI CFD 3.0% 正値 +20.0% に修正)** / **2026-06-10 v4 (構成・コスト注意事項削除、一次根拠を SBI CFD g14 ベースに更新、Shortlisted から CFD_SPREAD_LOW 誤値を除去)** / **2026-06-11 v5 (v4.5表を realistic full L×(SBI CFD建玉金利=想定元本全額, 一次確認済) + 正典窓(49窓)WFA で更新。E4 ⓽OOS: g14 (L-1)×借入基準 +22.4% → full L×正基準 +18.06% に訂正。R4: DH-W1 Trades 68.7→17.6 訂正(NAV符号反転の疑似指標を実リバランス値に修正)。R9: vz065_l7 N/A→CI95+16.45%/WFE1.328 補填。⚠マーク除去・vz065 WFE CAUTION注記に置換)** / **2026-06-11 v6 (ETF指定戦略 DH-W1/V0/V7/P7 の NASDAQ脚コストを CFD→TQQQ に是正。設計商品TQQQの正値で点指標を更新: DH-W1 min +14.73→+15.85%, V0 +13.43→+14.27%, V7 +15.07→+16.27%, P7 +15.84→+16.92%。CFD環境(E4/vz065)は不変。WFA列は†CFD基準据置・TQQQ再計算pending)** / **2026-06-11 v7 (ベスト候補再構成: CFD戦略 E4/vz065_l7 と ETF DH-W1/V0 を §アーカイブへ退避[削除せず指標保存]、vz065_l5 はCFD候補として残置[ユーザー判断]、V7-TQQQ/P09_TQQQ/LU1 を主候補・LU2 を別枠に追加。v6 の † WFA を TQQQ基準・正典窓49窓で再計算し確定[CFD分枝が旧†値を dWFE≤0.0002 で再現]、† 解除。CI95_lo: V7 +14.09→+15.48 / P7 +16.57→+17.85。標準10指標列を compute_10metrics §3.12 と全件整合確認。E4 は本番Active継続中=切替はユーザー判断)** / **2026-06-11 v7.2 (P09_TQQQ Active候補昇格を反映、最終確定評価レポート HORIZON_AND_SCORECARD_20260611.md への参照を追加[6次元スコアカード P09 8.12首位級・5y/10y分布・年次リターン表]、一次根拠ファイル表に同レポートと EVALUATION_UPGRADE_RESULTS_20260611.md を追加)**

---

## 🏆 現行ベスト戦略（v9・2026-06-26 確定）

> **ベスト戦略 = P09_STR_sc1.6 ／ ベスト戦略候補 = B3a_k365・P09_C1**（ユーザー決定 2026-06-26）。
> 一次根拠: [SCALE2_PROMOTION_ELIGIBILITY_20260626.md](SCALE2_PROMOTION_ELIGIBILITY_20260626.md)（昇格適格性 再検証：確定値の独立4桁再現・経路頑健MaxDD・IS-OOS gapは純レバの幾何効果と判明）・[P09_STR_SCALE_DIAL_20260623.md](P09_STR_SCALE_DIAL_20260623.md)（strong-mapダイヤル・FRONTIER 4桁再現サニティPASS）。
> 全値⓽税後（×0.8273・特定口座20.315%課税。TQQQ/くりっく株365 はNISA対象外）。min(IS,OOS)=保守的採用基準。

### ベスト戦略: P09_STR_sc1.6（CAGR重視・レバ拡張点）

**構成**: DH-W1（Asymm Hysteresis, Enter≥0.7/Exit≤0.3）＋ mom63 **strong ブーストマップ {Q0:1.60, Q1:1.50, Q2:1.10, Q3:1.00}** × uniform leverage **scale ×1.6** ＋ P09 OUT充填（Gold常時＋Bond@`bond_mom252>0`・逆ボラW63・T+5ラグ）＋ **C1**（OUT∧bondOFF日に現金利回りSOFR計上）。**コスト**: ≤3×=TQQQ（swap0.5%+TER0.86%）、>3×超過分=くりっく株365（取引所CFD・SOFR+0.75pp ≒ 超過0.25%/yr・strong は全IN日 L≥3 のため常時計上）。

| 指標（⓽税後 / ⓒ税前） | 値 | 備考（生の計測値） |
|---|---:|---|
| CAGR_IS⓽ / CAGR_OOS⓽ | +30.26% / **+26.21%** | min(IS,OOS)=OOS |
| IS-OOS gap⓽ | +4.90pp | gapは純レバの幾何効果（OUT年scale不変・強気年だけ振幅拡大、§昇格検証） |
| Sharpeⓒ / MaxDDⓒ | +1.055 / **−51.95%** | MaxDDは −50% をわずかに超過（scale比例の経路深化） |
| Worst10Y★⓽ | +19.36% | 10年テールは scale 上げで上昇 |
| WFA CI95_loⓡ / WFEⓞ | +30.52% / 0.898 | α/β PASS（WFEは1未満） |
| Regime_min（弱気相場CAGR） | −7.71% | 弱気相場で年−7.71%（レバ水準の帰結） |
| 最大実効レバ / 経路集約 | 7.68x / TUW93.4%・avgDD−14.59%・CVaR5%DD−39.54% | 全IN日L≥3＝くりっく株365証拠金/容量制約 |
| 最深DD局面 | 2004-01-20→2005-04-05・回復649日 | 単発でなく経路全体が深化（過小評価なし） |

**昇格の位置づけ（生の計測値・リスク選好判断）**: P09_STR_sc1.6 は旧ベスト B3a_k365（CAGR_OOS +20.98%）に対し **CAGR_OOS を +26.21% に引き上げる（+5.23pp）**。対価として MaxDD −38.2%→**−51.95%**、Regime_min −2.88%→**−7.71%**、最大実効レバ ≤4.83x→**7.68x** を引き受ける。独立QC（[SCALE2_PROMOTION_ELIGIBILITY_20260626.md](SCALE2_PROMOTION_ELIGIBILITY_20260626.md)）で確定値を4桁再現、IS-OOS gap +4.90pp は「IS期に都合よく効いた過学習」でなく「下方を守り上方の振幅だけ拡大する純レバの幾何効果」（暴落・OUT年 2001/2002/2008/2022 は完全にscale不変）と判明。**Sharpe（risk-adjusted）は純レバアップのため改善せず**、より攻めた scale2.0（CAGR +29.11%・MaxDD −61.63%）と、より保守的な scale1.4（CAGR +24.34%・MaxDD −46.48%・「−50%以内」）の中間点としてユーザーが選定。**最大実効レバ 7.68x の証拠金/容量制約は実務前提**（[MARGIN_CAPACITY_STRESS_RESULTS_20260617.md](MARGIN_CAPACITY_STRESS_RESULTS_20260617.md)：8%証拠金＋容量内AUMが採用条件）。

### ベスト戦略候補①: B3a_k365（旧ベスト・バランス重視）

**構成**: DH-W1 ＋ mom63 V7ブーストマップ **{Q0:1.40, Q1:1.40, Q2:1.05, Q3:1.00}** × uniform leverage **×1.15** ＋ P09 OUT充填 ＋ **C1**。

| 指標（⓽税後 / ⓒ税前） | 値 | 備考 |
|---|---:|---|
| CAGR_IS⓽ / CAGR_OOS⓽ | +23.10% / **+20.98%** | min(IS,OOS)=OOS・割引後≈20.1% |
| IS-OOS gap⓽ | +2.57pp | 過学習兆候は軽微 |
| Sharpeⓒ / MaxDDⓒ | 0.904 / **−38.2%** | MaxDDは sc1.6 比 +13.75pp 浅い |
| Worst10Y★⓽ / P10_5Y⓽ / Worst5Y⓽ | +14.53% / +8.08% / +0.10% | |
| WFA CI95_loⓡ / WFEⓞ | +22.52% / 0.987 | WFEはsc1.6より良好（0.987>0.898） |
| Regime_min / Trades/yrⓞ | −2.88% / 33.3 | |

**位置づけ**: 旧ベスト（〜2026-06-15）。MaxDD・WFE（汎化）・Regime_min で sc1.6 より良好＝**リスク効率・保守性を優先する場合の第一候補**。sc1.6 への切替を見送る/巻き戻す際の即時 fallback。
...
P09_STR_sc1.6 は WFA正典49窓 α/β PASS（CI95_lo +30.52%・WFE 0.898）。確定値は独立QC（[SCALE2_PROMOTION_ELIGIBILITY_20260626.md](SCALE2_PROMOTION_ELIGIBILITY_20260626.md)）で4桁再現。MaxDD −51.95% は −50% をわずかに超過し、経路集約指標（TUW93.4%・avgDD−14.59%・CVaR5%DD−39.54%）でも scale 比例に深化（過小評価なし・R-STAT-1遵守で block=21未使用）。IS-OOS gap +4.90pp は純レバの幾何効果。**昇格は「CAGR_OOS +20.98%→+26.21%（+5.23pp）取得 vs MaxDD −38.2%→−51.95%・Regime −2.88%→−7.71%・最大実効レバ 7.68x」のリスク選好判断**（ユーザー決定 2026-06-26・CAGR優先＋DD許容拡大と整合）。候補①B3a_k365 は MaxDD/WFE/Regime で sc1.6 を上回る保守的 fallback。
...
|---|---:|---:|---:|---:|---:|---|
| 1.4 | +24.34% | −46.48% | +17.87% | −5.86% | 0.936 | −50%以内・最も保守的な拡張点（保守選好の代替） |
| **1.6（🏆現ベスト・2026-06-26）** | **+26.21%** | −51.95% | +19.36% | −7.71% | 0.898 | **ベスト戦略に昇格。MaxDDは−50%をわずかに超過・gap+4.90ppは純レバ幾何効果。scale1.4と2.0の中間点** |
| 1.8 | +27.81% | −57.00% | +20.22% | −9.56% | 0.858 | より攻め（MaxDD−57%） |
| 2.0 | +29.11% | −61.63% | +20.84% | −11.41% | 0.818 | CAGR最高だがMaxDD−61.63%・Regime−11.41%・最大実効レバ9.6x（[昇格検証](SCALE2_PROMOTION_ELIGIBILITY_20260626.md)で見送り） |

**純レバアップ**＝Sharpe非改善（1.07→1.03漸減）・MaxDDは scale比例で悪化（[SCALE2_PROMOTION_ELIGIBILITY_20260626.md](SCALE2_PROMOTION_ELIGIBILITY_20260626.md)：経路集約指標でも一様に深化）。**現ベスト sc1.6 はリスク効率（Sharpe/WFE）では scale1.4 に劣るが、CAGR_OOS +26.21% を優先したユーザー選定点**。より保守的にするなら scale1.4（MaxDD −46.48%）、より攻めるなら scale2.0（CAGR +29.11%・ただしMaxDD −61.63%）へレバ点を移動。**本番切替は7月判断**。

> **❌ B1×Scale（不採用・2026-06-21）**: B1(P09_C1+IN脚下方偏差ブレーキ)×scale で「高CAGR×DD抑制両立」を検証したが、**同MaxDD帯で P09×低スケールが両軸(CAGR・MaxDD)で同等以上＝被支配**。**B1×2.0(+24.07%/−47.08%) ≈ P09×1.4(+24.34%/−46.48%)**で、ブレーキは実効的に「低スケール化装置」。**レバ拡張は B1 でなく P09 で実施すること**。詳細＝[B1_SCALE_FRONTIER_20260621.md](B1_SCALE_FRONTIER_20260621.md)、STRATEGY_REGISTRY §3.4。関連: A7 IN脚ボラブレーキ／A0配分A1-A6 も全て「時機でなくデレバ・ノイズ」で不採用＝**DD削減はレバ水準ダイヤル（B3c/P09低スケール）が本質**。**なぜIN脚ブレーキが危機で効かないか**: DH-W1は主要暴落中（2000/2008/2020/2022）**既に94-100% OUT（現金/Gold/Bond）**で、IN脚ブレーキに作用対象がない＝危機はOUT-fillが処理済。経路頑健な暴落窓検定で全ブレーキ TIMING_WEAK（[EVALUATION_STANDARD.md](EVALUATION_STANDARD.md) §4.4 R-STAT-2）。

### ⚙ 運用ステータス（重要・「ベスト戦略」≠「本番稼働中」）

- **本番Active（スプレッドシート稼働中）= E4 RegimeKLT**（下記 §アーカイブ）。**P09 は GAS（NASDAQ-strategy-gas リポ）で並走運用中**。
- P09_STR_sc1.6 を「ベスト戦略」と確定したのは**分析上の結論**（2026-06-26・旧 B3a_k365 から昇格）。本番切替は別判断で、**E4→P09 切替判断は7月上旬**予定。P09_STR_sc1.6 は strong-map×scale1.6＝GAS の BOOST_MAP を {1.60,1.50,1.10,1.00}・uniform leverage scale を 1.6 に変更すれば追従可能（最大実効レバ 7.68x のため証拠金8%・容量内AUMが前提）。保守化する場合は scale を 1.4 へ、巻き戻す場合は B3a_k365（v7マップ×1.15）へ。
...
- ❌ **税基準の混在**（2026-06-22）: 年次リターンを co-list する際、**全系列の税基準（pre-tax / after-tax ×0.8273）を揃えない**こと。特に `audit_results/annual_returns_sc180_200_20260620.csv`（旧P09×scale年次）は **pre-tax**（`NASDAQ_1x_BH` 2008=−40.54% が目印）。after-tax戦略と並べるなら ×0.8273 を適用。混合は B1_SCALE_FRONTIER 生成時にQCで検出・是正済の既知トラップ。係数根拠 `EVALUATION_STANDARD.md` §1。
- ❌ **経路依存極値の誤検定**（2026-06-22）: MaxDD/Worst10Y を **block=21 でブートストラップ**して「有意差なし＝効果なし」と結論すること（経路依存極値に無効・P≈0.5固定）。時機検定は無傷の暴落窓＋サイン検定で（`EVALUATION_STANDARD.md` §4.4 R-STAT-1/2）。
