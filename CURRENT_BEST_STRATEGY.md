...
| CAGR ⓽_OOS（税後・§3-A概算） | **⚠ 要注意（コスト前提依存）** | **SBI CFD (3.0%/yr) ベース** [一次根拠]: `7STRATEGY_PERFORMANCE_REPORT_20260529.md` → OOS **+22.4%** / IS ≈ **+20.0%** / **min +20.0%**（g14 WFA PASS、CI95_lo⓽=+16.3%）。— くりっく株365 (0.20%/yr) ベース [参考]: `aftertax_cagr_canonical_20260607.csv` → IS +27.41% / OOS +28.01%（SBI CFD ユーザーには非適用）。 |
...
| CAGR_IS (税前 ⓒ, canonical split) | +16.69% | **+18.61%** |
| CAGR_OOS (税前 ⓒ, canonical) | +18.03% | **+19.14%** |
...
| P10_5Y▷ | +5.21% | +5.22% |
| IS-OOS gap | −1.34pp | **−0.53pp** (overlay候補中最良) |
...
**税後 CAGR (canonical split, IS_END=2021-05-07)**: [SIGNAL_EXPANSION_FINAL_DECISION_20260607.md](SIGNAL_EXPANSION_FINAL_DECISION_20260607.md) §3.6 参照。yearly ×0.8273 applied to periodized returns。
- **NISA 内 (非課税)**: pretax 値そのまま (V7 CAGR_OOS = **+19.14%**, V0 = **+18.03%**)
- **課税口座 ×0.8273**: V7 CAGR_OOS aftertax = **+15.96%**, V0 = **+15.02%**
- IS aftertax: V7 **+15.74%**, V0 **+14.07%** / FULL aftertax: V7 **+15.76%**, V0 **+14.16%**
- **V7 min CAGR (NISA)**: min(IS+18.61%, OOS+19.14%) = **+18.61% > 18% target 達成** (gap +0.61pp)
- **V7 min CAGR (課税口座)**: min(IS+15.74%, OOS+15.96%) = **+15.74%**

詳細データ: [S3_OVERLAY_TUNING_REPORT_20260607.md](S3_OVERLAY_TUNING_REPORT_20260607.md) §6.2/§11 (canonical aftertax) / [s3_overlay_tuning_20260607.csv](data/signals/expansion/s3_overlay_tuning_20260607.csv) / [aftertax_cagr_canonical_20260607.csv](data/signals/expansion/aftertax_cagr_canonical_20260607.csv)
