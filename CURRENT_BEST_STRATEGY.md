...
| **ETF only (NISA等)** | **DH-W1** (Asymm Hysteresis) | IS **+15.80%** / OOS **+12.57%**（**min +12.57%**）†⓽<br>_ⓒ canonical: IS+18.10%/OOS+18.91%_ | +3.23pp(⓽) | 0.845 | −34.57% | +10.37% | +4.82% | 68.7 | ✅ 1.023 | +13.61% | 🟡 ETF 環境 Active 候補 |

> ⚠ **コスト・税の比較注意**:
> - **E4 Active**: ⓒ +33.53% / ⓽公式(aftertax_cagr) **+20.99%**（calendar OOS + moderate spread + §3-A）
> - **vz=0.65+l5+F10ε**: +18.93% は integrated report §0' ⓽（OOS 2021-2022〜2026 approx、moderate 0.05% + §3-A 適用）→ E4 ⓽ と直接比較可
> - **DH-W1**: ⓽ **+12.57%** OOS（aftertax_cagr calendar 2020-2026）→ 上の ⓒ +18.91% と混同しないこと
...
| CAGR ⓽_OOS（税後・§3-A概算） | **+20.99%**（公式）/ 単純推計≈+27.2% | 公式⓽: `aftertax_cagr_20260607.csv` → **+20.99%** (calendar OOS + moderate spread 0.05% + §3-A)。単純§3-A推計: (33.53−0.66)×0.8273≈+27.2%（moderate spread未加算）。保守: (30.5−0.66)×0.8273≈+24.6% |
