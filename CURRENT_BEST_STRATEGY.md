...
> **v7 のベスト候補は ETF/投信（NISA非課税）環境が中心。** CFDからは E4・vz065_l7 を §アーカイブへ退避し、**vz065_l5 のみ CFD候補として残置**（ユーザー判断 2026-06-11）。**現行本番 Active の E4 は実運用継続中**だが、NISA非課税・MaxDD/Sharpe 重視の選好下では主候補から外す。実運用の切替はユーザー判断。
>
...
| **CFD候補（課税環境・唯一）** | **vz=0.65+l5 (vz065_l5)** | IS +16.84% / OOS +20.85%（**min +16.84%**） | −4.01pp | 0.769 | −59.08% | +6.55% | +2.42% | 84.9 | 1.348 | +16.27% | 🔵 CFD候補（ユーザー判断2026-06-11で残置）。⚠ **WFE1.35 CAUTION**(postIS regime luck疑い)・20.3%課税・MaxDD−59%・Trades85/年。CFDを選択肢に残す場合の唯一候補。値はCFD基準・正典窓49窓 |
