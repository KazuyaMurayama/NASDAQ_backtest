...
**CAGRは scale を上げれば +24%近くまでベト内で伸びる（ベト到達≈scale1.50）が、純レバ＝Sharpe非改善・MaxDD/worst年(−26%)を比例的に食う・min改善は95%非有意**。フリーランチは無く、**レバ点はリスク選好で選ぶ**。B3aはバランス点として維持。CAGR最大化なら scale1.25（穏当）〜1.35強map（攻め）。
