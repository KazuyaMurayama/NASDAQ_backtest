# 戦略パフォーマンス統合レポート v4.4 — vz=0.65+l7+F10ε-AH (CFD 非対称機構移植) 採用 + 戦略名「New」廃止 (2026-06-03)
...
|:---------|-------------:|:------------:|-------------:|---------------:|------:|----------------------:|-------------------:|------------------:|:----------------:|-----------:|
| **🟢 vz=0.65+l7+F10ε** (REF) | **+21.49%** | OOS: **+21.49%**<br>IS: **+20.23%** | **-1.27pp** | **+0.829** | -65.95% | +9.96% | +4.05% | 52 ※ | ✅ LOW<br>(1.4) | **+0.199** |
| **🟢 vz=0.65+l7+F10ε-AH** ★v4.4 new (Asymm+Hyst, binary HOLD/OUT, hold ratio 53.1%) | **+26.53%** | OOS: **+26.53%**<br>IS: +18.90% | **-7.63pp** | **+0.954** | -64.89% | +8.45% | +1.73% | **56** (WFA) | ✅ LOW<br>(**1.554**) | +0.190 |
| **D5 vz=0.65/lmax=5.5** | +17.86% | OOS: +17.86%<br>IS: +20.08% | +2.22pp | +0.79 M | **-55.88%** | +12.21% | +6.76% | 28 ※ | ✅ LOW<br>(1.3) | +0.192 |
| **🟡 DH-W1 (Asymm+Hyst, DH base)** ★v4.3 new | **+13.66%** | OOS: +13.66%<br>IS: +15.61% | **+1.95pp** | +0.844 | **-34.57%** | +9.84% | +5.94% | 18 | ✅ LOW<br>(**1.00**) | +0.140 |
| **DH Dyn 2x3x [A]** | +9.56% | OOS: +9.56%<br>IS: +20.02% | +10.46pp ⚠⚠ | +0.60 | -41.57% | **+12.57%** | **+8.77%** | 27 ※ | ✅ LOW<br>(0.7) | +0.175 |
| **NDX 1x B&H** (Benchmark) | +8.27% | OOS: +8.27%<br>IS: +9.91% | +1.64pp | +0.516 | -77.93% | -4.85% | -0.08% | 0 | — | — |

太字 = 列最良値 / 🟢 = v6.3 で「条件付き昇格」確定 / 🟡 = v4.3 で採用 DH 改善版 (要追加検証) / ★v4.3 new = v4.3 で新規追加 (DH-W1, Asymm+Hysteresis 状態機械 + DH base 配分) / ⚠⚠ = IS-OOS gap > +5pp (古典 overfit 強警戒) / ⚠ = +5〜+8pp (中度 overfit)
...
| Strategy | CAGR<br>⓽<br>_<br>OOS | 累積<br>CAGR<br>⓽<br>OOS/IS | IS-OOS<br>gap<br>CAGR | Sharpe<br>ⓒ<br>_OOS | MaxDD<br>ⓒ | Worst<br>10Y★<br>⓽<br>CAGR | P10<br>⓽<br>5Y▷<br>CAGR | Trade<br>ⓞ<br>(回/<br>年) | Overfit<br>ⓞ<br>(WFE) | CI95<br>ⓡ<br>_lo |
|:---|---:|:---:|---:|---:|---:|---:|---:|---:|:---:|---:|
| vz=0.65+l7+F10ε (REF) | +21.49% | OOS: +21.49%<br>IS: **+20.23%** | -1.27pp | +0.829 | -65.95% | **+9.96%** | +4.05% | 105 | 1.369 | **+19.85%** |
| **🟢 vz=0.65+l7+F10ε-AH** ★ | **+26.53%** | OOS: **+26.53%**<br>IS: +18.90% | -7.63pp | **+0.954** | -64.89% | +8.45% | +1.73% | **56** | 1.554 | +18.96% |
| ⚠ vz=0.65+l7+F10ε-AT | +27.91% | OOS: +27.91%<br>IS: +16.38% | -11.53pp | +1.066 | -63.89% | +6.62% | +1.58% | 109 | **1.988** ⚠ | +15.95% |
| ❌ vz=0.65+l7+F10ε-HL | +25.42% | OOS: +25.42%<br>IS: +19.86% | -5.55pp | +0.923 | -64.81% | +9.59% | +2.30% | 106 | 1.472 | +19.85% |

### 個別判定
- ✅ **vz=0.65+l7+F10ε-AH (Asymm Hysteresis) — 採用推奨**:
  - **CAGR_OOS +26.53%** (REF +21.49 から **+5.04pp ✅**)
  - IS は +20.23 → +18.90 で **-1.33pp のみ** (僅少な代償)
  - **Trades/yr 56** (REF 105 から **半減**、コスト削減)
  - Sharpe +0.954 (REF +0.829 から +0.125 改善)
  - WFE 1.554 (OOS > IS 維持、過学習なし)
  - MaxDD -64.89% (REF と同等、悪化なし)
  - ★ 機構の本質: hysteresis (Enter 0.7 / Exit 0.3) が 1981/2000/2008/2022 等の major bear を回避、bull は全力で乗る
- ⚠ vz=0.65+l7+F10ε-AT (Continuous Tilt) — **棄却候補**:
  - CAGR_OOS +27.91% (最高) だが **IS -3.85pp 大幅低下**、Worst10Y +6.62% で REF +9.96% から劣化
  - WFE 1.988 ≈ 2.0 (規定の上限近く、**regime luck の疑い**)
  - 連続 scaling のため smooth だが過剰 reduction の罠
- ❌ vz=0.65+l7+F10ε-HL (Hyst-Lite) — **棄却**:
  - CAGR_OOS +25.42% で AH に劣る、Trades 同等 (cost 削減なし)、AH に対する優位性なし

### 推奨アクション
1. **vz=0.65+l7+F10ε-AH を §0' Active 候補 (双頭運用) に正式追加** (DH-W1 と並列、ETF/CFD 両環境で運用可能に)
2. **WFA 50 窓 + Bootstrap (AH vs REF) + Permutation 追加検証** (v4.5 で実施推奨)
3. **AH の hysteresis 閾値 (0.7, 0.3) sensitivity sweep** (g23f と同じ手順で CFD 版実施)
...
| **v4.4** | **2026-06-03** | **vz=0.65+l7+F10ε への非対称機構移植検証 (§6-4 新規)** ─ DH-W1 の Asymm Hysteresis を CFD 戦略 vz=0.65+l7+F10ε に移植する 3 候補 (AH/AT/HL) を実装・検証。**vz=0.65+l7+F10ε-AH** (binary HOLD/OUT) が最良 ─ **CAGR_OOS +21.49→+26.53% (+5.04pp)**、IS -1.33pp のみ、**Trades 半減 (105→56)**、Sharpe +0.829→+0.954、WFE 1.554 ─ §0' 表に追記。AT (continuous tilt) は IS -3.85pp + WFE 1.988 で棄却候補、HL (Hyst-Lite) は AH に対する優位なしで棄却。<br>**併せて vz=0.65+l7+F10ε を「New」と呼称する全表記を削除** (戦略名はパラメータ表記に統一)。 |

---

*管理者: 男座員也 (Kazuya Oza)*
*生成: Claude (Opus 4.7) on 2026-06-03 (v4.3)*
*準拠: EVALUATION_STANDARD.md v1.4, v3.1 §3-A 税モデル, v6.1 日次取引コスト, v6.2 7戦略拡張, v6.3 5検証パッケージ, v4 DH 配分×タイミング 2 軸変動版, v4.3 Asymm+Hysteresis (DH-W1) 採用 + 5 戦略再検証*
