# 戦略パフォーマンス統合レポート v4.3 — DH 改善「Asymmetric+Hysteresis (DH-W1)」採用 + 5 戦略全指標検証 (2026-06-03)
...
| **NEW P10_5Y** | +5.84% | **+4.05%** | metrics_from_nav の rolling 5Y CAGR percentile 計算が更新された結果。canonical な再計算値を採用 |
...
|:---------|-------------:|:------------:|-------------:|---------------:|------:|----------------------:|-------------------:|------------------:|:----------------:|-----------:|
| **🟢 vz=0.65+l7+F10ε** (NEW CANDIDATE) | **+21.49%** | OOS: **+21.49%**<br>IS: **+20.23%** | **-1.27pp** | **+0.829** | -65.95% | +9.96% | +4.05% | 52 ※ | ✅ LOW<br>(1.4) | **+0.199** |
...
**※ Trades/yr 方式差**: 報告書既存 (NEW=52, D5=28, DH=27) は旧 g14 trade counter (`lev_change>0.5` 流儀)、DH-W1=18 は WFA 50 窓平均 `mean_Trades_yr` (g23d 由来)。両者は集計流儀差で値の意味が異なる (g24 再計算では同基準で NEW=105, D5=75)。**取引頻度の概念差**であり、互いに正しい。
...
| 年 | 🟢 NEW (vz=0.65<br>+l7+F10ε) | D5 (vz=0.65<br>/lmax=5.5) | 🟡 DH-W1 (Asymm<br>+Hyst, DH base)<br>★v4.3 new | DH Dyn 2x3x [A] | NDX 1x B&H |
|---:|---:|---:|---:|---:|---:|
...
| 統計 | 🟢 NEW (vz=0.65<br>+l7+F10ε) | D5 (vz=0.65<br>/lmax=5.5) | 🟡 DH-W1 (Asymm<br>+Hyst, DH base)<br>★v4.3 new | DH Dyn 2x3x [A] | NDX 1x B&H |
|---|---:|---:|---:|---:|---:|
...
- **平均リターン**: NEW 🟢 (+26.80%) が首位、D5 (+24.50%) が次点。DH-W1 (+16.91%) は DH [A] (+21.19%) に劣るが、DH-Z2 (+16.02%) よりは僅か上
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| **🟢 NEW CANDIDATE (vz=0.65+l7+F10ε)** | +26.2% | -22.2% | +80.3% | +47.5% | +41.3% | -12.8% | **×3.38** | **338万円** |
...
→ **NEW 🟢 は OOS 6 年で 100 万円 → 338 万円** で 5 戦略中最高。**DH-W1 (×2.16) は DH-REF (×1.73) を 43 万円 (24.9%) 上回り**、hysteresis が 2021 bull capture (+22.4% vs Z2 旧 +11.3%)、2022 bear avoidance (0.0% vs REF -24.1%)、2025 bull capture (+21.2% vs Z2 旧 +13.9%) のすべてを両立。Z2 比でも更に +16 万円改善。
...
| **v4.3** | **2026-06-03** | **DH-Z2 を DH-W1 (Asymm+Hysteresis, DH base) に置換** + **g24 5 戦略再検証で発見した小数値乖離を整合修正**。<br>(A) §0'/§5/§6/§6-2 4 箇所の DH-Z2 → DH-W1 (OOS CAGR +12.26→+13.66%、IS +14.42→+15.61%、MaxDD -38.92→-34.57%、Min -17.21→-13.73%、WFE 1.058→0.997)。<br>(B) 整合修正: DH gap +10.29→+10.46pp、NEW P10_5Y +5.84→+4.05%、B&H P10_5Y +0.59→-0.08%。<br>(C) Trades/yr の方式差 (旧 g14 trade counter vs WFA mean_Trades_yr) を footnote 明記。<br>(D) §9-v4.3 g23/g24 一次根拠追加、§9-v4 g22 系は履歴保持で残置 |
